...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Columns.56e94df55c96.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,openNodeAndParents:function(bk){this.__dp(this.getModel(),bk);this.buildLookupTable();}
,closeNode:function(bl){if(qx.lang.Array.contains(this.__di,bl)){qx.lang.Array.remove(this.__di,bl);this.fireDataEvent(G,bl);this.buildLookupTable();}
;}
...
,isNodeOpen:function(bo){return qx.lang.Array.contains(this.__di,bo);}
,_init:function(){this.__dh=new qx.data.Array();this.__di=[];this.__dj=[];this._initLayer();}
...
,this);}
,getLookupTable:function(){return this.__dh;}
,_reverseLookup:function(bp){return bp;}
,_getDataFromRow:function(bq){return this.__dh.getItem(bq);}
,_getSelectables:function(){return this.__dh;}
,getOpenNodes:function(){return this.__di;}
,isNode:function(br){return qx.ui.tree.core.Util.isNode(br,this.getChildProperty());}
,getLevel:function(bs){return this.__dj[bs];}
...
,_applyModel:function(bQ,bP){this.__di=[];if(bQ!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bQ.constructor,qx.data.marshal.MEventBubbling)){this.warn(H+h+k);}
;}
;bQ.addListener(J,this._onChangeBubble,this);this.__do(bQ);}
...
;}
;if(this.__dh.indexOf(bV)!=-1){this.__dm();}
;}
;}
,_onUpdated:function(event){if(this.__dk==null){this.__dk=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__dk.schedule();}
,_onOpen:function(event){var ca=event.getRow();var bY=this.__dh.getItem(ca);if(this.isNode(bY)){if(this.isNodeOpen(bY)){this.closeNode(bY);}
...
;break;};}
;}
,_beforeApplySelection:function(cg){if(cg.length===0&&this.getSelectionMode()===g){var cf=this.__ds();var ch=this.getLookupTable().indexOf(cf);if(ch>=0){cg.push(ch);}
;}
;}
,_afterApplySelection:function(){var ci=this.getSelection();if(ci.getLength()>0&&this.getSelectionMode()===g){this.__dr(ci.getItem(0));}
else {this.__dl=[];}
;}
,__dm:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this._itemWidth=0;var cm=[];this.__dj=[];var ck=-1;var cl=this.getModel();if(cl!=null){if(!this.isHideRoot()){ck++ ;cm.push(cl);this.__dj.push(ck);}
;if(this.isNodeOpen(cl)){var cj=this.__dn(cl,ck);cm=cm.concat(cj);}
;}
;if(!qx.lang.Array.equals(this.__dh.toArray(),cm)){this._provider.removeBindings();this.__dh.removeAll();this.__dh.append(cm);this.__dq();this._updateSelection();}
;}
,__dn:function(cn,cs){var cr=[];cs++ ;if(!this.isNode(cn)){return cr;}
...
;if(this.isNode(cu)){this.__dj.push(cs);cr.push(cu);if(this.isNodeOpen(cu)){var co=this.__dn(cu,cs);cr=cr.concat(co);}
;}
else {if(this.isShowLeafs()){this.__dj.push(cs);cr.push(cu);}
;}
;}
;cv.dispose();return cr;}
,__do:function(cw){if(!qx.lang.Array.contains(this.__di,cw)){this.__di.push(cw);this.fireDataEvent(t,cw);}
;}
,__dp:function(cx,cy){if(cx===cy){this.__do(cy);return true;}
;if(!this.isNode(cx)){return false;}
;var cB=cx.get(this.getChildProperty());if(cB==null){return false;}
;for(var i=0;i<cB.getLength();i++ ){var cA=cB.getItem(i);var cz=this.__dp(cA,cy);if(cz===true){this.__do(cA);return true;}
;}
;return false;}
,__dq:function(){this.getPane().getRowConfig().setItemCount(this.__dh.getLength());this.getPane().fullUpdate();}
,getParent:function(cD){var cE=this.__dh.indexOf(cD);if(cE<0){return null;}
;var cF=this.__dj[cE];while(cE>0){cE-- ;var cC=this.__dj[cE];if(cC<cF){return this.__dh.getItem(cE);}
;}
;return null;}
,__dr:function(cG){this.__dl=[];var parent=this.getParent(cG);while(parent!=null){this.__dl.unshift(parent);parent=this.getParent(parent);}
;}
,__ds:function(){if(this.__dl==null){return this.getModel();}
;var cH=this.getLookupTable();var parent=this.__dl.pop();while(parent!=null){if(cH.contains(parent)){return parent;}
;parent=this.__dl.pop();}
...
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__dk!=null){this.__dk.cancel();this.__dk.dispose();}
;var cI=this.getModel();if(cI!=null){cI.removeListener(J,this._onChangeBubble,this);}
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__dh.dispose();this._layer=this._provider=this.__dh=this.__di=this.__dk=null;}
});}
)();
...
(function(){var a="BindingIds",b="model",c="",d="changeDelegate",f="virtual-tree-file",g="label",h="appearance",i="icon",j="bindItem",k=".length",l="]",m="virtual-tree-folder",n="[",o=".",p="String",q="qx.ui.tree.core.MWidgetController";qx.Mixin.define(q,{construct:function(){this.__dB=[];}
,properties:{labelPath:{check:p,nullable:true},iconPath:{check:p,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},childProperty:{check:p,nullable:true},delegate:{event:d,init:null,nullable:true}},members:{__dB:null,bindDefaultProperties:function(u,s){this.bindProperty(c,b,null,u,s);this.bindProperty(this.getLabelPath(),g,this.getLabelOptions(),u,s);var r=this.__dC(s);var t=this._tree.getLookupTable();t=qx.data.SingleValueBinding.resolvePropertyChain(t,r);if(qx.util.OOUtil.hasProperty(t.constructor,this.getChildProperty())){this.bindProperty(this.getChildProperty()+k,h,{converter:function(){return m;}
},u,s);}
else {u.setAppearance(f);}
;if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),i,this.getIconOptions(),u,s);}
;}
,bindProperty:function(y,B,z,x,v){var A=this.__dC(v,y);var w=this._tree.getLookupTable();var C=w.bind(A,x,B,z);this.__dD(x,C);}
,bindPropertyReverse:function(J,F,H,I,D){var G=this.__dC(D,J);var E=this._tree.getLookupTable();var K=I.bind(F,E,G,H);this.__dD(I,K);}
,removeBindings:function(){while(this.__dB.length>0){var L=this.__dB.pop();this._removeBindingsFrom(L);}
;}
...
;}
,_removeBindingsFrom:function(P){var Q=this.__dE(P);while(Q.length>0){var R=Q.pop();try{this._tree.getLookupTable().removeBinding(R);}
catch(e){P.removeBinding(R);}
;}
;if(qx.lang.Array.contains(this.__dB,P)){qx.lang.Array.remove(this.__dB,P);}
;}
,__dC:function(T,U){var S=n+T+l;if(U!=null&&U!=c){S+=o+U;}
;return S;}
,__dD:function(W,X){var V=this.__dE(W);if(!qx.lang.Array.contains(V,X)){V.push(X);}
;if(!qx.lang.Array.contains(this.__dB,W)){this.__dB.push(W);}
;}
,__dE:function(Y){var ba=Y.getUserData(a);if(ba==null){ba=[];Y.setUserData(a,ba);}
;return ba;}
...
;var o=this._renderer.getCellWidget();o.setOpen(p&&this._tree.isNodeOpen(s));o.addListener(a,this.__dF,this);o.setUserData(b,this.getChildProperty());o.setUserData(g,this._tree.isShowLeafs());if(this._tree.getSelection().contains(s)){this._styleSelectabled(o);}
...
,poolCellWidget:function(u){u.removeListener(a,this.__dF,this);this._removeBindingsFrom(u);this._renderer.pool(u);this._onPool(u);}
...
,__dF:function(event){var K=event.getTarget();var L=K.getUserData(k);var J=this._tree.getLookupTable().getItem(L);if(event.getData()){this._tree.openNodeWithoutScrolling(J);}
...
(function(){var a="source",b="changeLeadIcon",c="changeSize",d="changeMode",e="changeDate",f="checked",g="changeChecked",h="icon/16/places/user-desktop.png",i="value",j="size",k="leadIcon",l="date",m="mode",n="demobrowser.demo.virtual.tree.TreeItem",o="String",p="Boolean";qx.Class.define(n,{extend:qx.ui.tree.VirtualTreeItem,properties:{leadIcon:{check:o,event:b,nullable:true},checked:{check:p,event:g,nullable:true},size:{check:o,event:c,nullable:true},date:{check:o,event:e,nullable:true},mode:{check:o,event:d,nullable:true}},members:{__Ks:null,__Kt:null,__Ku:null,__Kv:null,__eG:null,_addWidgets:function(){var q=this.__Ks=new qx.ui.basic.Image();this.bind(k,q,a);q.setWidth(16);this.addWidget(q);this.addSpacer();this.addOpenButton();this.addIcon();this.setIcon(h);var r=this.__Kt=new qx.ui.form.CheckBox();this.bind(f,r,i);r.bind(i,this,f);r.setFocusable(false);r.setTriState(true);r.setMarginRight(4);this.addWidget(r);this.addLabel();this.addWidget(new qx.ui.core.Spacer(),{flex:1});var s=this.__Ku=new qx.ui.basic.Label();this.bind(j,s,i);s.setWidth(50);this.addWidget(s);s=this.__Kv=new qx.ui.basic.Label();this.bind(l,s,i);s.setWidth(150);this.addWidget(s);s=this.__eG=new qx.ui.basic.Label();this.bind(m,s,i);s.setWidth(80);this.addWidget(s);}
}});}
)();
