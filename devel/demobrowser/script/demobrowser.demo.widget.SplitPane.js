...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.SplitPane.1bde6df0d0d8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="slider",b="splitter",c="px",d="Integer",f="height",g="move",h="maxHeight",i="resize",j="pointerup",k="vertical",l="width",m="_applyOrientation",n="_applyOffset",o="splitpane",p="qx.ui.splitpane.Pane",q="pointerdown",r="top",s="minHeight",t="knob",u="horizontal",v="minWidth",w="appear",z="losecapture",A="left",B="pointermove",C="maxWidth",D="pointerout";qx.Class.define(p,{extend:qx.ui.core.Widget,construct:function(E){qx.ui.core.Widget.call(this);this.__fu=[];if(E){this.setOrientation(E);}
else {this.initOrientation();}
;this.__gy.addListener(q,this._onPointerDown,this);this.__gy.addListener(j,this._onPointerUp,this);this.__gy.addListener(B,this._onPointerMove,this);this.__gy.addListener(D,this._onPointerOut,this);this.__gy.addListener(z,this._onPointerUp,this);}
,properties:{appearance:{refine:true,init:o},offset:{check:d,init:6,apply:n},orientation:{init:u,check:[u,k],apply:m}},members:{__gr:null,__gs:false,__gt:null,__gu:null,__gv:null,__gw:null,__gx:null,__fu:null,__gy:null,_createChildControlImpl:function(H,G){var F;switch(H){case a:F=new qx.ui.splitpane.Slider(this);F.exclude();this._add(F,{type:H});break;case b:F=new qx.ui.splitpane.Splitter(this);this._add(F,{type:H});F.addListener(g,this.__gz,this);break;};return F||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,H);}
,__gz:function(e){this.__gB(e.getData());}
,__gA:function(K){this.__gy=new qx.ui.splitpane.Blocker(K);this.getContentElement().add(this.__gy);var I=this.getChildControl(b);var J=I.getWidth();if(!J){I.addListenerOnce(w,function(){this.__gB();}
,this);}
;I.addListener(i,function(e){var L=e.getData();if(L.height==0||L.width==0){this.__gy.hide();}
else {this.__gy.show();}
;}
,this);}
,getBlocker:function(){return this.__gy;}
,_applyOrientation:function(Q,N){var R=this.getChildControl(a);var M=this.getChildControl(b);this.__gv=Q===u;if(!this.__gy){this.__gA(Q);}
;this.__gy.setOrientation(Q);var P=this._getLayout();if(P){P.dispose();}
;var O=Q===k?new qx.ui.splitpane.VLayout:new qx.ui.splitpane.HLayout;this._setLayout(O);M.removeState(N);M.addState(Q);M.getChildControl(t).removeState(N);M.getChildControl(t).addState(Q);R.removeState(N);R.addState(Q);qx.ui.core.queue.Manager.flush();this.__gB();}
,_applyOffset:function(T,S){this.__gB();}
,__gB:function(V){var U=this.getChildControl(b);var ba=this.getOffset();var bb=U.getBounds();var Y=U.getContentElement().getDomElement();if(!Y){return;}
;if(this.__gv){var X=null;if(V){X=V.width;}
else if(bb){X=bb.width;}
;var bc=V&&V.left;if(X){if(isNaN(bc)){bc=qx.bom.element.Location.getPosition(Y).left;}
;this.__gy.setWidth(ba,X);this.__gy.setLeft(ba,bc);}
;}
...
;this.__gy.setHeight(ba,W);this.__gy.setTop(ba,top);}
;}
;}
...
,_onPointerDown:function(e){if(!e.isLeftPressed()){return;}
;var bg=this.getChildControl(b);var bj=bg.getContentLocation();var bh=this.getContentLocation();this.__gr=this.__gv?e.getDocumentLeft()-bj.left+bh.left:e.getDocumentTop()-bj.top+bh.top;var bi=this.getChildControl(a);var bk=bg.getBounds();bi.setUserBounds(bk.left,bk.top,bk.width,bk.height);bi.setZIndex(bg.getZIndex()+1);bi.show();this.__gs=true;this.__gy.capture();e.stop();}
,_onPointerMove:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());if(this.__gs){this.__gC();var bl=this.getChildControl(a);var bm=this.__gw;if(this.__gv){bl.setDomLeft(bm);this.__gy.setStyle(A,(bm-this.getOffset())+c);}
else {bl.setDomTop(bm);this.__gy.setStyle(r,(bm-this.getOffset())+c);}
;e.stop();}
;}
,_onPointerOut:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());}
,_onPointerUp:function(e){if(!this.__gs){return;}
;this._finalizeSizes();var bn=this.getChildControl(a);bn.exclude();this.__gs=false;this.releaseCapture();e.stop();}
,_finalizeSizes:function(){var br=this.__gw;var bo=this.__gx;if(br==null){return;}
...
else {if(this.__gv){bs.setWidth(br);bq.setWidth(bo);}
else {bs.setHeight(br);bq.setHeight(bo);}
;}
;}
,__gC:function(){if(this.__gv){var bx=v,bE=l,by=C,bC=this.__gt;}
else {var bx=s,bE=f,by=h,bC=this.__gu;}
;var bD=this._getChildren();var bv=bD[2].getSizeHint();var bA=bD[3].getSizeHint();var bB=bD[2].getBounds()[bE]+bD[3].getBounds()[bE];var bz=bC-this.__gr;var bw=bB-bz;if(bz<bv[bx]){bw-=bv[bx]-bz;bz=bv[bx];}
else if(bw<bA[bx]){bz-=bA[bx]-bw;bw=bA[bx];}
;if(bz>bv[by]){bw+=bz-bv[by];bz=bv[by];}
else if(bw>bA[by]){bz+=bw-bA[by];bw=bA[by];}
;this.__gw=bz;this.__gx=bw;}
,_isActiveDragSession:function(){return this.__gs;}
,_setLastPointerPosition:function(x,y){this.__gt=x;this.__gu=y;}
