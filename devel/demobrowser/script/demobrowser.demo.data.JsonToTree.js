...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.JsonToTree.f36881be5228.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,addSpacer:function(){if(!this.__JQ){this.__JQ=new qx.ui.core.Spacer();}
else {this._remove(this.__JQ);}
;this._add(this.__JQ);}
,addOpenButton:function(){this._add(this.getChildControl(l));}
,_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());}
;}
,addIcon:function(){var L=this.getChildControl(t);if(this.__JP){this._remove(L);}
;this._add(L);this.__JP=true;}
,addLabel:function(M){var N=this.getChildControl(D);if(this.__JO){this._remove(N);}
;if(M){this.setLabel(M);}
else {N.setValue(this.getLabel());}
;this._add(N);this.__JO=true;}
,_applyIcon:function(P,O){if(!this.__JS()){this.__JT(P);}
else if(!this.isOpen()){this.__JT(P);}
;}
,_applyIconOpened:function(R,Q){if(this.isOpen()){if(this.__JR()&&this.__JS()){this.__JT(R);}
else if(!this.__JR()&&this.__JS()){this.__JT(R);}
;}
;}
...
;var X;if(W){X=this.__JS()?this.getIconOpened():null;}
else {X=this.getIcon();}
;if(X){this.__JT(X);}
;W?this.addState(u):this.removeState(u);}
,__JR:function(){return qx.util.PropertyUtil.getUserValue(this,t);}
,__JS:function(){return qx.util.PropertyUtil.getUserValue(this,v);}
,__JT:function(Y){var ba=this.getChildControl(t,true);if(ba){ba.setSource(Y);}
;}
...
;}
;if(this.__JQ){this.__JQ.setWidth((this.getLevel()+1)*this.getIndent()-bf);}
;}
...
(function(){var a="open",b="abstract",c="__fv",d="qx.debug",e="'. It is not a child of this tree item.",f="addItem",g="visible",h="excluded",j="__fu",k="removeItem",m="Invalid child index: ",n="Cannot remove treeitem '",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:b,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__fu=[];}
,properties:{parent:{check:o,nullable:true}},members:{__fu:null,__fv:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
;}
;}
,getChildrenContainer:function(){if(!this.__fv){this.__fv=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?g:h});}
;return this.__fv;}
,hasChildrenContainer:function(){return this.__fv;}
...
;}
,__fw:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);}
;}
...
;this.__fu.push(Q);if(!N){this.__fw();}
;if(P){Q.recursiveAddToWidgetQueue();P.fireNonBubblingEvent(f,qx.event.type.Data,[Q]);}
;}
;if(P){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(X,T){if(qx.core.Environment.get(d)){this.assert(T<=this.__fu.length&&T>=0,m+T);}
;if(T==this.__fu.length){this.add(X);return;}
;var W=X.getParent();if(W){W.remove(X);}
;var U=this.getChildrenContainer();X.setParent(this);var V=this.hasChildren();var S=this.__fu[T];U.addBefore(X,S);if(X.hasChildren()){U.addAfter(X.getChildrenContainer(),X);}
;qx.lang.Array.insertAt(this.__fu,X,T);if(!V){this.__fw();}
;if(this.getTree()){X.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(Y,bb){if(qx.core.Environment.get(d)){this.assert(this.__fu.indexOf(bb)>=0);}
;var ba=Y.getParent();if(ba){ba.remove(Y);}
;this.addAt(Y,this.__fu.indexOf(bb));}
,addAfter:function(bd,bc){if(qx.core.Environment.get(d)){this.assert(this.__fu.indexOf(bc)>=0);}
;var be=bd.getParent();if(be){be.remove(bd);}
;this.addAt(bd,this.__fu.indexOf(bc)+1);}
,addAtBegin:function(bf){this.addAt(bf,0);}
,remove:function(bk){for(var i=0,l=arguments.length;i<l;i++ ){var bj=arguments[i];if(this.__fu.indexOf(bj)==-1){this.warn(n+bj+e);return;}
...
;}
;qx.lang.Array.remove(this.__fu,bj);bj.setParent(null);bg.remove(bj);}
;var bh=this.getTree();if(bh){bh.fireNonBubblingEvent(k,qx.event.type.Data,[bj]);}
;qx.ui.core.queue.Widget.add(this);}
,removeAt:function(bl){var bm=this.__fu[bl];if(bm){this.remove(bm);}
;}
,removeAll:function(){var bn=this.__fu.concat();for(var i=this.__fu.length-1;i>=0;i-- ){this.remove(this.__fu[i]);}
;return bn;}
...
(function(){var a="Enter",b="_applyOpenMode",c="excluded",d="Space",f="Left",g="Boolean",h="changeOpenMode",j="_applyRootOpenClose",k="visible",l="changeSelection",m="qx.ui.tree.Tree",n="qx.event.type.Data",o="qx.ui.tree.core.AbstractTreeItem",p="tree",q="dbltap",r="_applyHideRoot",s="changeRoot",t="__d",u="_applyRoot",v="keypress",w="none",x="pane",y="tap",z="Right";qx.Class.define(m,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__d=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(x).add(this.__d);this.initOpenMode();this.initRootOpenClose();this.addListener(l,this._onChangeSelection,this);this.addListener(v,this._onKeyPress,this);}
,events:{addItem:n,removeItem:n},properties:{openMode:{check:[y,q,w],init:q,apply:b,event:h,themeable:true},root:{check:o,init:null,nullable:true,event:s,apply:u},hideRoot:{check:g,init:false,apply:r},rootOpenClose:{check:g,init:false,apply:j},appearance:{refine:true,init:p},focusable:{refine:true,init:true}},members:{__d:null,SELECTION_MANAGER:qx.ui.tree.selection.SelectionManager,getChildrenContainer:function(){return this.__d;}
...
,_getContentPaddingTarget:function(){return this.__d;}
...
(function(){var a="changeModel",b="_applyDelegate",c="qx.core.Object",d="_applyLabelPath",f="icon",g="change",h="label",k="' twice.",l="_applyTarget",m="changeTarget",n="_applyIconPath",o="Can not reverse bind the same target property '",p="_applyLabelOptions",q="_applyChildPath",r="_applyIconOptions",s="qx.data.controller.Tree",t="String",u="Can not bind the same target property '",v="_applyModel",w="get";qx.Class.define(s,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(y,z,A,x){qx.core.Object.call(this);this.__jk={};this.__wr=[];this.__ws={};if(A!=null){this.setChildPath(A);}
;if(x!=null){this.setLabelPath(x);}
;if(y!=null){this.setModel(y);}
;if(z!=null){this.setTarget(z);}
;}
,properties:{model:{check:c,apply:v,event:a,nullable:true,dereference:true},target:{apply:l,event:m,init:null,nullable:true,dereference:true},childPath:{check:t,apply:q,nullable:true},labelPath:{check:t,apply:d,nullable:true},iconPath:{check:t,apply:n,nullable:true},labelOptions:{apply:p,nullable:true},iconOptions:{apply:r,nullable:true},delegate:{apply:b,init:null,nullable:true}},members:{__ws:null,__jk:null,__wr:null,__wt:null,_applyDelegate:function(C,B){this._setConfigureItem(C,B);this._setCreateItem(C,B);this._setBindItem(C,B);}
,_applyIconOptions:function(E,D){this.__wA();}
,_applyLabelOptions:function(G,F){this.__wA();}
,_applyTarget:function(I,H){if(H!=undefined){this.__lD(H);}
;if(this.getModel()!=null){this.__ww();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__ww();}
,_applyChildPath:function(M,L){this.__wt=L;this.__ww();this.__wt=null;}
,_applyIconPath:function(O,N){this.__wA();}
,_applyLabelPath:function(Q,P){this.__ww();}
,__wu:function(R){var T=R.getTarget();var S=this.__ws[T.toHashCode()].treeNode;var U=this.__ws[T.toHashCode()].modelNode;this.__wx(S,U);this._updateSelection();}
,__wv:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__ws[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__ws[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__wv,this);var W=V.treeNode;this.__ws[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__wx(W,bd);this._updateSelection();}
...
;return be;}
,__ww:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__lD();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__dD(this.getModel(),bg);this.__wx(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__wx:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__ws[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__wu,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__wv,this);this.__ws[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__dD(bj.getItem(i),bo);this.__wx(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__wz(bk,bn);}
;}
,__lD:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__wy(bv);var bu=bv.getModel();if(bu){this.__wB(bu);}
;bv.destroy();this.__ws={};}
;}
,__wy:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__wy(bx[i]);}
;this.__wz(bx[i],bw);}
;}
,__wz:function(bz,bC){var bD=bz.getModel();var by=this.__wt||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__ws[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__wy(bz);delete this.__ws[bE.toHashCode()];}
;this.__wB(bD);bC.remove(bz);bz.destroy();}
,bindProperty:function(bG,bJ,bI,bF,bL){var bK=bL.bind(bG,bF,bJ,bI);if(this.__jk[bJ]==null){this.__jk[bJ]={};}
;var bH=this.__jk[bJ];if(bH[bL.toHashCode()]){if(bH[bL.toHashCode()].id){throw new Error(u+bJ+k);}
;bH[bL.toHashCode()].id=bK;}
else {bH[bL.toHashCode()]={id:bK,reverseId:null,treeNode:bF};}
;if(!qx.lang.Array.contains(this.__wr,bJ)){this.__wr.push(bJ);}
;}
,bindPropertyReverse:function(bQ,bM,bO,bP,bS){var bR=bP.bind(bM,bS,bQ,bO);if(this.__jk[bM]==null){this.__jk[bM]={};}
;var bN=this.__jk[bM];if(bN[bS.toHashCode()]){if(bN[bS.toHashCode()].reverseId){throw new Error(o+bQ+k);}
;bN[bS.toHashCode()].reverseId=bR;}
else {bN[bS.toHashCode()]={id:null,reverseId:bR,treeNode:bP};}
;if(!qx.lang.Array.contains(this.__wr,bM)){this.__wr.push(bM);}
;}
...
;}
,__wA:function(){var bW;for(var ca in this.__jk){bW=ca;break;}
;for(var bX in this.__jk[bW]){var bV=this.__jk[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__wB(bY);this.__dD(bY,bV);}
;}
,__dD:function(cd,cb){var cc=this.getDelegate();if(cc!=null&&cc.bindItem!=null){cc.bindItem(this,cb,cd);}
else {this.bindDefaultProperties(cb,cd);}
;}
,__wB:function(cg){for(var i=0;i<this.__wr.length;i++ ){var ce=this.__wr[i];var cf=this.__jk[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
;if(cf.reverseId){cf.treeNode.removeBinding(cf.reverseId);cf.reverseId=null;}
;delete this.__jk[ce][cg.toHashCode()];}
;}
;}
...
;this._startSelectionModification();this.__lD();this.__ww();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__ww();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__jk=this.__ws=this.__wr=null;}
});}
)();
