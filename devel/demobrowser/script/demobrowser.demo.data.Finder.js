...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Finder.bb7abf4e03b0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="indexOf",b="addAfter",c="add",d="addBefore",e="_",f="addAt",g="hasChildren",h="removeAt",i="removeAll",j="getChildren",k="remove",l="qx.ui.core.MRemoteChildrenHandling";qx.Mixin.define(l,{members:{__p:function(q,m,o,n){var p=this.getChildrenContainer();if(p===this){q=e+q;}
;return (p[q])(m,o,n);}
,getChildren:function(){return this.__p(j);}
,hasChildren:function(){return this.__p(g);}
,add:function(s,r){return this.__p(c,s,r);}
,remove:function(t){return this.__p(k,t);}
,removeAll:function(){return this.__p(i);}
,indexOf:function(u){return this.__p(a,u);}
,addAt:function(x,v,w){this.__p(f,x,v,w);}
,addBefore:function(y,A,z){this.__p(d,y,A,z);}
,addAfter:function(D,B,C){this.__p(b,D,B,C);}
,removeAt:function(E){return this.__p(h,E);}
}});}
)();
...
(function(){var a="__cA",b="qx.ui.window.Manager";qx.Class.define(b,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__cA:null,setDesktop:function(c){this.__cA=c;this.updateStack();}
,getDesktop:function(){return this.__cA;}
,changeActiveWindow:function(d,e){if(d){this.bringToFront(d);d.setActive(true);}
;if(e){e.resetActive();}
;}
,_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(){this.__cA.forceUnblock();var h=this.__cA.getWindows();var g=this._minZIndex;var m=g+h.length*2;var j=g+h.length*4;var k=null;for(var i=0,l=h.length;i<l;i++ ){var f=h[i];if(!f.isVisible()){continue;}
;k=k||f;if(f.isModal()){f.setZIndex(j);this.__cA.blockContent(j-1);j+=2;k=f;}
else if(f.isAlwaysOnTop()){f.setZIndex(m);m+=2;}
else {f.setZIndex(g);g+=2;}
;if(!k.isModal()&&f.isActive()||f.getZIndex()>k.getZIndex()){k=f;}
;}
;this.__cA.setActiveWindow(k);}
,bringToFront:function(o){var n=this.__cA.getWindows();var p=qx.lang.Array.remove(n,o);if(p){n.push(o);this.updateStack();}
;}
,sendToBack:function(r){var q=this.__cA.getWindows();var s=qx.lang.Array.remove(q,r);if(s){q.unshift(r);this.updateStack();}
;}
},destruct:function(){this._disposeObjects(a);}
});}
)();
(function(){var a="pointerup",b="Boolean",c="losecapture",d="move",f="qx.ui.core.MMovable",g="The move handle could not be redefined!",h="pointerdown",i="maximized",j="__cB",k="pointermove",l="roll",m="__cC",n="move-frame";qx.Mixin.define(f,{properties:{movable:{check:b,init:true},useMoveFrame:{check:b,init:false}},members:{__cB:null,__cC:null,__cD:null,__cE:null,__cF:null,__cG:null,__cH:null,__cI:false,__cJ:null,__cK:0,_activateMoveHandle:function(o){if(this.__cB){throw new Error(g);}
;this.__cB=o;o.addListener(h,this._onMovePointerDown,this);o.addListener(a,this._onMovePointerUp,this);o.addListener(k,this._onMovePointerMove,this);o.addListener(c,this.__cO,this);}
,__cL:function(){var p=this.__cC;if(!p){p=this.__cC=new qx.ui.core.Widget();p.setAppearance(n);p.exclude();qx.core.Init.getApplication().getRoot().add(p);}
;return p;}
,__cM:function(){var location=this.getContentLocation();var r=this.getBounds();var q=this.__cL();q.setUserBounds(location.left,location.top,r.width,r.height);q.show();q.setZIndex(this.getZIndex()+1);}
,__cN:function(e){var t=this.__cD;var s=Math.max(t.left,Math.min(t.right,e.getDocumentLeft()));var v=Math.max(t.top,Math.min(t.bottom,e.getDocumentTop()));var u=this.__cE+s;var w=this.__cF+v;return {viewportLeft:parseInt(u,10),viewportTop:parseInt(w,10),parentLeft:parseInt(u-this.__cG,10),parentTop:parseInt(w-this.__cH,10)};}
,_onMoveRoll:function(e){e.stop();}
,_onMovePointerDown:function(e){if(!this.getMovable()||this.hasState(i)){return;}
;this.addListener(l,this._onMoveRoll,this);var parent=this.getLayoutParent();var x=parent.getContentLocation();var z=parent.getBounds();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isBlocked()){this.__cJ=parent.getBlockerColor();this.__cK=parent.getBlockerOpacity();parent.setBlockerColor(null);parent.setBlockerOpacity(1);parent.blockContent(this.getZIndex()-1);this.__cI=true;}
;}
;this.__cD={left:x.left,top:x.top,right:x.left+z.width,bottom:x.top+z.height};var y=this.getContentLocation();this.__cG=x.left;this.__cH=x.top;this.__cE=y.left-e.getDocumentLeft();this.__cF=y.top-e.getDocumentTop();this.addState(d);this.__cB.capture();if(this.getUseMoveFrame()){this.__cM();}
;e.stop();}
,_onMovePointerMove:function(e){if(!this.hasState(d)){return;}
;var B=this.__cN(e);if(this.getUseMoveFrame()){this.__cL().setDomPosition(B.viewportLeft,B.viewportTop);}
else {var A=this.getLayoutParent().getInsets();this.setDomPosition(B.parentLeft-(A.left||0),B.parentTop-(A.top||0));}
;e.stopPropagation();}
,_onMovePointerUp:function(e){if(this.hasListener(l)){this.removeListener(l,this._onMoveRoll,this);}
;if(!this.hasState(d)){return;}
;this.removeState(d);var parent=this.getLayoutParent();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__cI){parent.unblock();parent.setBlockerColor(this.__cJ);parent.setBlockerOpacity(this.__cK);this.__cJ=null;this.__cK=0;this.__cI=false;}
;}
;this.__cB.releaseCapture();var D=this.__cN(e);var C=this.getLayoutParent().getInsets();this.setLayoutProperties({left:D.parentLeft-(C.left||0),top:D.parentTop-(C.top||0)});if(this.getUseMoveFrame()){this.__cL().exclude();}
;e.stopPropagation();}
,__cO:function(e){if(!this.hasState(d)){return;}
;this.removeState(d);if(this.getUseMoveFrame()){this.__cL().exclude();}
;}
},destruct:function(){this._disposeObjects(m,j);this.__cD=null;}
});}
)();
...
;this.__cP=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:a,init:true},resizableRight:{check:a,init:true},resizableBottom:{check:a,init:true},resizableLeft:{check:a,init:true},resizable:{group:[z,g,w,l],mode:n},resizeSensitivity:{check:j,init:5},useResizeFrame:{check:a,init:true}},members:{__cP:null,__cQ:null,__cR:null,__cS:null,__cT:null,__cU:null,__cV:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__cQ;if(!C){C=this.__cQ=new qx.ui.core.Widget();C.setAppearance(v);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__cW:function(){var location=this.getContentLocation();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__cX:function(e){var F=this.__cR;var E=this.getSizeHint();var I=this.__cV;var H=this.__cU;var J=H.width;var G=H.height;var L=H.left;var top=H.top;var K;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(I.top,Math.min(I.bottom,e.getDocumentTop()))-this.__cT;if(F&this.RESIZE_TOP){G-=K;}
else {G+=K;}
;if(G<E.minHeight){G=E.minHeight;}
else if(G>E.maxHeight){G=E.maxHeight;}
;if(F&this.RESIZE_TOP){top+=H.height-G;}
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(I.left,Math.min(I.right,e.getDocumentLeft()))-this.__cS;if(F&this.RESIZE_LEFT){J-=K;}
else {J+=K;}
...
,__cY:{'1':f,'2':x,'4':b,'8':k,'5':q,'6':d,'9':h,'10':i},__da:function(e){var location=this.getContentLocation();var O=this.getResizeSensitivity();var N=e.getDocumentLeft();var P=e.getDocumentTop();var M=this.__db(location,N,P,O);if(M>0){M=M|this.__db(location,N,P,O*2);}
;this.__cR=M;}
,__db:function(location,T,R,S){var Q=0;if(this.getResizableTop()&&Math.abs(location.top-R)<S&&T>location.left-S&&T<location.right+S){Q+=this.RESIZE_TOP;}
...
;return Q;}
,__dc:function(e){if(!this.__cR||!this.getEnabled()||e.getPointerType()==c){return;}
;this.addState(p);this.__cS=e.getDocumentLeft();this.__cT=e.getDocumentTop();var location=this.getContentLocation();var V=this.getBounds();this.__cU={top:location.top,left:location.left,width:location.right-location.left,height:location.bottom-location.top,bounds:qx.lang.Object.clone(V)};var parent=this.getLayoutParent();var W=parent.getContentLocation();var U=parent.getBounds();this.__cV={left:W.left,top:W.top,right:W.left+U.width,bottom:W.top+U.height};if(this.getUseResizeFrame()){this.__cW();}
;this.capture();e.stop();}
,__dd:function(e){if(!this.hasState(p)||!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var X=this.__cX(e);this.setWidth(X.width);this.setHeight(X.height);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:X.parentLeft,top:X.parentTop});}
;this.__cR=0;this.removeState(p);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
,__de:function(e){if(!this.__cR){return;}
...
;}
,__df:function(e){if(!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.hasState(p)){var ba=this.__cX(e);if(this.getUseResizeFrame()){var Y=this._getResizeFrame();Y.setUserBounds(ba.viewportLeft,ba.viewportTop,ba.width,ba.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(o)&&!this.__cP.isSessionActive()){this.__da(e);var bd=this.__cR;var bc=this.getApplicationRoot();if(bd){var bb=this.__cY[bd];this.setCursor(bb);bc.setGlobalCursor(bb);}
else if(this.getCursor()){this.resetCursor();bc.resetGlobalCursor();}
;}
;}
,__dg:function(e){if(e.getPointerType()==c){return;}
;if(this.getCursor()&&!this.hasState(p)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__cQ!=null&&!qx.core.ObjectRegistry.inShutDown){this.__cQ.destroy();this.__cQ=null;}
;this.__cP=null;}
});}
)();
...
;var br=this.getLayoutProperties();this.__dA=br.left===undefined?0:br.left;this.__dz=br.top===undefined?0:br.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(F);this._updateCaptionBar();this.fireEvent(Q);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(b,qx.event.type.Event,[false,true])){var bs=this.getLayoutProperties();this.__dA=bs.left===undefined?0:bs.left;this.__dz=bs.top===undefined?0:bs.top;this.removeState(F);this.hide();this.fireEvent(v);}
;}
...
;var bt=this.__dA;var top=this.__dz;this.setLayoutProperties({edge:null,left:bt,top:top});this.removeState(F);this._updateCaptionBar();this.fireEvent(S);}
;}
...
;throw new Error(n);}
,getSelection:function(){return qx.lang.Object.getValues(this.__yx);}
,getSortedSelection:function(){var bU=this.getSelectables();var bT=qx.lang.Object.getValues(this.__yx);bT.sort(function(a,b){return bU.indexOf(a)-bU.indexOf(b);}
);return bT;}
,isItemSelected:function(bV){var bW=this._selectableToHashCode(bV);return this.__yx[bW]!==undefined;}
,isSelectionEmpty:function(){return qx.lang.Object.isEmpty(this.__yx);}
...
;this._fireChange();}
,_setLeadItem:function(cb){var ca=this.__GN;if(ca!==null){this._styleSelectable(ca,u,false);}
;if(cb!==null){this._styleSelectable(cb,u,true);}
;this.__GN=cb;}
,getLeadItem:function(){return this.__GN;}
,_setAnchorItem:function(cd){var cc=this.__GO;if(cc!=null){this._styleSelectable(cc,I,false);}
;if(cd!=null){this._styleSelectable(cd,I,true);}
;this.__GO=cd;}
,_getAnchorItem:function(){return this.__GO!==null?this.__GO:null;}
...
;this._fireChange();}
,handlePointerOver:function(event){if(this.__GQ!=null&&this.__GQ!=this._getScroll().top){this.__GQ=null;return;}
...
;var cD=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cA=event.isShiftPressed();if(this.isItemSelected(cB)&&!cA&&!cD&&!this.getDrag()){this.__GP=cB;this._userInteraction=false;return;}
else {this.__GP=null;}
;this._scrollItemIntoView(cB);var cC=this.getMode();if(this.getDrag()&&cC!==bt&&cC!==h&&!cA&&!cD&&event.getPointerType()==F){this._setAnchorItem(cB);this._setLeadItem(cB);this.__GD=this._getLocation();this.__GA=this._getScroll();this.__GE=event.getDocumentLeft()+this.__GA.left;this.__GF=event.getDocumentTop()+this.__GA.top;this.__GG=true;this._capture();}
;this._fireChange(bD);this._userInteraction=false;}
,handleTap:function(event){this._userInteraction=true;var cH=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cE=event.isShiftPressed();if(!cH&&!cE&&this.__GP!=null){this._userInteraction=false;var cF=this._getSelectableFromPointerEvent(event);if(cF===null||!this.isItemSelected(cF)){return;}
;}
...
,handlePointerMove:function(event){if(!this.__GG){return;}
;this.__GH=event.getDocumentLeft();this.__GI=event.getDocumentTop();this._userInteraction=true;var cI=this.__GH+this.__GA.left;if(cI>this.__GE){this.__GJ=1;}
else if(cI<this.__GE){this.__GJ=-1;}
else {this.__GJ=0;}
;var cJ=this.__GI+this.__GA.top;if(cJ>this.__GF){this.__GK=1;}
else if(cJ<this.__GF){this.__GK=-1;}
else {this.__GK=0;}
;var location=this.__GD;if(this.__GH<location.left){this.__Gx=this.__GH-location.left;}
else if(this.__GH>location.right){this.__Gx=this.__GH-location.right;}
else {this.__Gx=0;}
;if(this.__GI<location.top){this.__Gy=this.__GI-location.top;}
else if(this.__GI>location.bottom){this.__Gy=this.__GI-location.bottom;}
else {this.__Gy=0;}
;if(!this.__Gz){this.__Gz=new qx.event.Timer(100);this.__Gz.addListener(G,this._onInterval,this);}
;this.__Gz.start();this._autoSelect();event.stopPropagation();this._userInteraction=false;}
,handleAddItem:function(e){var cK=e.getData();if(this.getMode()===h&&this.isSelectionEmpty()){this.addItem(cK);}
;}
,handleRemoveItem:function(e){this.removeItem(e.getData());}
,_cleanup:function(){if(!this.getDrag()&&this.__GG){return;}
;if(this.__GL){this._fireChange(bD);}
;delete this.__GG;delete this.__GB;delete this.__GC;this._releaseCapture();if(this.__Gz){this.__Gz.stop();}
;}
,_onInterval:function(e){this._scrollBy(this.__Gx,this.__Gy);this.__GA=this._getScroll();this._autoSelect();}
,_autoSelect:function(){var cT=this._getDimension();var cM=Math.max(0,Math.min(this.__GH-this.__GD.left,cT.width))+this.__GA.left;var cL=Math.max(0,Math.min(this.__GI-this.__GD.top,cT.height))+this.__GA.top;if(this.__GB===cM&&this.__GC===cL){return;}
;this.__GB=cM;this.__GC=cL;var cQ=this._getAnchorItem();var cO=cQ;var cR=this.__GJ;var cU,cN;while(cR!==0){cU=cR>0?this._getRelatedSelectable(cO,t):this._getRelatedSelectable(cO,V);if(cU!==null){cN=this._getSelectableLocationX(cU);if((cR>0&&cN.left<=cM)||(cR<0&&cN.right>=cM)){cO=cU;continue;}
;}
;break;}
;var cS=this.__GK;var cV,cP;while(cS!==0){cV=cS>0?this._getRelatedSelectable(cO,z):this._getRelatedSelectable(cO,bB);if(cV!==null){cP=this._getSelectableLocationY(cV);if((cS>0&&cP.top<=cL)||(cS<0&&cP.bottom>=cL)){cO=cV;continue;}
;}
;break;}
...
;this._fireChange(bw);}
,__EI:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dd,dc;var df=event.getKeyIdentifier();var de=this.getMode();var cY=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var da=event.isShiftPressed();var db=false;if(df===A&&cY){if(de!==bt&&de!==h){this._selectAllItems();db=true;}
;}
...
;db=true;}
;}
else if(this.__EI[df]){db=true;if(de===bt||de==h){dd=this._getSelectedItem();}
...
;}
;break;};this.__GQ=this._getScroll().top;this._scrollItemIntoView(dc);}
;}
...
;}
,_clearSelection:function(){var di=this.__yx;for(var dj in di){this._removeFromSelection(di[dj]);}
;this.__yx={};}
,_selectItemRange:function(dk,dp,dm){var dq=this._getSelectableRange(dk,dp);if(!dm){var dn=this.__yx;var dr=this.__GR(dq);for(var dl in dn){if(!dr[dl]){this._removeFromSelection(dn[dl]);}
;}
;}
...
;}
,__GR:function(dw){var dx={};var dv;for(var i=0,l=dw.length;i<l;i++ ){dv=dw[i];dx[this._selectableToHashCode(dv)]=dv;}
;return dx;}
,_getSelectedItem:function(){for(var dy in this.__yx){return this.__yx[dy];}
;return null;}
,_setSelectedItem:function(dA){if(this._isSelectable(dA)){var dz=this.__yx;var dB=this._selectableToHashCode(dA);if(!dz[dB]||(dz.length>=2)){this._clearSelection();this._addToSelection(dA);}
;}
;}
,_addToSelection:function(dC){var dD=this._selectableToHashCode(dC);if(this.__yx[dD]==null&&this._isSelectable(dC)){this.__yx[dD]=dC;this._styleSelectable(dC,R,true);this.__GL=true;}
;}
,_toggleInSelection:function(dE){var dF=this._selectableToHashCode(dE);if(this.__yx[dF]==null){this.__yx[dF]=dE;this._styleSelectable(dE,R,true);}
else {delete this.__yx[dF];this._styleSelectable(dE,R,false);}
;this.__GL=true;}
,_removeFromSelection:function(dG){var dH=this._selectableToHashCode(dG);if(this.__yx[dH]!=null){delete this.__yx[dH];this._styleSelectable(dG,R,false);this.__GL=true;}
;}
...
;}
;var dP=dK[0];var dI=dO;var dM=this.__yx;for(var dN in dM){if(dJ[dN]){delete dJ[dN];}
...
;if(!dL){return false;}
;this._scrollItemIntoView(dI);this._setLeadItem(dP);this._setAnchorItem(dP);this.__GL=true;this._fireChange();}
,_fireChange:function(dQ){if(this.__GL){this.__GM=dQ||null;this.fireDataEvent(bA,this.getSelection());delete this.__GL;}
;}
...
;return dS;}
;return null;}
},destruct:function(){this._disposeObjects(bs);this.__yx=this.__GP=this.__GO=null;this.__GN=null;}
});}
)();
(function(){var a="vertical",b="under",c="above",d="qx.ui.core.selection.Widget",e="left",f="right";qx.Class.define(d,{extend:qx.ui.core.selection.Abstract,construct:function(g){qx.ui.core.selection.Abstract.call(this);this.__fm=g;}
...
;}
;return null;}
,_getRelatedSelectable:function(N,O){var L=this.__fm.getOrientation()===a;var P=this.__fm.getChildren();var K=P.indexOf(N);var M;if((L&&O===c)||(!L&&O===e)){for(var i=K-1;i>=0;i-- ){M=P[i];if(this._isItemSelectable(M)){return M;}
;}
;}
...
;this.__ff.removeItem(I);}
,selectRange:function(J,K){this.__ff.selectItemRange(J,K);}
,resetSelection:function(){this.__ff.clearSelection();}
...
else {var M=this.getSelection();if(!qx.lang.Array.equals(M,L)){this.__ff.replaceSelection(L);}
;}
;}
,getSelection:function(){return this.__ff.getSelection();}
,getSortedSelection:function(){return this.__ff.getSortedSelection();}
,isSelectionEmpty:function(){return this.__ff.isSelectionEmpty();}
,getSelectionContext:function(){return this.__ff.getSelectionContext();}
,_getManager:function(){return this.__ff;}
,getSelectables:function(N){return this.__ff.getSelectables(N);}
,invertSelection:function(){this.__ff.invertSelection();}
,_getLeadItem:function(){var O=this.__ff.getMode();if(O===r||O===b){return this.__ff.getSelectedItem();}
else {return this.__ff.getLeadItem();}
;}
,_applySelectionMode:function(Q,P){this.__ff.setMode(Q);}
,_applyDragSelection:function(S,R){this.__ff.setDrag(S);}
,_applyQuickSelection:function(U,T){this.__ff.setQuick(U);}
...
(function(){var a="qx.ui.form.List",b="text",c="Boolean",d="one",f="addChildWidget",g="_applySpacing",h="Enter",j="Integer",k="action",m="keyinput",n="qx.event.type.Data",o="addItem",p="vertical",q="removeChildWidget",r="",s="_applyOrientation",t="__d",u="single",v="keypress",w="list",x="horizontal",y="label",z="pane",A="removeItem";qx.Class.define(a,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__d=this._createListItemContainer();this.__d.addListener(f,this._onAddChild,this);this.__d.addListener(q,this._onRemoveChild,this);this.getChildControl(z).add(this.__d);if(B){this.setOrientation(x);}
else {this.initOrientation();}
;this.addListener(v,this._onKeyPress);this.addListener(m,this._onKeyInput);this.__Hv=r;}
,events:{addItem:n,removeItem:n},properties:{appearance:{refine:true,init:w},focusable:{refine:true,init:true},orientation:{check:[x,p],init:p,apply:s},spacing:{check:j,init:0,apply:g,themeable:true},enableInlineFind:{check:c,init:true}},members:{__Hv:null,__Hw:null,__d:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__d;}
...
,_applyOrientation:function(F,C){var D=F===x;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__d;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__d.getLayout().setSpacing(H);}
...
;if(((new Date).valueOf()-this.__Hw)>1000){this.__Hv=r;}
;this.__Hv+=e.getChar();var K=this.findItemByLabelFuzzy(this.__Hv);if(K){this.setSelection([K]);}
;this.__Hw=(new Date).valueOf();}
...
;if(this.getSelection()==null){this.__KL=new qx.data.Array();this.setSelection(this.__KL);}
;}
,properties:{selection:{check:b,event:f,apply:d,init:null}},events:{"changeSelection":h},members:{_modifingSelection:0,__KM:null,__KN:null,__KL:null,_applySelection:function(k,j){if(this.__KN!=undefined&&j!=undefined){j.removeListenerById(this.__KN);this.__KN=null;}
;if(k){this.__KN=k.addListener(a,this.__KO,this);}
;this._updateSelection();}
,__KO:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__KP()&&!this.__KQ()){return;}
;if(this._inSelectionModification()){return;}
;var m=this.getTarget().getSelection();var l=this.getSelection();if(l==null){l=new qx.data.Array();this.__KL=l;this.setSelection(l);}
...
,_addChangeTargetListener:function(p,o){if(this.__KM!=undefined&&o!=undefined){o.removeListenerById(this.__KM);}
;if(p!=null){if(this.__KP()||this.__KQ()){this.__KM=p.addListener(f,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()||!this.getSelection()){return;}
;this._startSelectionModification();if(this.__KP()){var s=[];for(var i=0;i<this.getSelection().length;i++ ){var t=this.getSelection().getItem(i);var u=this.__KR(t);if(u!=null){s.push(u);}
;}
...
;}
;}
else if(this.__KQ()){var r=this.getSelection().getItem(this.getSelection().length-1);if(r!==undefined){this.__FU(r);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__KP:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.IMultiSelection);}
,__KQ:function(){var w=this.getTarget().constructor;return qx.Class.implementsInterface(w,qx.ui.core.ISingleSelection);}
,__FU:function(x){var y=this.__KR(x);if(y==null){return;}
;if(this.__KP()){this.getTarget().addToSelection(y);}
else if(this.__KQ()){this.getTarget().setSelection([y]);}
;}
,__KR:function(z){var A=this.getTarget().getSelectables(true);for(var i=0;i<A.length;i++ ){if(A[i].getModel()==z){return A[i];}
;}
;return null;}
...
},destruct:function(){if(this.__KL){this.__KL.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="_applyLabelOptions",c="_applyLabelPath",d="changeTarget",e="_applyModel",f="change",g="icon",h="ReverseBindingId",k="qx.data.controller.List",m="_applyIconPath",n="BindingId",o="_applyDelegate",p="",q="changeDelegate",r="_applyTarget",s="]",t="qx.data.IListData",u="model[",v="model",w="label",x="String",y="_applyIconOptions",z=".";qx.Class.define(k,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__dh=[];this.__wr=[];this.__KS=[];this.__KT={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:t,apply:e,event:a,nullable:true,dereference:true},target:{apply:r,event:d,nullable:true,init:null,dereference:true},labelPath:{check:x,apply:c,nullable:true},iconPath:{check:x,apply:m,nullable:true},labelOptions:{apply:b,nullable:true},iconOptions:{apply:y,nullable:true},delegate:{apply:o,event:q,init:null,nullable:true}},members:{__lB:null,__dh:null,__KT:null,__wr:null,__KS:null,__KU:null,__KV:null,update:function(){this.__KY();this.__wA();this._updateSelection();}
,_applyDelegate:function(E,D){this._setConfigureItem(E,D);this._setFilter(E,D);this._setCreateItem(E,D);this._setBindItem(E,D);}
,_applyIconOptions:function(G,F){this.__wA();}
,_applyLabelOptions:function(I,H){this.__wA();}
,_applyIconPath:function(K,J){this.__wA();}
,_applyLabelPath:function(M,L){this.__wA();}
,_applyModel:function(P,N){if(N!=undefined){if(this.__lB!=undefined){N.removeListenerById(this.__lB);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(P!=null){this.__lB=P.addListener(f,this.__KX,this);this.__IU();this.__KY();if(N==null){this._changeTargetSelection();}
else {this.__KU=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var O=this.getTarget();if(O!=null){var length=O.getChildren().length;for(var i=0;i<length;i++ ){this.__Lc();}
;}
;}
;}
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__dh.length;i++ ){this.__Lb(this.__Le(i));}
;}
;}
;}
,__KW:false,__KX:function(){if(this.__KW)return;this.__KW=true;this.__KV=true;qx.ui.core.queue.Widget.add(this);this.update();this.__KW=false;}
,syncWidget:function(){if(this.__KU){this._changeTargetSelection();}
;if(this.__KV){this._updateSelection();}
;this.__KV=this.__KU=null;}
,__KY:function(){if(this.getTarget()==null){return;}
;this.__IU();var U=this.__dh.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__Lb(this.__Le(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__Lc();}
;}
;this.__IU();this.__KU=true;qx.ui.core.queue.Widget.add(this);}
,__La:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__lB);this.__lB=V.addListener(f,this.__KX,this);}
;}
...
;return W;}
,__Lb:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__Lc:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bo,br,bp,bn,bm){if(bp!=null){var bp=qx.lang.Object.clone(bp);this.__KT[br]=bp.onUpdate;delete bp.onUpdate;}
else {bp={};this.__KT[br]=null;}
;bp.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bm);bp.ignoreConverter=v;var bq=u+bm+s;if(bo!=null&&bo!=p){bq+=z+bo;}
;var bs=this.bind(bq,bn,br,bp);bn.setUserData(br+n,bs);if(!qx.lang.Array.contains(this.__wr,br)){this.__wr.push(br);}
;}
,bindPropertyReverse:function(by,bv,bw,bx,bt){var bu=u+bt+s;if(by!=null&&by!=p){bu+=z+by;}
;var bz=bx.bind(bv,this,bu,bw);bx.setUserData(by+h,bz);if(!qx.lang.Array.contains(this.__KS,by)){this.__KS.push(by);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__wr.length;i++ ){if(this.__KT[this.__wr[i]]!=null){this.__KT[this.__wr[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__wr.length;i++ ){var bE=bD.getUserData(this.__wr[i]+n);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__KS.length;i++ ){var bE=bD.getUserData(this.__KS[i]+h);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__wA:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++ ){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__Le(i));}
;this.__La();}
...
,_setFilter:function(bR,bP){if((bR==null||bR.filter==null)&&(bP!=null&&bP.filter!=null)){this.__Ld();}
...
;var bQ=this.__dh;this.__IU();if(bQ.length>this.__dh.length){for(var j=bQ.length;j>this.__dh.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bQ.length<this.__dh.length){for(var j=bQ.length;j<this.__dh.length;j++ ){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__Le(i));}
;this.__La();this._endSelectionModification();this._updateSelection();}
,__Ld:function(){this.__IU();this.__KY();this.__wA();this.__KV=true;qx.ui.core.queue.Widget.add(this);}
,__IU:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__dh=[];for(var i=0;i<bW.getLength();i++ ){if(bV==null||bV(bW.getItem(i))){this.__dh.push(i);}
;}
;}
,__Le:function(bY){return this.__dh[bY];}
},destruct:function(){this.__dh=this.__KT=this.__wr=null;this.__KS=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
...
;this.fireDataEvent(e,this.getModel());if(this.__jJ){this.__jJ.dispose();this.__jJ=null;}
;}
,reload:function(){var R=this.getUrl();if(R!=null){this._createRequest(R);}
;}
},destruct:function(){if(this.__jJ!=null){this._disposeObjects(r);}
...
},members:{__JW:null,__JX:function(z){return Object.keys(z).sort().join(a);}
,toClass:function(A,B){this.__JY(A,B,null,0);}
,__JY:function(N,I,C,K){if(!qx.lang.Type.isObject(N)||!!N.$$isString||N instanceof qx.core.Object){if(N instanceof Array||qx.Bootstrap.getClass(N)==h){for(var i=0;i<N.length;i++ ){this.__JY(N[i],I,C+o+i+m,K+1);}
;}
;return;}
;var E=this.__JX(N);if(this.__Kd(E,C,K)){return;}
;for(var M in N){this.__JY(N[M],I,M,K+1);}
;if(qx.Class.isDefined(s+E)){return;}
;if(this.__JW&&this.__JW.getModelClass&&this.__JW.getModelClass(E,N,C,K)!=null){return;}
;var O={};var F={__Ka:this.__Ka};for(var M in N){if(this.__JW&&this.__JW.getPropertyMapping){M=this.__JW.getPropertyMapping(M,E);}
...
;if(this.__JW&&this.__JW.getValidationRule){var H=this.__JW.getValidationRule(E,M);if(H){O[M].validate=k+M;F[k+M]=H;}
;}
;}
;if(this.__JW&&this.__JW.getModelSuperClass){var L=this.__JW.getModelSuperClass(E,C,K)||qx.core.Object;}
else {var L=qx.core.Object;}
;var G=[];if(this.__JW&&this.__JW.getModelMixins){var J=this.__JW.getModelMixins(E,C,K);if(!qx.lang.Type.isArray(J)){if(J!=null){G=[J];}
;}
else {G=J;}
;}
;if(I){G.push(qx.data.marshal.MEventBubbling);}
;var D={extend:L,include:G,properties:O,members:F,destruct:this.__Kb};qx.Class.define(s+E,D);}
,__Kb:function(){var P=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var Q in P){this.__Ka(this.get(P[Q].name));}
;}
,__Ka:function(R){if(!(R instanceof qx.core.Object)){return;}
;if(R.isDisposed()){return;}
;R.dispose();}
,__Kc:function(S,W,X,V){var Y;if(this.__JW&&this.__JW.getModelClass){Y=this.__JW.getModelClass(S,W,X,V);}
;if(Y!=null){return (new Y());}
else {var U=s+S;var T=qx.Class.getByName(U);if(!T){throw new Error(b+U+l);}
;return (new T());}
;}
,__Kd:function(ba,bd,bb){var bc=this.__JW;return bc&&bc.ignore&&bc.ignore(ba,bd,bb);}
,toModel:function(be){return this.__Ke(be,null,0);}
,__Ke:function(bq,bg,bm){var bk=qx.lang.Type.isObject(bq);var bf=bq instanceof Array||qx.Bootstrap.getClass(bq)==h;if((!bk&&!bf)||!!bq.$$isString||bq instanceof qx.core.Object){return bq;}
else if(this.__Kd(this.__JX(bq),bg,bm)){return bq;}
else if(bf){var bo=qx.data.Array;if(this.__JW&&this.__JW.getArrayClass){var bl=this.__JW.getArrayClass(bg,bm);bo=bl||bo;}
;var br=new bo();br.setAutoDisposeItems(true);for(var i=0;i<bq.length;i++ ){br.push(this.__Ke(bq[i],bg+o+i+m,bm+1));}
;return br;}
else if(bk){var bh=this.__JX(bq);var bp=this.__Kc(bh,bq,bg,bm);for(var bn in bq){var bi=bn;if(this.__JW&&this.__JW.getPropertyMapping){bi=this.__JW.getPropertyMapping(bn,bh);}
;var bs=bi.replace(/-|\.|\s+/g,d);if((qx.core.Environment.get(t))&&qx.core.Environment.get(e)){if(bs!=bi){this.warn(c+bn+n+bi+v);}
;}
;bi=bs;var bj=p+qx.lang.String.firstUp(bi);if(bp[bj]){bp[bj](this.__Ke(bq[bn],bn,bm+1));}
;}
;return bp;}
;throw new Error(j);}
...
;this.__me={};var S=this._transport=this._createTransport();this._setPhase(H);this.__mf=qx.lang.Function.bind(this._onReadyStateChange,this);this.__mg=qx.lang.Function.bind(this._onLoad,this);this.__mh=qx.lang.Function.bind(this._onLoadEnd,this);this.__mi=qx.lang.Function.bind(this._onAbort,this);this.__mj=qx.lang.Function.bind(this._onTimeout,this);this.__mk=qx.lang.Function.bind(this._onError,this);S.onreadystatechange=this.__mf;S.onload=this.__mg;S.onloadend=this.__mh;S.onabort=this.__mi;S.ontimeout=this.__mj;S.onerror=this.__mk;}
...
,nullable:true},authentication:{check:k,nullable:true}},members:{__mf:null,__mg:null,__mh:null,__mi:null,__mj:null,__mk:null,__ml:null,__mm:null,__mn:null,__me:null,__mo:null,_transport:null,_createTransport:function(){throw new Error(F);}
...
;this.__mm=true;this.__mn=t;this._transport.abort();}
...
;}
,_getAllRequestHeaders:function(){var bd={};qx.lang.Object.mergeWith(bd,this._getConfiguredRequestHeaders());qx.lang.Object.mergeWith(bd,this.__mp());qx.lang.Object.mergeWith(bd,this.__mo);qx.lang.Object.mergeWith(bd,this.__me);return bd;}
,__mp:function(){var bf=this.getAuthentication(),be={};if(bf){bf.getAuthHeaders().forEach(function(bg){be[bg.key]=bg.value;}
);return be;}
;}
,setRequestHeader:function(bh,bi){this.__me[bh]=bi;}
,getRequestHeader:function(bj){return this.__me[bj];}
,removeRequestHeader:function(bk){if(this.__me[bk]){delete this.__me[bk];}
;}
,getTransport:function(){return this._transport;}
,getReadyState:function(){return this._transport.readyState;}
,getPhase:function(){return this.__mn;}
...
,getResponse:function(){return this.__ml;}
,_setResponse:function(bo){var bn=bo;if(this.__ml!==bo){this.__ml=bo;this.fireEvent(I,qx.event.type.Data,[this.__ml,bn]);}
;}
,_onReadyStateChange:function(){var bp=this.getReadyState();if(qx.core.Environment.get(f)){this.debug(C+bp);}
;this.fireEvent(E);if(this.__mm){return;}
;if(bp===3){this._setPhase(q);}
;if(this.isDone()){this.__mq();}
;}
,__mq:function(){if(qx.core.Environment.get(f)){this.debug(B+this.getStatus());}
...
,_setPhase:function(br){var bs=this.__mn;if(qx.core.Environment.get(n)){qx.core.Assert.assertString(br);qx.core.Assert.assertMatch(br,/^(unsent)|(opened)|(sent)|(loading)|(load)|(success)|(abort)|(timeout)|(statusError)$/);}
;this.__mn=br;this.fireDataEvent(c,br,bs);}
...
;var A=z[h+qx.lang.String.firstUp(name)]();if(qx.lang.Type.isArray(A)){var D=qx.data&&qx.data.IListData&&qx.Class.hasInterface(A&&A.constructor,qx.data.IListData);for(var i=0;i<A.length;i++ ){var F=D?A.getItem(i):A[i];E+=this.__JJ(name,F,C);}
;}
else if(qx.lang.Type.isDate(A)&&y!=null){E+=this.__JJ(name,y.format(A),C);}
else {E+=this.__JJ(name,A,C);}
;}
;return E.substring(0,E.length-1);}
,__JJ:function(name,I,G){if(I&&I.$$type==o){I=I.classname;}
...
(function(){var a="Retry open native request with method: ",b="activex",c="No XHR support available.",d="If-None-Match",f="xhr",g="If-Modified-Since",h="engine.version",i="onunload",j="GET",k="-1",l="qx.debug.io",m="Undefined",n="Open native request with method: ",o="error",p="loadend",q="Blob",r="HTMLDocument",s="progress",t="load",u="Received native readyState: ",v="abort",w=", url: ",x="String",y="browser.documentmode",z="",A="readystatechange",B="Microsoft.XMLHTTP",C="Already disposed",D="browser.version",E="opera",F="qx.bom.request.Xhr",G="on",H="timeout",I="gecko",J="If-Match",K="mshtml",L="engine.name",M="Microsoft.XMLDOM",N="file:",O="Send native request",P="FormData",Q="If-Range",R="Content-Type",S="io.xhr",T="Not enough arguments",U="ArrayBuffer",V="undefined",W=", async: ",X="Native XHR object doesn't support overrideMimeType.";qx.Bootstrap.define(F,{extend:Object,construct:function(){var Y=qx.Bootstrap.bind(this.__Gn,this);if(qx.event&&qx.event.GlobalError&&qx.event.GlobalError.observeMethod){this.__Ga=qx.event.GlobalError.observeMethod(Y);}
else {this.__Ga=Y;}
;this.__Gb=qx.Bootstrap.bind(this.__Gl,this);this.__Gc=qx.Bootstrap.bind(this.__Gm,this);this.__mj=qx.Bootstrap.bind(this.__Gr,this);this.__Gk();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__Gd=qx.Bootstrap.bind(this.__Gu,this);window.attachEvent(i,this.__Gd);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":F,"error":F,"loadend":F,"timeout":F,"abort":F,"load":F,"progress":F},members:{readyState:0,responseText:z,responseXML:null,status:0,statusText:z,timeout:0,progress:null,open:function(be,ba,bb,bd,bc){this.__Gw();if(typeof ba===V){throw new Error(T);}
else if(typeof be===V){be=j;}
;this.__mm=false;this.__Ge=false;this.__Gf=false;this.__uh=ba;if(typeof bb==V){bb=true;}
;this.__Gg=bb;if(!this.__Gv()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__Gk();}
;this.__Gi.onreadystatechange=this.__Ga;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,n+be+w+ba+W+bb);}
;this.__Gi.open(be,ba,bb,bd,bc);}
catch(bf){if(!qx.util.Request.isCrossDomain(ba)){throw bf;}
;if(!this.__Gg){this.__Gh=bf;}
;if(this.__Gg){if(window.XDomainRequest){this.readyState=4;this.__Gi=new XDomainRequest();this.__Gi.onerror=qx.Bootstrap.bind(function(){this._emit(A);this._emit(o);this._emit(p);}
,this);if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,a+be+w+ba+W+bb);}
;this.__Gi.open(be,ba,bb,bd,bc);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__mx){return;}
;this.readyState=4;this._emit(A);this._emit(o);this._emit(p);}
,this));}
;}
;if(qx.core.Environment.get(L)===K&&qx.core.Environment.get(y)<9&&this.__Gi.readyState>0){this.__Gi.setRequestHeader(g,k);}
;if(qx.core.Environment.get(L)===I&&parseInt(qx.core.Environment.get(h),10)<2&&!this.__Gg){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(A);}
;}
,setRequestHeader:function(bg,bh){this.__Gw();if(bg==J||bg==g||bg==d||bg==Q){this.__Gf=true;}
;this.__Gi.setRequestHeader(bg,bh);return this;}
,send:function(bj){this.__Gw();if(!this.__Gg&&this.__Gh){throw this.__Gh;}
;if(qx.core.Environment.get(L)===E&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__mQ=window.setTimeout(this.__mj,this.timeout);}
;bj=typeof bj==V?null:bj;var bi=qx.Bootstrap.getClass(bj);bj=(bj!==null&&this.__Gj.indexOf(bi)===-1)?bj.toString():bj;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,O);}
;this.__Gi.send(bj);}
catch(bl){if(!this.__Gg){throw bl;}
;if(this._getProtocol()===N){this.readyState=2;this.__Go();var bk=this;window.setTimeout(function(){if(bk.__mx){return;}
;bk.readyState=3;bk.__Go();bk.readyState=4;bk.__Go();}
);}
;}
;if(qx.core.Environment.get(L)===I&&!this.__Gg){this.__Gn();}
;this.__Ge=true;return this;}
,abort:function(){this.__Gw();this.__mm=true;this.__Gi.abort();if(this.__Gi&&this.readyState!==qx.bom.request.Xhr.DONE){this.readyState=this.__Gi.readyState;}
;return this;}
...
,getResponseHeader:function(bo){this.__Gw();if(qx.core.Environment.get(y)===9&&this.__Gi.aborted){return z;}
;return this.__Gi.getResponseHeader(bo);}
,getAllResponseHeaders:function(){this.__Gw();if(qx.core.Environment.get(y)===9&&this.__Gi.aborted){return z;}
;return this.__Gi.getAllResponseHeaders();}
,overrideMimeType:function(bp){this.__Gw();if(this.__Gi.overrideMimeType){this.__Gi.overrideMimeType(bp);}
else {throw new Error(X);}
;return this;}
,getRequest:function(){return this.__Gi;}
,dispose:function(){if(this.__mx){return false;}
;window.clearTimeout(this.__mQ);if(window.detachEvent){window.detachEvent(i,this.__Gd);}
;try{this.__Gi.onreadystatechange;}
catch(br){return false;}
;var bq=function(){}
;this.__Gi.onreadystatechange=bq;this.__Gi.onload=bq;this.__Gi.onerror=bq;this.__Gi.onprogress=bq;this.abort();this.__Gi=null;this.__mx=true;return true;}
,isDisposed:function(){return !!this.__mx;}
...
,_getProtocol:function(){var bt=this.__uh;var bu=/^(\w+:)\/\//;if(bt!==null&&bt.match){var bv=bt.match(bu);if(bv&&bv[1]){return bv[1];}
;}
;return window.location.protocol;}
,__Gi:null,__Gg:null,__Ga:null,__Gb:null,__Gc:null,__Gd:null,__mj:null,__Ge:null,__uh:null,__mm:null,__nv:null,__mx:null,__mQ:null,__Gh:null,__Gf:null,__Gj:null,__Gk:function(){this.__Gi=this._createNativeXhr();this.__Gi.onreadystatechange=this.__Ga;if(qx.Bootstrap.getClass(this.__Gi.onabort)!==m){this.__Gi.onabort=this.__Gb;}
;if(qx.Bootstrap.getClass(this.__Gi.onprogress)!==m){this.__Gi.onprogress=this.__Gc;this.progress={lengthComputable:false,loaded:0,total:0};}
;this.__mx=this.__Ge=this.__mm=false;this.__Gj=[U,q,r,x,P];}
,__Gl:function(){if(!this.__mm){this.abort();}
;}
,__Gm:function(e){this.progress.lengthComputable=e.lengthComputable;this.progress.loaded=e.loaded;this.progress.total=e.total;this._emit(s);}
,__Gn:function(){var bw=this.__Gi,bx=true;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,u+bw.readyState);}
;if(this.readyState==bw.readyState){return;}
;this.readyState=bw.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__mm&&!this.__Ge){return;}
;if(!this.__Gg&&(bw.readyState==2||bw.readyState==3)){return;}
...
;if(bx){this.__Gs();this.__Gt();}
;}
;this.__Go();if(this.readyState==qx.bom.request.Xhr.DONE){if(bw){bw.onreadystatechange=function(){}
;}
;}
;}
,__Go:function(){if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__mQ);}
;this._emit(A);if(this.readyState===qx.bom.request.Xhr.DONE){this.__Gp();}
;}
,__Gp:function(){if(this.__nv){this._emit(H);if(qx.core.Environment.get(L)===E){this._emit(o);}
;this.__nv=false;}
else {if(this.__mm){this._emit(v);}
else {if(this.__Gq()){this._emit(o);}
else {this._emit(t);}
;}
;}
;this._emit(p);}
,__Gq:function(){var bz;if(this._getProtocol()===N){bz=!this.responseText;}
else {bz=!this.statusText&&this.status!==204;}
;return bz;}
,__Gr:function(){var bA=this.__Gi;this.readyState=qx.bom.request.Xhr.DONE;this.__nv=true;bA.aborted=true;bA.abort();this.responseText=z;this.responseXML=null;this.__Go();}
,__Gs:function(){var bB=this.readyState===qx.bom.request.Xhr.DONE;if(this._getProtocol()===N&&this.status===0&&bB){if(!this.__Gq()){this.status=200;}
;}
;if(this.status===1223){this.status=204;}
;if(qx.core.Environment.get(L)===E){if(bB&&this.__Gf&&!this.__mm&&this.status===0){this.status=304;}
;}
;}
,__Gt:function(){if(qx.core.Environment.get(L)==K&&(this.getResponseHeader(R)||z).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bC=new window.ActiveXObject(M);bC.async=false;bC.validateOnParse=false;bC.loadXML(this.responseText);this.responseXML=bC;}
;}
,__Gu:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__Gv:function(){var name=qx.core.Environment.get(L);var bD=qx.core.Environment.get(D);return !(name==K&&bD<9||name==I&&bD<3.5);}
,__Gw:function(){if(this.__mx){throw new Error(C);}
;}
...
;}
,statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},members:{__mr:null,parse:function(h,g){var f=this._getParser(g);if(typeof f===a){if(h!==c){return f.call(this,h);}
;}
;return h;}
,setParser:function(i){if(typeof qx.util.ResponseParser.PARSER[i]===a){return this.__mr=qx.util.ResponseParser.PARSER[i];}
;if(qx.core.Environment.get(d)){qx.core.Assert.assertFunction(i);}
;return this.__mr=i;}
,_getParser:function(k){var j=this.__mr,m=c,l=c;if(j){return j;}
