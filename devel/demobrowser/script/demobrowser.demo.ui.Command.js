...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.Command.963657f45671.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="-",c="PageUp",d="Escape",f="Boolean",g="+",h="qx.event.type.Data",j="_applyShortcut",k="PrintScreen",l="NumLock",m="short",n="keydown",o="5",p="8",q="execute",r="Meta",s="0",t="",u="2",v="Control",w="Shift",x="You can only specify one non modifier key!",y="3",z="Unidentified",A="/",B="Delete",C="String",D="changeEnabled",E="*",F="qx.bom.Shortcut",G="6",H="4",I="1",J="Alt",K="Not a valid key name for a shortcut: ",L="PageDown",M="Whitespaces are not allowed within shortcuts",N="_applyEnabled",O="keypress",P="7",Q="a",R="z",S="9";qx.Class.define(F,{extend:qx.core.Object,construct:function(T){qx.core.Object.call(this);this.__HC={};this.__HD=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":h},properties:{enabled:{init:true,check:f,event:D,apply:N},shortcut:{check:C,apply:j,nullable:true},autoRepeat:{check:f,init:false}},members:{__HC:t,__HD:t,execute:function(U){this.fireDataEvent(q,U);}
,__HE:function(event){if(this.getEnabled()&&this.__HF(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__ne:function(event){if(this.getEnabled()&&this.__HF(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__HE,this);qx.event.Registration.addListener(document.documentElement,O,this.__ne,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__HE,this);qx.event.Registration.removeListener(document.documentElement,O,this.__ne,this);}
;}
,_applyShortcut:function(bb,Y){if(bb){if(bb.search(/[\s]+/)!=-1){var bc=M;this.error(bc);throw new Error(bc);}
;this.__HC={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__HD=null;var X;var a=[];while(bb.length>0&&X!=-1){X=bb.search(/[-+]+/);a.push((bb.length==1||X==-1)?bb:bb.substring(0,X));bb=bb.substring(X+1);}
;var ba=a.length;for(var i=0;i<ba;i++ ){var bd=this.__HH(a[i]);switch(bd){case v:case w:case r:case J:this.__HC[bd]=true;break;case z:var bc=K+a[i];this.error(bc);throw bc;default:if(this.__HD){var bc=x;this.error(bc);throw bc;}
;this.__HD=bd;};}
;}
;return true;}
,__HF:function(e){var be=this.__HD;if(!be){return false;}
;if((!this.__HC.Shift&&e.isShiftPressed())||(this.__HC.Shift&&!e.isShiftPressed())||(!this.__HC.Control&&e.isCtrlPressed())||(this.__HC.Control&&!e.isCtrlPressed())||(!this.__HC.Meta&&e.isMetaPressed())||(this.__HC.Meta&&!e.isMetaPressed())||(!this.__HC.Alt&&e.isAltPressed())||(this.__HC.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__HG:{esc:d,ctrl:v,print:k,del:B,pageup:c,pagedown:L,numlock:l,numpad_0:s,numpad_1:I,numpad_2:u,numpad_3:y,numpad_4:H,numpad_5:o,numpad_6:G,numpad_7:P,numpad_8:p,numpad_9:S,numpad_divide:A,numpad_multiply:E,numpad_minus:b,numpad_plus:g},__HH:function(bg){var bf=qx.event.util.Keyboard;var bh=z;if(bf.isValidKeyIdentifier(bg)){return bg;}
;if(bg.length==1&&bg>=Q&&bg<=R){return bg.toUpperCase();}
;bg=bg.toLowerCase();var bh=this.__HG[bg]||qx.lang.String.firstUp(bg);if(bf.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bj=this.__HD;var bk=[];for(var bi in this.__HC){if(this.__HC[bi]){bk.push(qx.locale.Key.getKeyName(m,bi));}
;}
;if(bj){bk.push(qx.locale.Key.getKeyName(m,bj));}
;return bk.join(g);}
},destruct:function(){this.setEnabled(false);this.__HC=this.__HD=null;}
});}
)();
...
;var P=K.getMarginLeft()+K.getMarginRight();P=Math.max(P,this.getSpacing());var J=K.getSizeHint().width+P;this.__HT(K);G-=J;if(E&&E.getVisibility()!=o){E.setVisibility(o);G+=L;var H=E.getMarginLeft()+E.getMarginRight();G+=Math.max(H,this.getSpacing());}
;}
while(G>F);}
else if(this.__HQ.length>0){do {var Q=this.__HQ[0];if(Q){var P=Q.getMarginLeft()+Q.getMarginRight();P=Math.max(P,this.getSpacing());if(Q.getContentElement().getDomElement()==null){Q.syncAppearance();Q.invalidateLayoutCache();}
;var I=Q.getSizeHint().width;var O=false;if(this.__HQ.length==1&&L>0){var N=P-this.getSpacing();var M=G-L+I+N;O=F>M;}
;if(F>G+I+P||O){this.__HS(Q);G+=I;if(E&&this.__HQ.length==0){E.setVisibility(y);}
;}
else {return;}
;}
;}
while(F>=G&&this.__HQ.length>0);}
;}
,__HS:function(R){R.setVisibility(o);this.__HQ.shift();this.fireDataEvent(m,R);}
,__HT:function(S){if(!S){return;}
;this.__HQ.unshift(S);S.setVisibility(y);this.fireDataEvent(A,S);}
,_getNextToHide:function(){for(var i=this.__HR.length-1;i>=0;i-- ){var T=this.__HR[i];if(T&&T.getVisibility&&T.getVisibility()==o){return T;}
;}
...
;}
;}
,setRemovePriority:function(X,W,Y){if(!Y&&this.__HR[W]!=undefined){throw new Error(v);}
;this.__HR[W]=X;}
...
;for(var i=0;i<this.__HQ.length;i++ ){this.__HQ[i].setVisibility(o);}
;this.__HQ=[];}
;}
...
;bf.setVisibility(y);}
;}
,__HU:false,_setAllowMenuOpenHover:function(bg){this.__HU=bg;}
...
(function(){var a="blur",b="Enter",c="Escape",d="Space",f="keydown",g="Left",h="Down",j="Up",k="singleton",l="qx.debug",m="interval",n="roll",o="event.touch",p="__EC",q="Object is no menu: ",r="__ED",s="pointerdown",t="qx.ui.menu.Manager",u="__BN",v="keyup",w="keypress",x="contextmenu",y="Right";qx.Class.define(t,{type:k,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__BN=[];var z=document.body;var A=qx.event.Registration;A.addListener(window.document.documentElement,s,this._onPointerDown,this,true);A.addListener(z,n,this._onRoll,this,true);A.addListener(z,f,this._onKeyUpDown,this,true);A.addListener(z,v,this._onKeyUpDown,this,true);A.addListener(z,w,this._onKeyPress,this,true);if(!qx.core.Environment.get(o)){qx.bom.Element.addListener(window,a,this.hideAll,this);}
;this.__EC=new qx.event.Timer();this.__EC.addListener(m,this._onOpenInterval,this);this.__ED=new qx.event.Timer();this.__ED.addListener(m,this._onCloseInterval,this);}
,members:{__EE:null,__EF:null,__EC:null,__ED:null,__BN:null,_getChild:function(B,D,C,G){var F=B.getChildren();var length=F.length;var E;for(var i=D;i<length&&i>=0;i+=C){E=F[i];if(E.isEnabled()&&!E.isAnonymous()&&E.isVisible()){return E;}
;}
...
;H=H.getLayoutParent();}
;return false;}
,_isMenuOpener:function(J){var I=this.__BN;for(var i=0;i<I.length;i++ ){if(I[i].getOpener()===J){return true;}
;}
;return false;}
...
;}
;var M=this.__BN;M.push(L);L.setZIndex(1e6+M.length);}
,remove:function(N){if(qx.core.Environment.get(l)){if(!(N instanceof qx.ui.menu.Menu)){throw new Error(q+N);}
;}
;var O=this.__BN;if(O){qx.lang.Array.remove(O,N);}
;}
,hideAll:function(){var P=this.__BN;if(P){for(var i=P.length-1;i>=0;i-- ){P[i].exclude();}
;}
;}
,getActiveMenu:function(){var Q=this.__BN;return Q.length>0?Q[Q.length-1]:null;}
,scheduleOpen:function(R){this.cancelClose(R);if(R.isVisible()){if(this.__EE){this.cancelOpen(this.__EE);}
;}
else if(this.__EE!=R){this.__EE=R;this.__EC.restartWith(R.getOpenInterval());}
;}
,scheduleClose:function(S){this.cancelOpen(S);if(!S.isVisible()){if(this.__EF){this.cancelClose(this.__EF);}
;}
else if(this.__EF!=S){this.__EF=S;this.__ED.restartWith(S.getCloseInterval());}
;}
,cancelOpen:function(T){if(this.__EE==T){this.__EC.stop();this.__EE=null;}
;}
,cancelClose:function(U){if(this.__EF==U){this.__ED.stop();this.__EF=null;}
;}
,_onOpenInterval:function(e){this.__EC.stop();this.__EE.open();this.__EE=null;}
,_onCloseInterval:function(e){this.__ED.stop();this.__EF.exclude();this.__EF=null;}
,preventContextMenuOnce:function(){qx.event.Registration.addListener(document.body,x,this.__EG,this,true);}
,__EG:function(e){var V=e.getTarget();V=qx.ui.core.Widget.getWidgetByElement(V,true);if(this._isInMenu(V)){e.stopPropagation();e.preventDefault();}
;qx.event.Registration.removeListener(document.body,x,this.__EG,this,true);}
...
;if(this.__BN.length>0&&!this._isInMenu(W)){this.hideAll();}
;}
,__EH:{"Enter":1,"Space":1},__EI:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var X=this.getActiveMenu();if(!X){return;}
;var Y=e.getKeyIdentifier();if(this.__EI[Y]||(this.__EH[Y]&&X.getSelectedButton())){e.stopPropagation();}
;}
,_onKeyPress:function(e){var bc=this.getActiveMenu();if(!bc){return;}
;var ba=e.getKeyIdentifier();var be=this.__EI[ba];var bb=this.__EH[ba];if(be){switch(ba){case j:this._onKeyPressUp(bc);break;case h:this._onKeyPressDown(bc);break;case g:this._onKeyPressLeft(bc);break;case y:this._onKeyPressRight(bc);break;case c:this.hideAll();break;};e.stopPropagation();e.preventDefault();}
...
;}
,_onRoll:function(e){var bK=e.getTarget();bK=qx.ui.core.Widget.getWidgetByElement(bK,true);if(this.__BN.length>0&&!this._isInMenu(bK)&&!this._isMenuOpener(bK)&&!e.getMomentum()){this.hideAll();}
;}
},destruct:function(){var bM=qx.event.Registration;var bL=document.body;bM.removeListener(window.document.documentElement,s,this._onPointerDown,this,true);bM.removeListener(bL,f,this._onKeyUpDown,this,true);bM.removeListener(bL,v,this._onKeyUpDown,this,true);bM.removeListener(bL,w,this._onKeyPress,this,true);this._disposeObjects(p,r);this._disposeArray(u);}
});}
)();
...
;this.__HX();}
,__HX:function(){if(this.isVisible()){if(this.getBlockBackground()){var V=this.getZIndex();this._blocker.blockContent(V-1);}
;}
...
,__HY:function(){var bn=this._getMenuBounds();if(!bn){this.addListenerOnce(n,this.__HY,this);return;}
...
;this.__HW=bp;qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(bq){this.getChildControl(h);if(this.__HW){this.__HW.call(this);delete this.__HW;}
;}
...
;this.__HY();}
;}
,_onPointerOver:function(e){var bt=qx.ui.menu.Manager.getInstance();bt.cancelClose(this);var bu=e.getTarget();if(bu.isEnabled()&&bu instanceof qx.ui.menu.AbstractButton){this.setSelectedButton(bu);var bs=bu.getMenu&&bu.getMenu();if(bs){bs.setOpener(bu);bt.scheduleOpen(bs);this.__HV=bs;}
else {var bv=this.getOpenedButton();if(bv){bt.scheduleClose(bv.getMenu());}
;if(this.__HV){bt.cancelOpen(this.__HV);this.__HV=null;}
;}
;}
...
;if(this.__HV){bw.cancelOpen(this.__HV);}
;}
;}
...
(function(){var a="qx.ui.menu.Layout",b="Integer",c="_applyLayoutChange";qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:b,init:0,apply:c},spanColumn:{check:b,init:1,nullable:true,apply:c},iconColumnWidth:{check:b,init:0,themeable:true,apply:c},arrowColumnWidth:{check:b,init:0,themeable:true,apply:c}},members:{__fY:null,_computeSizeHint:function(){var q=this._getLayoutChildren();var o,g,k;var e=this.getSpanColumn();var j=this.__fY=[0,0,0,0];var n=this.getColumnSpacing();var m=0;var f=0;for(var i=0,l=q.length;i<l;i++ ){o=q[i];if(o.isAnonymous()){continue;}
...
,getColumnSizes:function(){return this.__fY||null;}
...
;var x=this.__EJ(y[0]);var w=x.getColumnSizes();var s=x.getSpacingX();var z=qx.lang.Array.sum(w)+s*(w.length-1);if(z<o){w[1]+=o-z;}
...
;}
;}
,__EJ:function(B){while(!(B instanceof qx.ui.menu.Menu)){B=B.getLayoutParent();}
;return B;}
...
(function(){var a="hovered",b="hover-button",c="pointerover",d="interval",f="__rq",g="qx.ui.form.HoverButton",h="Integer",i="pointerout";qx.Class.define(g,{extend:qx.ui.basic.Atom,include:[qx.ui.core.MExecutable],implement:[qx.ui.form.IExecutable],construct:function(j,k){qx.ui.basic.Atom.call(this,j,k);this.addListener(c,this._onPointerOver,this);this.addListener(i,this._onPointerOut,this);this.__rq=new qx.event.AcceleratingTimer();this.__rq.addListener(d,this._onInterval,this);}
,properties:{appearance:{refine:true,init:b},interval:{check:h,init:80},firstInterval:{check:h,init:200},minTimer:{check:h,init:20},timerDecrease:{check:h,init:2}},members:{__rq:null,_onPointerOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;this.__rq.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.addState(a);}
,_onPointerOut:function(e){this.__rq.stop();this.removeState(a);if(!this.isEnabled()||e.getTarget()!==this){return;}
;}
...
,__Ia:function(){var y=this.getChildrenContainer().getChildren();y=y.filter(function(z){return z.getVisibility()==k;}
...
,properties:{wrap:{check:b,init:true,apply:f},appearance:{refine:true,init:j},singleStep:{check:i,init:20},minimalLineHeight:{check:i,apply:h,init:4},autoSize:{check:b,apply:w,init:false}},members:{__dU:null,__dV:null,__dW:null,setValue:function(M){M=qx.ui.form.AbstractField.prototype.setValue.call(this,M);this.__dX();return M;}
...
;}
,__dX:function(){if(this.isAutoSize()){var R=this.__dY();if(R&&this.getBounds()){this.__dW=this.__dW||this._getAreaHeight();var T=this._getScrolledAreaHeight();if(this.getMaxHeight()){var P=this.getInsets();var Q=-P.top+this.getMaxHeight()-P.bottom;if(T>Q){this.getContentElement().setStyle(I,k);}
else {this.getContentElement().setStyle(I,H);}
;}
;var S=Math.max(T,this.__dW);this._setAreaHeight(S);}
else {this.getContentElement().addListenerOnce(x,function(){this.__dX();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(U){if(this._getAreaHeight()!==U){this.__dV=U;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__ec();}
;}
,_getScrolledAreaHeight:function(){var W=this.__dY();var V=W.getDomElement();if(V){if(!V.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;this.__eb(W);if(qx.core.Environment.get(C)==B&&qx.core.Environment.get(s)==8){if(!V.scrollTop){qx.html.Element.flush();}
;}
;return V.scrollTop;}
;}
,__dY:function(){this.__dU=this.__dU||this.__ea();return this.__dU;}
,__ea:function(){var ba,Y,bb,X;ba=this.getContentElement();if(!ba.getDomElement()){return null;}
;bb=qx.bom.Element.clone(ba.getDomElement());X=new qx.html.Input(j);X.useElement(bb);Y=X;Y.setStyles({position:K,top:0,left:o,height:0,overflow:H},true);Y.removeAttribute(G);Y.removeAttribute(z);Y.setAttribute(y,g);Y.setValue(ba.getValue()||t);Y.insertBefore(ba);this.__eb(Y);return Y;}
,__eb:function(bc){bc=bc.getDomElement();if(bc){bc.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bh,bg){if(qx.core.Environment.get(m)){this.__ed();}
;if(bh){this.__dX();this.addListener(a,this.__dX,this);if(!this.getBounds()){this.addListenerOnce(x,function(){this.getContentElement().setStyle(I,H);}
);}
else {this.getContentElement().setStyle(I,H);}
;}
else {this.removeListener(a,this.__dX);this.getContentElement().setStyle(I,k);}
;}
,_applyDimension:function(bi){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(m)){this.__ed();}
;if(bi===this.getMaxHeight()){this.__dX();}
;}
,__ec:function(){var content=this.getContentElement();var bk=content.getDomElement();var bj=content.getStyle(u);content.setStyle(u,parseInt(bj,10)+1000+c,true);if(bk){qx.bom.element.Dimension.getWidth(bk);}
;content.setStyle(u,bj,true);}
,__ed:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(n+J+v);}
;}
,_getContentHint:function(){var bl=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bl.height=bl.height*this.getMinimalLineHeight();bl.width=this._getTextSize().width*20;if(this.isAutoSize()){bl.height=this.__dV||bl.height;}
;return bl;}
},destruct:function(){this.setAutoSize(false);if(this.__dU){this.__dU.dispose();}
;}
});}
)();
