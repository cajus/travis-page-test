...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.33b69d5e8f92.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
);var N=new qx.ui.basic.Label(k);I.add(N,{left:600,top:20});var G=this.__fA=new qx.ui.form.List();G.setDraggable(true);G.setDroppable(true);G.setSelectionMode(t);I.add(G,{left:600,top:40});for(var i=0;i<20;i++ ){G.add(new qx.ui.form.ListItem(D+i,g));}
...
);G.addListener(b,function(e){var bf=e.getOriginalTarget();if(bf instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__JM=bf;}
...
;}
);G.addListener(f,function(e){this.__JN(e.getOriginalTarget(),e.getDragTarget());}
,this);H.addListener(f,function(e){this.__JN(this.__JM);}
,this);}
,__JN:function(bi,bg){if(bi.classname!=m){return;}
;var bh=this.__fA.getSortedSelection();if(bh.length===0){bh=[bg];}
else if(bh.indexOf(bg)==-1){bh=[bg];}
;for(var i=0,l=bh.length;i<l;i++ ){this.__fA.addBefore(bh[i],bi);}
;}
