...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow.aea41804a8a9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var x=new qx.ui.layout.LineSizeIterator(D,this.getSpacingX());var B=z.top;while(x.hasMoreLines()){var C=x.computeNextLine(A);this.__sE(C,B,A,z);B+=C.height+this.getSpacingY();}
;}
,__sE:function(L,K,I,E){var H=qx.ui.layout.Util;var F=E.left;if(this.getAlignX()!=l){F=E.left+I-L.width;if(this.getAlignX()==j){F=E.left+Math.round((I-L.width)/2);}
;}
...
;}
,_computeSizeHint:function(){return this.__sF(Infinity);}
,hasHeightForWidth:function(){return true;}
,getHeightForWidth:function(P){return this.__sF(P).height;}
...
;return S;}
,__sF:function(T){var U=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var V=0;var W=0;var X=0;while(U.hasMoreLines()){var Y=U.computeNextLine(T);X+=1;W=Math.max(W,Y.width);V+=Y.height;}
;return {width:W,height:V+this.getSpacingY()*(X-1)};}
}});}
)();
(function(){var a="No more lines to compute",b="qx.ui.layout.LineSizeIterator";qx.Class.define(b,{extend:Object,construct:function(d,c){this.__fu=d;this.__po=c;this.__pp=d.length>0;this.__pq=0;}
,members:{__fu:null,__po:null,__pp:null,__pq:null,computeNextLine:function(k){var k=k||Infinity;if(!this.__pp){throw new Error(a);}
;var p=this.__fu;var n=0;var f=0;var o=[];var j=[];for(var i=this.__pq;i<p.length;i++ ){var m=p[i];var q=m.getSizeHint();var g=this.__pr(i);var h=q.width+g;var e=i==this.__pq;if(!e&&f+h>k){this.__pq=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);j.push(g);f+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__pq=i+1;break;}
;}
;if(i>=p.length){this.__pp=false;}
;return {height:n,width:f,children:o,gapsBefore:j};}
,__pr:function(r){var s=r==this.__pq;if(s){return this.__fu[r].getMarginLeft();}
else {return Math.max(this.__fu[r-1].getMarginRight(),this.__fu[r].getMarginLeft(),this.__po);}
;}
,hasMoreLines:function(){return this.__pp;}
}});}
)();
