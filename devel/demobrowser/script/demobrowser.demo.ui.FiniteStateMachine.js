...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.b246c2e4c651.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c="__tw",d=" => ",f="#autoActionsBeforeOnexit",g=": Could not find friendly name for '",h=": event '",j=" blocked.  Re-queuing.",k="string",l="qx.event.type.Data",m="'",n="Attempt to transition to nonexistent state ",o="Saved-state stack is empty",p="Internal error: invalid nextState",q=" not handled.  Ignoring.",r="State ",s="#autoActionsBeforeOntransition",u=" returned a value other than ",v="#",w=" already exists",x="Attempt to transition to POP_STATE_STACK ",y=":",z="Transition ",A=" does not exist",B="' on '",C="qx.util.fsm.FiniteStateMachine",D="String",E="terminated",F="#autoActionsAfterOntransition",G="",H="#actionsAfterOnentry",I="Saved-state stack is full",J="#autoActionsBeforeOnentry",K=": no transition found.  No state change.",L="__tv",M="#autoActionsAfterOnentry",N="#actionsBeforeOnentry",O="', because the finite state machine is not running.",P=": Pushed event: ",Q=": Cannot listen to event '",R="Machine started with no available states",S=": Process event: ",T="#entry",U="#ontransition",V=" on ",W="Invalid state: not an instance of ",X="Number",Y="#exit",bp="true, false, or null.",bq="Explicit transition ",br="  ",bl="qx.util.fsm.State",bm="while state stack is empty.",bn=".  Ignoring.",bo="#autoActionsAfterOnexit",bs=": Event '",bt="object",bu=" not handled for target ",bv=": Queued event: ";qx.Class.define(C,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__hc={};this.__tt=null;this.__tu=[];this.__tv=[];this.__tw=[];this.__tx={};this.__ty={};this.__tz={};this.__tA={};this.__tB={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":l},properties:{name:{check:D,nullable:true},state:{check:D,nullable:true},previousState:{check:D,nullable:true},nextState:{check:D,nullable:true},maxSavedStates:{check:X,init:2},debugFlags:{check:X,init:7}},members:{__hc:null,__tt:null,__tv:null,__tw:null,__tu:null,__tx:null,__ty:null,__tz:null,__tA:null,__tB:null,__tC:false,__tD:true,isTerminated:function(){return this.__tD;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var by=bx.getName();if(by in this.__hc){throw new Error(r+by+w);}
;if(this.__tt==null){this.__tt=by;}
;this.__hc[by]=bx;}
,replaceState:function(bC,bB){if(!bC instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var bA=bC.getName();var bz=this.__hc[bA];this.__hc[bA]=bC;if(bB){bz._bNeedDispose=true;}
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__ty[bG]=bH;this.__tz[bH]=bG;this.__tx[bG]=bD;if(!bE){return;}
;if(typeof (bE)==k){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__tA[bF]){this.__tA[bF]={};}
;this.__tA[bF][bG]=true;if(!this.__tB[bG]){this.__tB[bG]=[];}
;this.__tB[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__ty[bL];if(this.__tB[bL]){for(var i=0;i<this.__tB[bL].length;i++ ){bJ=this.__tB[bL][i];delete this.__tA[bJ][bL];bM=true;for(bK in this.__tA[bJ]){bM=false;break;}
;if(bM){delete this.__tA[bJ];}
;}
;delete this.__tB[bL];}
;delete this.__tz[bI];delete this.__ty[bL];delete this.__tx[bL];}
,getObject:function(bN){return this.__tx[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__tz[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__tA[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__ty){var bT=this.__ty[bR];var bS=this.getObject(bR);this.debug(bR+d+bT);this.debug(br+bT+d+this.__tz[bT]);this.debug(br+bR+d+this.getObject(bR));this.debug(br+this.getObject(bR)+d+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__hc,"startState":this.__tt,"eventQueue":this.__tv,"blockedEvents":this.__tw,"savedStates":this.__tu,"friendlyToObject":this.__tx,"friendlyToHash":this.__ty,"hashToFriendly":this.__tz,"groupToFriendly":this.__tA,"friendlyToGroups":this.__tB});}
,start:function(){this.__tD=false;var bV=this.__tt;if(bV==null){throw new Error(R);}
...
;this.__hc[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+v+bV+T);}
;this.__hc[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+v+bV+H);}
;this.__hc[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__tu.length>=this.getMaxSavedStates()){throw new Error(I);}
;if(bW===true){this.__tu.push(this.getState());}
else if(bW){this.__tu.push(bW);}
else {this.__tu.push(this.getPreviousState());}
;}
,popState:function(){if(this.__tu.length==0){throw new Error(o);}
;return this.__tu.pop();}
,postponeEvent:function(event){this.__tw.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__tv.push(event);}
else {this.__tv.unshift(event);}
...
;}
;}
,eventListener:function(event){if(this.__tD){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__tE();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__tD){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__tE:function(){if(this.__tC){return;}
;this.__tC=true;while(this.__tv.length>0){var event=this.__tv.pop();var ch=this.__tF(event);if(ch){event.dispose();}
;}
;this.__tC=false;}
,__tF:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:G));}
;ck=this.getState();cn=this.__hc[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+m+q);}
;return true;}
...
;this.__tw.unshift(event);return false;default:if(typeof (cp)==k){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+A);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(z+ck+y+t+u+bp);};cl=cm.getNextState();if(typeof (cl)==k){if(!cl in this.__hc){throw new Error(n+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__tu.length==0){throw new Error(x+bm);}
;cl=this.__tu.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__tD=true;this.setNextState(null);break;default:throw new Error(p);};}
...
;if(this.__tD){if(cr){this.debug(this.getName()+v+b);}
;this.fireDataEvent(E,ck);return true;}
;cn=this.__hc[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+v+ck+J);}
...
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__tw.length;i++ ){e=this.__tw.pop();this.__tv.unshift(e);}
;if(cs){this.debug(this.getName()+v+cj+d+this.getName()+v+ck);}
;return true;}
;if(cs){this.debug(this.getName()+v+ck+h+event.getType()+m+K);}
;return true;}
},destruct:function(){this._disposeArray(L);this._disposeArray(c);this.__tu=this.__hc=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k=")",l=" fsm.getObject('",m="groups",n="Invalid function parameters: ",o="Invalid onexit type: ",p="  for (var i = 0; i < groupObjects.length; i++)",q="qx.util.fsm.Transition",r="context",s="__tm",t="string",u="autoActionsAfterOnentry",v="Invalid numeric value in events object: ",w='"',x="Invalid state name",y="__tn",z=" value: ",A="');",B="State ",C="autoActionsBeforeOnexit",D="autoActionsBeforeOnentry",E="__tk",F="}",G="): ",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L=": ",M="expected array, found ",N="Invalid friendly name in 'objects' list: ",O="autoActionsAfterOnexit",P="  fsm.debug(ex);",Q="(",R="Invalid onentry type: ",S="    fsm.getObject(objName).",T=")(fsm)",U="events",V="undefined",W="Invalid ",X="try",Y="    var objName = groupObjects[i];",ba="__tq",bb="').",bc="__tl",bd="objects",be="expected object, found ",bf="__tr",bg="The events object must be provided in new state info",bh="catch(ex)",bi="  {",bj="__to",bk="    fsm.getGroupObjects('",bl="Invalid value in events object ",bm="Invalid transition: not an instance of ",bn="  }",bo="Invalid value in events object: ",bp="Invalid 'objects' list: expected array, got ",bq="Invalid numeric value in events object ",br="qx.util.fsm.State",bs="onentry",bt=";",bu="Invalid function request parameter type: ",bv="Invalid 'groups' list: expected array, got ",bw="fsm",bx=",",by="__tp",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(br,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(r,bE);for(var bC in bF){switch(bC){case bs:this.setOnentry(this.__ts(bF[bC],bE));break;case I:this.setOnexit(this.__ts(bF[bC],bE));break;case D:this.setAutoActionsBeforeOnentry(bF[bC]);break;case u:this.setAutoActionsAfterOnentry(bF[bC]);break;case C:this.setAutoActionsBeforeOnexit(bF[bC]);break;case O:this.setAutoActionsAfterOnexit(bF[bC]);break;case U:this.setEvents(bF[bC]);break;case r:break;default:this.setUserData(bC,bF[bC]);this.debug(B+bD+L+bz+bC);break;};}
...
;for(var i=0;i<functionRequest.length;i++ ){objectAndGroupList=functionRequest[i];if(typeof (objectAndGroupList)!=bB){throw new Error(bu+be+typeof (functionRequest[param]));}
...
;func+=l+a[j]+bb+funcFragment+bt;}
;var g=objectAndGroupList[m];if(g){if(!g instanceof Array){throw new Error(bv+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=d+bk+g[j]+A+p+bi+Y+S+funcFragment+bt+bn;}
;}
;}
;}
;func+=F+bh+J+P+F;return qx.lang.Function.bind(new Function(bw,func),context);}
},properties:{name:{transform:E,nullable:true},onentry:{transform:bc,nullable:true,init:function(bG,event){}
},onexit:{transform:s,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:bj,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:by,nullable:true,init:function(bJ,event){}
...
},events:{transform:y,nullable:true}},members:{__tk:function(bM){if(typeof (bM)!=t||bM.length<1){throw new Error(x);}
;return bM;}
,__tl:function(bN){switch(typeof (bN)){case V:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(R+typeof (bN));};}
,__tm:function(bP){switch(typeof (bP)){case V:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(o+typeof (bP));};}
,__tn:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
...
else if(typeof (bR[bS])!=t&&typeof (bR[bS])!=bA){throw new Error(bl+Q+e+G+bS+L+bR[bS]);}
;}
;}
else if(typeof (bR)!=t&&typeof (bR)!=bA){throw new Error(bo+e+L+bT[e]);}
;}
;return bT;}
,__to:function(bU){return qx.util.fsm.State._commonTransformAutoActions(D,bU,this.getUserData(r));}
,__tp:function(bV){return qx.util.fsm.State._commonTransformAutoActions(u,bV,this.getUserData(r));}
,__tq:function(bW){return qx.util.fsm.State._commonTransformAutoActions(C,bW,this.getUserData(r));}
,__tr:function(bX){return qx.util.fsm.State._commonTransformAutoActions(O,bX,this.getUserData(r));}
,__ts:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+q);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="autoActionsAfterOntransition",e="__tG",g="context",h="string",i="predicate",j="nextState must be an explicit state name, ",k="__tI",l="Transition ",m=": ",n="__tK",o="number",p="Invalid transition nextState type: ",q="Invalid transition nextState value: ",r="Adding user-provided field to transition: ",s="ontransition",t="Invalid transition predicate type: ",u="boolean",v="nextState",w="Invalid ontransition type: ",x="String",y="__tJ",z="__tH",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(g,E);for(var C in D){switch(C){case i:this.setPredicate(this.__ts(D[C],E));break;case v:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__ts(D[C],E));break;case d:this.setAutoActionsAfterOntransition(this.__ts(D[C],E));break;case s:this.setOntransition(this.__ts(D[C],E));break;case g:break;default:this.setUserData(C,D[C]);this.debug(l+F+m+r+C);break;};}
;}
,properties:{name:{check:x,nullable:true},predicate:{init:function(G,event){return true;}
,transform:e},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:z},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:k},autoActionsAfterOntransition:{init:function(I,event){}
,transform:y},ontransition:{init:function(J,event){}
,transform:n}},members:{__tG:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
;case u:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(g));default:throw new Error(t+typeof (K));};}
,__tH:function(N){switch(typeof (N)){case h:return N;case o:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(q+N+m+j+c);};break;default:throw new Error(p+typeof (N));};}
,__tI:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(g));}
,__tJ:function(P){return qx.util.fsm.State._commonTransformAutoActions(d,P,this.getUserData(g));}
,__tK:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(g));default:throw new Error(w+typeof (Q));};}
,__ts:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
