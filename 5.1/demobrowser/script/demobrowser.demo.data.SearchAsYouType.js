...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.b8a91b9161c6.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="_applySearchString",b="demobrowser.demo.data.filter.SearchAsYouTypeFilter",c="String",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__MF=e;this.filter=qx.lang.Function.bind(function(f){return f.search(this.getSearchString())!=-1;}
,this);this.__mH=null;}
,properties:{searchString:{check:c,apply:a,init:d}},members:{__mH:null,__MF:null,_applySearchString:function(h,g){var i=qx.util.TimerManager.getInstance();if(this.__mH!=null){i.stop(this.__mH);this.__mH=null;}
;this.__mH=i.start(function(){this.__MF.update();this.__mH=null;}
,0,this,null,200);}
,filter:null},destruct:function(){this.__MF=null;}
});}
)();
(function(){var a="number",b="qx.util.TimerManager",c="interval",d="singleton";qx.Class.define(b,{extend:qx.core.Object,type:d,statics:{__mF:[],__mG:{},__mH:0},members:{__mI:false,start:function(h,g,j,e,k){if(typeof k!=a){k=g||0;}
;var f=(new Date()).getTime()+k;this.self(arguments).__mG[ ++this.self(arguments).__mH]={callback:h,userData:e||null,expireAt:f,recurTime:g,context:j||this};this.__mJ(f,this.self(arguments).__mH);return this.self(arguments).__mH;}
,stop:function(l){var m=this.self(arguments).__mF;var length=m.length;for(var i=0;i<length;i++ ){if(m[i]==l){m.splice(i,1);break;}
;}
;delete this.self(arguments).__mG[l];if(m.length==0&&this.__mI){qx.event.Idle.getInstance().removeListener(c,this.__mK,this);this.__mI=false;}
;}
,__mJ:function(n,q){var p=this.self(arguments).__mF;var o=this.self(arguments).__mG;var length=p.length;for(var i=0;i<length;i++ ){if(o[p[i]].expireAt>n){p.splice(i,0,q);break;}
;}
;if(p.length==length){p.push(q);}
;if(!this.__mI){qx.event.Idle.getInstance().addListener(c,this.__mK,this);this.__mI=true;}
;}
,__mK:function(){var s=(new Date()).getTime();var u=this.self(arguments).__mF;var t=this.self(arguments).__mG;while(u.length>0&&t[u[0]].expireAt<=s){var w=u.shift();var r=t[w];r.callback.call(r.context,r.userData,w);if(r.recurTime&&t[w]){var v=(new Date()).getTime();r.expireAt=v+r.recurTime;this.__mJ(r.expireAt,w);}
else {delete t[w];}
;}
;if(u.length==0&&this.__mI){qx.event.Idle.getInstance().removeListener(c,this.__mK,this);this.__mI=false;}
;}
}});}
)();
