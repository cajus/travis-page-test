...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.72ab85ed7fe1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
;e.addData(ba,W);}
);var L=new qx.ui.basic.Label(w);I.add(L,{left:140,top:20});var J=new qx.ui.form.List();J.setDroppable(true);J.setSelectionMode(t);I.add(J,{left:140,top:40});J.addListener(f,function(e){this.debug(E+e.getRelatedTarget());var bb=e.getData(C);for(var i=0,l=bb.length;i<l;i++ ){this.add(bb[i]);}
;}
...
;}
);var N=new qx.ui.basic.Label(k);I.add(N,{left:600,top:20});var G=this.__fW=new qx.ui.form.List();G.setDraggable(true);G.setDroppable(true);G.setSelectionMode(t);I.add(G,{left:600,top:40});for(var i=0;i<20;i++ ){G.add(new qx.ui.form.ListItem(D+i,g));}
...
);G.addListener(b,function(e){var bf=e.getOriginalTarget();if(bf instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Oa=bf;}
...
;}
);G.addListener(f,function(e){this.__Ob(e.getOriginalTarget(),e.getDragTarget());}
,this);H.addListener(f,function(e){this.__Ob(this.__Oa);}
,this);}
,__Ob:function(bi,bg){if(bi.classname!=m){return;}
;var bh=this.__fW.getSortedSelection();if(bh.length===0){bh=[bg];}
else if(bh.indexOf(bg)==-1){bh=[bg];}
;for(var i=0,l=bh.length;i<l;i++ ){this.__fW.addBefore(bh[i],bi);}
;}
...
,properties:{wrap:{check:b,init:true,apply:f},appearance:{refine:true,init:j},singleStep:{check:i,init:20},minimalLineHeight:{check:i,apply:h,init:4},autoSize:{check:b,apply:w,init:false}},members:{__wf:null,__wg:null,__wh:null,setValue:function(M){M=qx.ui.form.AbstractField.prototype.setValue.call(this,M);this.__wi();return M;}
...
;}
,__wi:function(){if(this.isAutoSize()){var R=this.__wj();if(R&&this.getBounds()){this.__wh=this.__wh||this._getAreaHeight();var T=this._getScrolledAreaHeight();if(this.getMaxHeight()){var P=this.getInsets();var Q=-P.top+this.getMaxHeight()-P.bottom;if(T>Q){this.getContentElement().setStyle(I,k);}
else {this.getContentElement().setStyle(I,H);}
;}
;var S=Math.max(T,this.__wh);this._setAreaHeight(S);}
else {this.getContentElement().addListenerOnce(x,function(){this.__wi();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(U){if(this._getAreaHeight()!==U){this.__wg=U;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__wm();}
;}
,_getScrolledAreaHeight:function(){var W=this.__wj();var V=W.getDomElement();if(V){if(!V.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;this.__wl(W);if(qx.core.Environment.get(C)==B&&qx.core.Environment.get(s)==8){if(!V.scrollTop){qx.html.Element.flush();}
;}
;return V.scrollTop;}
;}
,__wj:function(){this.__wf=this.__wf||this.__wk();return this.__wf;}
,__wk:function(){var ba,Y,bb,X;ba=this.getContentElement();if(!ba.getDomElement()){return null;}
;bb=qx.bom.Element.clone(ba.getDomElement());X=new qx.html.Input(j);X.useElement(bb);Y=X;Y.setStyles({position:K,top:0,left:o,height:0,overflow:H},true);Y.removeAttribute(G);Y.removeAttribute(z);Y.setAttribute(y,g);Y.setValue(ba.getValue()||t);Y.insertBefore(ba);this.__wl(Y);return Y;}
,__wl:function(bc){bc=bc.getDomElement();if(bc){bc.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bh,bg){if(qx.core.Environment.get(m)){this.__wn();}
;if(bh){this.__wi();this.addListener(a,this.__wi,this);if(!this.getBounds()){this.addListenerOnce(x,function(){this.getContentElement().setStyle(I,H);}
);}
else {this.getContentElement().setStyle(I,H);}
;}
else {this.removeListener(a,this.__wi);this.getContentElement().setStyle(I,k);}
;}
,_applyDimension:function(bi){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(m)){this.__wn();}
;if(bi===this.getMaxHeight()){this.__wi();}
;}
,__wm:function(){var content=this.getContentElement();var bk=content.getDomElement();var bj=content.getStyle(u);content.setStyle(u,parseInt(bj,10)+1000+c,true);if(bk){qx.bom.element.Dimension.getWidth(bk);}
;content.setStyle(u,bj,true);}
,__wn:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(n+J+v);}
;}
,_getContentHint:function(){var bl=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bl.height=bl.height*this.getMinimalLineHeight();bl.width=this._getTextSize().width*20;if(this.isAutoSize()){bl.height=this.__wg||bl.height;}
;return bl;}
},destruct:function(){this.setAutoSize(false);if(this.__wf){this.__wf.dispose();}
;}
});}
)();
