...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.SplitPane.f792a61284af.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="slider",b="splitter",c="px",d="Integer",f="height",g="move",h="maxHeight",i="resize",j="pointerup",k="vertical",l="width",m="_applyOrientation",n="_applyOffset",o="splitpane",p="qx.ui.splitpane.Pane",q="pointerdown",r="top",s="minHeight",t="knob",u="horizontal",v="minWidth",w="appear",z="losecapture",A="left",B="pointermove",C="maxWidth",D="pointerout";qx.Class.define(p,{extend:qx.ui.core.Widget,construct:function(E){qx.ui.core.Widget.call(this);this.__bQ=[];if(E){this.setOrientation(E);}
else {this.initOrientation();}
;this.__bY.addListener(q,this._onPointerDown,this);this.__bY.addListener(j,this._onPointerUp,this);this.__bY.addListener(B,this._onPointerMove,this);this.__bY.addListener(D,this._onPointerOut,this);this.__bY.addListener(z,this._onPointerUp,this);}
,properties:{appearance:{refine:true,init:o},offset:{check:d,init:6,apply:n},orientation:{init:u,check:[u,k],apply:m}},members:{__bR:null,__bS:false,__bT:null,__bU:null,__bV:null,__bW:null,__bX:null,__bQ:null,__bY:null,_createChildControlImpl:function(H,G){var F;switch(H){case a:F=new qx.ui.splitpane.Slider(this);F.exclude();this._add(F,{type:H});break;case b:F=new qx.ui.splitpane.Splitter(this);this._add(F,{type:H});F.addListener(g,this.__ca,this);break;};return F||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,H);}
,__ca:function(e){this.__cc(e.getData());}
,__cb:function(K){this.__bY=new qx.ui.splitpane.Blocker(K);this.getContentElement().add(this.__bY);var I=this.getChildControl(b);var J=I.getWidth();if(!J){I.addListenerOnce(w,function(){this.__cc();}
,this);}
;I.addListener(i,function(e){var L=e.getData();if(L.height==0||L.width==0){this.__bY.hide();}
else {this.__bY.show();}
;}
,this);}
,getBlocker:function(){return this.__bY;}
,_applyOrientation:function(Q,N){var R=this.getChildControl(a);var M=this.getChildControl(b);this.__bV=Q===u;if(!this.__bY){this.__cb(Q);}
;this.__bY.setOrientation(Q);var P=this._getLayout();if(P){P.dispose();}
;var O=Q===k?new qx.ui.splitpane.VLayout:new qx.ui.splitpane.HLayout;this._setLayout(O);M.removeState(N);M.addState(Q);M.getChildControl(t).removeState(N);M.getChildControl(t).addState(Q);R.removeState(N);R.addState(Q);qx.ui.core.queue.Manager.flush();this.__cc();}
,_applyOffset:function(T,S){this.__cc();}
,__cc:function(V){var U=this.getChildControl(b);var ba=this.getOffset();var bb=U.getBounds();var Y=U.getContentElement().getDomElement();if(!Y){return;}
;if(this.__bV){var X=null;if(V){X=V.width;}
else if(bb){X=bb.width;}
;var bc=V&&V.left;if(X){if(isNaN(bc)){bc=qx.bom.element.Location.getPosition(Y).left;}
;this.__bY.setWidth(ba,X);this.__bY.setLeft(ba,bc);}
;}
...
;this.__bY.setHeight(ba,W);this.__bY.setTop(ba,top);}
;}
;}
,add:function(be,bd){if(bd==null){this._add(be);}
else {this._add(be,{flex:bd});}
;this.__bQ.push(be);}
,remove:function(bf){this._remove(bf);qx.lang.Array.remove(this.__bQ,bf);}
,getChildren:function(){return this.__bQ;}
,_onPointerDown:function(e){if(!e.isLeftPressed()){return;}
;var bg=this.getChildControl(b);var bj=bg.getContentLocation();var bh=this.getContentLocation();this.__bR=this.__bV?e.getDocumentLeft()-bj.left+bh.left:e.getDocumentTop()-bj.top+bh.top;var bi=this.getChildControl(a);var bk=bg.getBounds();bi.setUserBounds(bk.left,bk.top,bk.width,bk.height);bi.setZIndex(bg.getZIndex()+1);bi.show();this.__bS=true;this.__bY.capture();e.stop();}
,_onPointerMove:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());if(this.__bS){this.__cd();var bl=this.getChildControl(a);var bm=this.__bW;if(this.__bV){bl.setDomLeft(bm);this.__bY.setStyle(A,(bm-this.getOffset())+c);}
else {bl.setDomTop(bm);this.__bY.setStyle(r,(bm-this.getOffset())+c);}
;e.stop();}
;}
,_onPointerOut:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());}
,_onPointerUp:function(e){if(!this.__bS){return;}
;this._finalizeSizes();var bn=this.getChildControl(a);bn.exclude();this.__bS=false;this.releaseCapture();e.stop();}
,_finalizeSizes:function(){var br=this.__bW;var bo=this.__bX;if(br==null){return;}
...
else {if(this.__bV){bs.setWidth(br);bq.setWidth(bo);}
else {bs.setHeight(br);bq.setHeight(bo);}
;}
;}
,__cd:function(){if(this.__bV){var bx=v,bE=l,by=C,bC=this.__bT;}
else {var bx=s,bE=f,by=h,bC=this.__bU;}
;var bD=this._getChildren();var bv=bD[2].getSizeHint();var bA=bD[3].getSizeHint();var bB=bD[2].getBounds()[bE]+bD[3].getBounds()[bE];var bz=bC-this.__bR;var bw=bB-bz;if(bz<bv[bx]){bw-=bv[bx]-bz;bz=bv[bx];}
else if(bw<bA[bx]){bz-=bA[bx]-bw;bw=bA[bx];}
;if(bz>bv[by]){bw+=bz-bv[by];bz=bv[by];}
else if(bw>bA[by]){bz+=bw-bA[by];bw=bA[by];}
;this.__bW=bz;this.__bX=bw;}
,_isActiveDragSession:function(){return this.__bS;}
,_setLastPointerPosition:function(x,y){this.__bT=x;this.__bU=y;}
