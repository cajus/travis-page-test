...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.418101c742dd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c=" => ",d="#autoActionsBeforeOnexit",f=": Could not find friendly name for '",g=": event '",h=" blocked.  Re-queuing.",j="string",k="qx.event.type.Data",l="'",m="Attempt to transition to nonexistent state ",n="Saved-state stack is empty",o="Internal error: invalid nextState",p=" not handled.  Ignoring.",q="State ",r="#autoActionsBeforeOntransition",s=" returned a value other than ",u="#",v="__rS",w=" already exists",x="Attempt to transition to POP_STATE_STACK ",y=":",z="Transition ",A=" does not exist",B="' on '",C="qx.util.fsm.FiniteStateMachine",D="String",E="terminated",F="#autoActionsAfterOntransition",G="",H="#actionsAfterOnentry",I="Saved-state stack is full",J="#autoActionsBeforeOnentry",K=": no transition found.  No state change.",L="#autoActionsAfterOnentry",M="#actionsBeforeOnentry",N="', because the finite state machine is not running.",O=": Pushed event: ",P=": Cannot listen to event '",Q="Machine started with no available states",R=": Process event: ",S="#entry",T="#ontransition",U=" on ",V="Invalid state: not an instance of ",W="Number",X="#exit",Y="true, false, or null.",bp="Explicit transition ",bq="  ",br="qx.util.fsm.State",bl="while state stack is empty.",bm=".  Ignoring.",bn="#autoActionsAfterOnexit",bo=": Event '",bs="object",bt=" not handled for target ",bu="__rT",bv=": Queued event: ";qx.Class.define(C,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__eS={};this.__rQ=null;this.__rR=[];this.__rS=[];this.__rT=[];this.__rU={};this.__rV={};this.__rW={};this.__rX={};this.__rY={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":k},properties:{name:{check:D,nullable:true},state:{check:D,nullable:true},previousState:{check:D,nullable:true},nextState:{check:D,nullable:true},maxSavedStates:{check:W,init:2},debugFlags:{check:W,init:7}},members:{__eS:null,__rQ:null,__rS:null,__rT:null,__rR:null,__rU:null,__rV:null,__rW:null,__rX:null,__rY:null,__sa:false,__sb:true,isTerminated:function(){return this.__sb;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(V+br);}
;var by=bx.getName();if(by in this.__eS){throw new Error(q+by+w);}
;if(this.__rQ==null){this.__rQ=by;}
;this.__eS[by]=bx;}
,replaceState:function(bC,bB){if(!bC instanceof qx.util.fsm.State){throw new Error(V+br);}
;var bA=bC.getName();var bz=this.__eS[bA];this.__eS[bA]=bC;if(bB){bz._bNeedDispose=true;}
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__rV[bG]=bH;this.__rW[bH]=bG;this.__rU[bG]=bD;if(!bE){return;}
;if(typeof (bE)==j){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__rX[bF]){this.__rX[bF]={};}
;this.__rX[bF][bG]=true;if(!this.__rY[bG]){this.__rY[bG]=[];}
;this.__rY[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__rV[bL];if(this.__rY[bL]){for(var i=0;i<this.__rY[bL].length;i++ ){bJ=this.__rY[bL][i];delete this.__rX[bJ][bL];bM=true;for(bK in this.__rX[bJ]){bM=false;break;}
;if(bM){delete this.__rX[bJ];}
;}
;delete this.__rY[bL];}
;delete this.__rW[bI];delete this.__rV[bL];delete this.__rU[bL];}
,getObject:function(bN){return this.__rU[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__rW[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__rX[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__rV){var bT=this.__rV[bR];var bS=this.getObject(bR);this.debug(bR+c+bT);this.debug(bq+bT+c+this.__rW[bT]);this.debug(bq+bR+c+this.getObject(bR));this.debug(bq+this.getObject(bR)+c+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__eS,"startState":this.__rQ,"eventQueue":this.__rS,"blockedEvents":this.__rT,"savedStates":this.__rR,"friendlyToObject":this.__rU,"friendlyToHash":this.__rV,"hashToFriendly":this.__rW,"groupToFriendly":this.__rX,"friendlyToGroups":this.__rY});}
,start:function(){this.__sb=false;var bV=this.__rQ;if(bV==null){throw new Error(Q);}
...
;this.__eS[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+u+bV+S);}
;this.__eS[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+u+bV+H);}
;this.__eS[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__rR.length>=this.getMaxSavedStates()){throw new Error(I);}
;if(bW===true){this.__rR.push(this.getState());}
else if(bW){this.__rR.push(bW);}
else {this.__rR.push(this.getPreviousState());}
;}
,popState:function(){if(this.__rR.length==0){throw new Error(n);}
;return this.__rR.pop();}
,postponeEvent:function(event){this.__rT.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__rS.push(event);}
else {this.__rS.unshift(event);}
...
;}
;}
,eventListener:function(event){if(this.__sb){this.debug(this.getName()+P+event.getType()+N);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__sc();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__sb){this.debug(this.getName()+P+cb+N);return;}
...
,this,cg||20);}
,__sc:function(){if(this.__sa){return;}
;this.__sa=true;while(this.__rS.length>0){var event=this.__rS.pop();var ch=this.__sd(event);if(ch){event.dispose();}
;}
;this.__sa=false;}
,__sd:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+R+event.getType()+(ct?U+ct:G));}
;ck=this.getState();cn=this.__eS[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bo+event.getType()+l+p);}
;return true;}
...
;this.__rT.unshift(event);return false;default:if(typeof (cp)==j){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bp+cp+A);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(z+ck+y+t+s+Y);};cl=cm.getNextState();if(typeof (cl)==j){if(!cl in this.__eS){throw new Error(m+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__rR.length==0){throw new Error(x+bl);}
;cl=this.__rR.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__sb=true;this.setNextState(null);break;default:throw new Error(o);};}
...
;if(this.__sb){if(cr){this.debug(this.getName()+u+b);}
;this.fireDataEvent(E,ck);return true;}
;cn=this.__eS[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+u+ck+J);}
...
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__rT.length;i++ ){e=this.__rT.pop();this.__rS.unshift(e);}
;if(cs){this.debug(this.getName()+u+cj+c+this.getName()+u+ck);}
;return true;}
;if(cs){this.debug(this.getName()+u+ck+g+event.getType()+l+K);}
;return true;}
},destruct:function(){this._disposeArray(v);this._disposeArray(bu);this.__rR=this.__eS=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k=")",l=" fsm.getObject('",m="groups",n="Invalid function parameters: ",o="Invalid onexit type: ",p="  for (var i = 0; i < groupObjects.length; i++)",q="context",r="string",s="autoActionsAfterOnentry",t="Invalid numeric value in events object: ",u='"',v="__rH",w="__rJ",x="Invalid state name",y=" value: ",z="');",A="State ",B="autoActionsBeforeOnexit",C="autoActionsBeforeOnentry",D="__rN",E="}",F="): ",G="events must be an object",H="onexit",I="{",J="Invalid function request type: ",K=": ",L="expected array, found ",M="Invalid friendly name in 'objects' list: ",N="autoActionsAfterOnexit",O="  fsm.debug(ex);",P="(",Q="__rL",R="Invalid onentry type: ",S="    fsm.getObject(objName).",T=")(fsm)",U="events",V="undefined",W="Invalid ",X="try",Y="    var objName = groupObjects[i];",ba="Invalid value in events object: ",bb="').",bc="objects",bd="expected object, found ",be="qx.util.fsm.Transition",bf="The events object must be provided in new state info",bg="catch(ex)",bh="  {",bi="    fsm.getGroupObjects('",bj="Invalid value in events object ",bk="Invalid transition: not an instance of ",bl="  }",bm="__rK",bn="__rO",bo="__rI",bp="Invalid 'objects' list: expected array, got ",bq="Invalid numeric value in events object ",br="qx.util.fsm.State",bs="onentry",bt="__rM",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(br,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(q,bE);for(var bC in bF){switch(bC){case bs:this.setOnentry(this.__rP(bF[bC],bE));break;case H:this.setOnexit(this.__rP(bF[bC],bE));break;case C:this.setAutoActionsBeforeOnentry(bF[bC]);break;case s:this.setAutoActionsAfterOnentry(bF[bC]);break;case B:this.setAutoActionsBeforeOnexit(bF[bC]);break;case N:this.setAutoActionsAfterOnexit(bF[bC]);break;case U:this.setEvents(bF[bC]);break;case q:break;default:this.setUserData(bC,bF[bC]);this.debug(A+bD+K+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bf);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(actionType,value,context){if(typeof (value)!=bB){throw new Error(W+actionType+y+typeof (value));}
;var funcFragment;var func=X+I;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(J+L+typeof (functionRequest));}
...
;if(typeof (params[j])==c){funcFragment+=P+params[j]+T;}
...
;for(j=0;j<g.length;j++ ){func+=d+bi+g[j]+z+p+bh+Y+S+funcFragment+bu+bl;}
;}
;}
;}
;func+=E+bg+I+O+E;return qx.lang.Function.bind(new Function(bx,func),context);}
...
},autoActionsBeforeOnentry:{transform:Q,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:bt,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:D,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:bn,nullable:true,init:function(bL,event){}
},events:{transform:bm,nullable:true}},members:{__rH:function(bM){if(typeof (bM)!=r||bM.length<1){throw new Error(x);}
;return bM;}
,__rI:function(bN){switch(typeof (bN)){case V:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(q));default:throw new Error(R+typeof (bN));};}
,__rJ:function(bP){switch(typeof (bP)){case V:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(q));default:throw new Error(o+typeof (bP));};}
,__rK:function(bT){if(typeof (bT)!=bB){throw new Error(G);}
...
else if(typeof (bR[bS])!=r&&typeof (bR[bS])!=bA){throw new Error(bj+P+e+F+bS+K+bR[bS]);}
;}
;}
else if(typeof (bR)!=r&&typeof (bR)!=bA){throw new Error(ba+e+K+bT[e]);}
;}
;return bT;}
,__rL:function(bU){return qx.util.fsm.State._commonTransformAutoActions(C,bU,this.getUserData(q));}
,__rM:function(bV){return qx.util.fsm.State._commonTransformAutoActions(s,bV,this.getUserData(q));}
,__rN:function(bW){return qx.util.fsm.State._commonTransformAutoActions(B,bW,this.getUserData(q));}
,__rO:function(bX){return qx.util.fsm.State._commonTransformAutoActions(N,bX,this.getUserData(q));}
,__rP:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bk+be);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="autoActionsAfterOntransition",e="__sg",g="__se",h="string",i="predicate",j="__sf",k="__si",l="nextState must be an explicit state name, ",m="Invalid transition predicate type: ",n="Transition ",o=": ",p="String",q="Invalid transition nextState type: ",r="number",s="__sh",t="Invalid transition nextState value: ",u="Adding user-provided field to transition: ",v="ontransition",w="context",x="boolean",y="nextState",z="Invalid ontransition type: ",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(w,E);for(var C in D){switch(C){case i:this.setPredicate(this.__rP(D[C],E));break;case y:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__rP(D[C],E));break;case d:this.setAutoActionsAfterOntransition(this.__rP(D[C],E));break;case v:this.setOntransition(this.__rP(D[C],E));break;case w:break;default:this.setUserData(C,D[C]);this.debug(n+F+o+u+C);break;};}
;}
,properties:{name:{check:p,nullable:true},predicate:{init:function(G,event){return true;}
,transform:g},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:j},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:e},autoActionsAfterOntransition:{init:function(I,event){}
,transform:s},ontransition:{init:function(J,event){}
,transform:k}},members:{__se:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
;case x:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(w));default:throw new Error(m+typeof (K));};}
,__sf:function(N){switch(typeof (N)){case h:return N;case r:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(t+N+o+l+c);};break;default:throw new Error(q+typeof (N));};}
,__sg:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(w));}
,__sh:function(P){return qx.util.fsm.State._commonTransformAutoActions(d,P,this.getUserData(w));}
,__si:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(w));default:throw new Error(z+typeof (Q));};}
,__rP:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
