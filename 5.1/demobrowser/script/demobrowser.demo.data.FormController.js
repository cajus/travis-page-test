...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.FormController.e76134bfe9e3.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="change",b="qx.ui.form.Form",c="qx.event.type.Event",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__tu=[];this._buttons=[];this._buttonOptions=[];this._validationManager=this._createValidationManager();this._resetter=this._createResetter();}
,events:{"change":c},members:{__tu:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(g,f,h,name,e,k){if(this.__LL()){this.__tu.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__tu[this._groupCounter].items.push(g);this.__tu[this._groupCounter].labels.push(f);this.__tu[this._groupCounter].options.push(k);if(name==null){name=f.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,d);}
;this.__tu[this._groupCounter].names.push(name);this._validationManager.add(g,h,e);this._resetter.add(g);this.fireEvent(a);}
,addGroupHeader:function(m,l){if(!this.__LL()){this._groupCounter++ ;}
;this.__tu.push({title:m,items:[],labels:[],names:[],options:[],headerOptions:l});this.fireEvent(a);}
,addButton:function(n,o){this._buttons.push(n);this._buttonOptions.push(o||null);this.fireEvent(a);}
,__LL:function(){return this.__tu.length===0;}
,remove:function(p){for(var i=0;i<this.__tu.length;i++ ){var r=this.__tu[i];for(var j=0;j<r.items.length;j++ ){var q=r.items[j];if(q===p){r.items.splice(j,1);r.labels.splice(j,1);r.names.splice(j,1);r.options.splice(j,1);this._validationManager.remove(p);this._resetter.remove(p);this.fireEvent(a);return true;}
;}
;}
;return false;}
,removeGroupHeader:function(u){for(var i=0;i<this.__tu.length;i++ ){var t=this.__tu[i];if(t.title===u){var s;if(i==0){if(this.__tu.length==1){t.title=null;t.headerOptions={};this.fireEvent(a);return true;}
else {s=this.__tu[i+1];}
;}
else {s=this.__tu[i-1];}
;s.items=s.items.concat(t.items);s.labels=s.labels.concat(t.labels);s.names=s.names.concat(t.names);s.options=s.options.concat(t.options);this.__tu.splice(i,1);this._groupCounter-- ;this.fireEvent(a);return true;}
;}
;return false;}
...
;}
;return false;}
,getItems:function(){var x={};for(var i=0;i<this.__tu.length;i++ ){var y=this.__tu[i];for(var j=0;j<y.names.length;j++ ){var name=y.names[j];x[name]=y.items[j];}
;}
;return x;}
...
,getGroups:function(){return this.__tu;}
...
},destruct:function(){this.__tu=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__HU=[];}
,members:{__HU:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__LG(e)){var f=e.getSelection();}
else if(this.__LO(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__HU.push({item:e,init:f});}
,remove:function(g){for(var i=0;i<this.__HU.length;i++ ){var h=this.__HU[i];if(h.item===g){this.__HU.splice(i,1);return true;}
;}
;return false;}
,reset:function(){for(var i=0;i<this.__HU.length;i++ ){var j=this.__HU[i];this.__LM(j.item,j.init);}
;}
,resetItem:function(k){var l;for(var i=0;i<this.__HU.length;i++ ){var m=this.__HU[i];if(m.item===k){l=m.init;break;}
;}
;if(l===undefined){throw new Error(a);}
;this.__LM(k,l);}
,__LM:function(n,o){if(this._supportsValue(n)){n.setValue(o);}
else if(this.__LG(n)||this.__LO(n)){n.setSelection(o);}
;}
,redefine:function(){for(var i=0;i<this.__HU.length;i++ ){var p=this.__HU[i].item;this.__HU[i].init=this.__LN(p);}
;}
,redefineItem:function(q){var r;for(var i=0;i<this.__HU.length;i++ ){if(this.__HU[i].item===q){r=this.__HU[i];break;}
;}
;if(r===undefined){throw new Error(a);}
;r.init=this.__LN(r.item);}
,__LN:function(s){if(this._supportsValue(s)){return s.getValue();}
else if(this.__LG(s)||this.__LO(s)){return s.getSelection();}
;}
,__LG:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.ui.core.ISingleSelection);}
,__LO:function(v){var w=v.constructor;return qx.Class.hasInterface(w,qx.data.controller.ISelection);}
...
,events:{"changeValue":l},properties:{dateFormat:{check:c,apply:s},placeholder:{check:r,nullable:true,apply:E},appearance:{refine:true,init:D},focusable:{refine:true,init:true},width:{refine:true,init:120}},statics:{__LQ:null,__LR:null,getDefaultDateFormatter:function(){var K=qx.locale.Date.getDateFormat(y).toString();if(K==this.__LQ){return this.__LR;}
;if(this.__LR){this.__LR.dispose();}
;this.__LR=new qx.util.format.DateFormat(K,qx.locale.Manager.getInstance().getLocale());this.__LQ=K;return this.__LR;}
},members:{__LS:null,_forwardStates:{focused:true,invalid:true},_setDefaultDateFormat:function(){this.setDateFormat(qx.ui.form.DateField.getDefaultDateFormatter());}
,_addLocaleChangeListener:function(){if(qx.core.Environment.get(x)){this.__LS=qx.locale.Manager.getInstance().addListener(o,function(){this._setDefaultDateFormat();}
,this);}
;}
...
},destruct:function(){if(qx.core.Environment.get(x)){if(this.__LS){qx.locale.Manager.getInstance().removeListenerById(this.__LS);}
;}
;}
});}
)();
(function(){var a='format',b="short",c="HH:mm",d='_',e="stand-alone",f="HHmmsszz",g="cldr_date_format_",h="HHmm",j="narrow",k="HHmmss",l="qx.debug",m="This case should never happen.",n="cldr_time_format_",o="sat",p="abbreviated",q="wide",r="HH:mm:ss zz",s="full",t="cldr_pm",u="cldr_day_",v="_",w="cldr_month_",x="long",y="wed",z="medium",A="fri",B="cldr_am",C="tue",D="qx.locale.Date",E="mon",F="cldr_date_time_format_",G="sun",H="format",I="thu",J="HH:mm:ss",K='stand-alone';qx.Class.define(D,{statics:{__yP:qx.locale.Manager.getInstance(),getAmMarker:function(L){return this.__yP.localize(B,[],L);}
,getPmMarker:function(M){return this.__yP.localize(t,[],M);}
...
;var Q=[G,E,C,y,I,A,o];var R=[];for(var i=0;i<Q.length;i++ ){var N=u+S+v+length+v+Q[i];R.push(P?this.__yQ(S,S===a?K:a,N,O):this.__yP.localize(N,[],O));}
;return R;}
...
;var W=[G,E,C,y,I,A,o];var T=u+U+v+length+v+W[Y];return X?this.__yQ(U,U===a?K:a,T,V):this.__yP.localize(T,[],V);}
...
;var bd=[];for(var i=0;i<12;i++ ){var ba=w+be+v+length+v+(i+1);bd.push(bc?this.__yQ(be,be===a?K:a,ba,bb):this.__yP.localize(ba,[],bb));}
;return bd;}
...
;var bf=w+bg+v+length+v+(bi+1);return bj?this.__yQ(bg,bg===a?K:a,bf,bh):this.__yP.localize(bf,[],bh);}
,getDateFormat:function(bm,bk){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bm,[b,z,x,s]);}
;var bl=g+bm;return this.__yP.localize(bl,[],bk);}
,getDateTimeFormat:function(br,bp,bn){var bq=F+br;var bo=this.__yP.localize(bq,[],bn);if(bo==bq){bo=bp;}
;return bo;}
,getTimeFormat:function(bv,bs){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bv,[b,z,x,s]);}
;var bu=n+bv;var bt=this.__yP.localize(bu,[],bs);if(bt!=bu){return bt;}
...
else {bK=this.__yP.getTerritory()||this.__yP.getLanguage();}
;return bK.toUpperCase();}
,__yQ:function(bN,bM,bQ,bP){var bO=this.__yP.localize(bQ,[],bP);if(bO==bQ){var bL=bQ.replace(d+bN+d,d+bM+d);return this.__yP.localize(bL,[],bP);}
else {return bO;}
;}
}});}
)();
...
(function(){var a="-",b="QQQ",c='W',d="qqqq",e="h",f="KK",g='L',h=" is not supported",m="Z",n="Date string '",o="GGGGG",p='abbreviated',q="Y+",r=". Wildcard ",s="eeeee",t="(\\d\\d?)",u="LL",v="abbreviated",w="EEEE",x="^",y=":",z='y',A="quoted_literal",B="weekOfYear",C='3rd quarter',D="SSS",E='a',F="day",G="(-*",H="qqq",I="(\\d\\d)",J="locale",K="K",L='q',M="Provided date is invalid",N="pm",O="Error parsing date '",P='Anno Domini',Q='G',R="a",S='2nd quarter',T="yyyy-MM-dd'T'HH:mm:ss",U="GMT",V="HH:mm:ss",W="dd",X="+",Y="qx.util.format.DateFormat",eo='weekDay',ek="am",ep="eee",el="quarter",em="ee",ej="q",en="QQQQ",eu="HHmmss",ev="ww",ew="H",ex='Y+',eq="wide",er='wide',es="(\\d\\d\\d?)",et="(GMT[\\+\\-]\\d\\d:\\d\\d)",eB="_applyLocale",ff="Q",eC="c",eD="(",ey="weekOfMonth",ez="ccc",gg="long",eA="Malformed date format: ",eE="EEEEE",eF='Y',eG="EE",eL="GGG",eM='-',eN="w",eH="mm",eI='h',eJ="S",eK='s',eS='A',eT="yyyy-MM-dd",eU="G",eV='z',eO="GG",eP=")",eQ="kk",eR="ss",fa="([\\+\\-]\\d\\d\\d\\d)",fb="default",gk='BC',fc='H',eW='S',eX='1st quarter',gi="stand-alone",eY="0",fd="MMMM",fe='c',fq="d",fp='Q',fo='Q1',fu="eeee",ft="QQ",fs="LLLLL",fr="HH",fj='k',fi="m",fh="DDD",fg="D",fn='D',fm="LLLL",fl="(\\d\\d*?)",fk='e',fB="': the value for day or month is too large",fA="literal",fz='K',fy="L",fF="hh",fE="ms",fD="(\\d\\d?\\d?)",fC="cccc",fx="e",fw='y+',fv="|",fQ="W",fP="dayOfYear",fO="GGGG",fU='Q2',fT='B',fS="MM",fR='Q4',fJ="y+",fI='narrow',fH="ccccc",fG="E",fN="wildcard",fM='',fL="' does not match the date format: ",fK="short",gb='d',ga="unkown",fY="narrow",fX="\\d?",gf="'",ge="k",gd='Before Christ',gc='Z',fW="qx.debug",fV='m',dR=" ",dQ="yyyy-MM-dd'T'HH:mm:ss'Z'",gl="String",dO='Q3',dP="z",dN="DD",gj="min",dL="",dM='M',dK="sec",gh='isoUtcDateTime',dI='4th quarter',dJ="(\\d?)",dH="SS",eb="MMM",ec="s",dY="M",ea='w',dW="EEE",dX="$",dV="?",dG='AD',dT="\\d",dU="+?",dS="hour",ei="MMMMM",eg='E',eh='month',ee="LLL",ef="qq",ed="format";qx.Class.define(Y,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(gn,gm){qx.core.Object.call(this);if(!gm){this.__uV=qx.locale.Manager.getInstance().getLocale();this.__yw=qx.locale.Manager.getInstance().bind(J,this,J);}
else {this.__uV=gm;this.setLocale(gm);}
;this.__yx=this.__uV;if(gn!=null){this.__iB=gn.toString();if(this.__iB in qx.util.format.DateFormat.ISO_MASKS){if(this.__iB===gh){this.__yy=true;}
;this.__iB=qx.util.format.DateFormat.ISO_MASKS[this.__iB];}
;}
else {this.__iB=qx.locale.Date.getDateFormat(gg,this.__uV)+dR+qx.locale.Date.getDateTimeFormat(eu,V,this.__uV);}
;}
,properties:{locale:{apply:eB,nullable:true,check:gl}},statics:{getDateInstance:function(){var go=qx.util.format.DateFormat;var gp=qx.locale.Date.getDateFormat(fK)+dL;if(go._dateInstance==null||go._dateInstance.__iB!=gp){go._dateInstance=new go(gp);}
;return go._dateInstance;}
,getDateTimeInstance:function(){var gq=qx.util.format.DateFormat;var gr=qx.locale.Date.getDateFormat(gg)+dR+qx.locale.Date.getDateTimeFormat(eu,V);if(gq._dateTimeInstance==null||gq._dateTimeInstance.__iB!=gr){gq._dateTimeInstance=new gq(gr);}
;return gq._dateTimeInstance;}
,ASSUME_YEAR_2000_THRESHOLD:30,ISO_MASKS:{isoDate:eT,isoTime:V,isoDateTime:T,isoUtcDateTime:dQ},AM_MARKER:ek,PM_MARKER:N},members:{__yw:null,__uV:null,__yx:null,__iB:null,__yz:null,__yA:null,__yB:null,__yy:null,__yC:function(gt,gs){var gu=dL+(gt<0?((-1)*gt):gt);while(gu.length<gs){gu=eY+gu;}
;return gt<0?a+gu:gu;}
,__yD:function(gv){var gw=new Date(gv.getTime());var gx=gw.getDate();while(gw.getMonth()!=0){gw.setDate(-1);gx+=gw.getDate()+1;}
;return gx;}
,__yE:function(gy){return new Date(gy.getTime()+(3-((gy.getDay()+6)%7))*86400000);}
,__yF:function(gz){var gB=this.__yE(gz);var gC=gB.getFullYear();var gA=this.__yE(new Date(gC,0,4));return Math.floor(1.5+(gB.getTime()-gA.getTime())/86400000/7);}
,__yG:function(gD){var gF=this.__yE(gD);var gE=this.__yE(new Date(gD.getFullYear(),gD.getMonth(),4));return Math.floor(1.5+(gF.getTime()-gE.getTime())/86400000/7);}
,__yH:function(gG){var gH=this.__yE(gG);return gH.getFullYear();}
,__yI:function(gJ){var gI=new Date(gJ,2,1);gI.setDate(-1);return gI.getDate()+1===29;}
,__yJ:function(gO,gP){var gN=0;var gQ=0;if(!gP){gP=1971;}
...
,__yK:function(gU,gS,gR){var gT;switch(gS){case 11:gT=gU-1;if(gU!=this.__yH(new Date(gT,gS,gR))){gT=gU;}
;break;case 0:gT=gU+1;if(gU!=this.__yH(new Date(gT,gS,gR))){gT=gU;}
;break;default:gT=gU;};return gT;}
,_applyLocale:function(gW,gV){this.__uV=gW===null?this.setLocale(this.__yx):gW;}
,format:function(hn){if(hn==null){return null;}
;if(isNaN(hn.getTime())){if(qx.core.Environment.get(fW)){qx.log.Logger.error(M);}
;return null;}
;if(this.__yy){hn=new Date(hn.getUTCFullYear(),hn.getUTCMonth(),hn.getUTCDate(),hn.getUTCHours(),hn.getUTCMinutes(),hn.getUTCSeconds(),hn.getUTCMilliseconds());}
;var ha=this.__uV;var hf=hn.getFullYear();var hb=hn.getMonth();var he=hn.getDate();var hi=hn.getDay();var hl=hn.getHours();var hs=hn.getMinutes();var hj=hn.getSeconds();var hq=hn.getMilliseconds();var hh=hn.getTimezoneOffset();var hm=hh>0?1:-1;var gX=Math.floor(Math.abs(hh)/60);var hp=Math.abs(hh)%60;this.__yL();var hr=dL;for(var i=0;i<this.__yB.length;i++ ){var hg=this.__yB[i];if(hg.type==fA){hr+=hg.text;}
else {var gY=hg.character;var hk=hg.size;var ht=dV;switch(gY){case z:if(hk==2){ht=this.__yC(hf%100,2);}
else {var hc=Math.abs(hf);ht=hc+dL;if(hk>ht.length){for(var j=ht.length;j<hk;j++ ){ht=eY+ht;}
;}
;if(hf<0){ht=a+ht;}
;}
;break;case eF:ht=this.__yH(hn)+dL;var hc=ht.replace(eM,fM);if(hk>ht.length){for(var j=hc.length;j<hk;j++ ){hc=eY+hc;}
;}
;ht=ht.indexOf(a)!=-1?a+hc:hc;break;case Q:if(hk>=1&&hk<=3){ht=hf>0?dG:gk;}
else if(hk==4){ht=hf>0?P:gd;}
else if(hk==5){ht=hf>0?eS:fT;}
;break;case fp:if(hk==1||hk==2){ht=this.__yC(parseInt(hb/4)+1,hk);}
;if(hk==3){ht=fp+(parseInt(hb/4)+1);}
;break;case L:if(hk==1||hk==2){ht=this.__yC(parseInt(hb/4)+1,hk);}
;if(hk==3){ht=fp+(parseInt(hb/4)+1);}
;break;case fn:ht=this.__yC(this.__yD(hn),hk);break;case gb:ht=this.__yC(he,hk);break;case ea:ht=this.__yC(this.__yF(hn),hk);break;case c:ht=this.__yG(hn);break;case eg:if(hk>=1&&hk<=3){ht=qx.locale.Date.getDayName(v,hi,ha,ed,true);}
else if(hk==4){ht=qx.locale.Date.getDayName(eq,hi,ha,ed,true);}
else if(hk==5){ht=qx.locale.Date.getDayName(fY,hi,ha,ed,true);}
;break;case fk:var hd=qx.locale.Date.getWeekStart(ha);var ho=1+((hi-hd>=0)?(hi-hd):7+(hi-hd));if(hk>=1&&hk<=2){ht=this.__yC(ho,hk);}
...
;break;case dM:if(hk==1||hk==2){ht=this.__yC(hb+1,hk);}
...
;break;case g:if(hk==1||hk==2){ht=this.__yC(hb+1,hk);}
...
;break;case E:ht=(hl<12)?qx.locale.Date.getAmMarker(ha):qx.locale.Date.getPmMarker(ha);break;case fc:ht=this.__yC(hl,hk);break;case fj:ht=this.__yC((hl==0)?24:hl,hk);break;case fz:ht=this.__yC(hl%12,hk);break;case eI:ht=this.__yC(((hl%12)==0)?12:(hl%12),hk);break;case fV:ht=this.__yC(hs,hk);break;case eK:ht=this.__yC(hj,hk);break;case eW:ht=this.__yC(hq,3);if(hk<ht.length){ht=ht.substr(0,hk);}
else {while(hk>ht.length){ht+=eY;}
;}
;break;case eV:if(hk>=1&&hk<=4){ht=U+((hm>0)?a:X)+this.__yC(Math.abs(gX),2)+y+this.__yC(hp,2);}
;break;case gc:if(hk>=1&&hk<=3){ht=((hm>0)?a:X)+this.__yC(Math.abs(gX),2)+this.__yC(hp,2);}
else {ht=U+((hm>0)?a:X)+this.__yC(Math.abs(gX),2)+y+this.__yC(hp,2);}
;break;};hr+=ht;}
;}
;return hr;}
,parse:function(hA){this.__yM();var hD=this.__yz.regex.exec(hA);if(hD==null){throw new Error(n+hA+fL+this.__iB);}
;var hu={era:1,year:1970,quarter:1,month:0,day:1,dayOfYear:1,hour:0,ispm:false,weekDay:4,weekYear:1970,weekOfMonth:1,weekOfYear:1,min:0,sec:0,ms:0};var hy=1;var hv=false;var hw=false;for(var i=0;i<this.__yz.usedRules.length;i++ ){var hx=this.__yz.usedRules[i];var hz=hD[hy];if(hx.field!=null){hu[hx.field]=parseInt(hz,10);}
else {hx.manipulator(hu,hz,hx.pattern);}
;if(hx.pattern==q){var hB=false;for(var k=0;k<this.__yz.usedRules.length;k++ ){if(this.__yz.usedRules[k].pattern==fw){hB=true;break;}
;}
;if(!hB){hv=true;}
;}
;if(hx.pattern.indexOf(fg)!=-1){var hF=false;for(var k=0;k<this.__yz.usedRules.length;k++ ){if(this.__yz.usedRules[k].pattern.indexOf(fq)!=-1){hF=true;break;}
;}
;if(!hF){hw=true;}
;}
;hy+=(hx.groups==null)?1:hx.groups;}
;if(hv){hu.year=this.__yK(hu.weekYear,hu.month,hu.day);}
;if(hw){var hE=this.__yJ(hu.dayOfYear,hu.year);hu.month=hE.month;hu.day=hE.day;}
;if(hu.era<0&&(hu.year*hu.era<0)){hu.year=hu.year*hu.era;}
;var hC=new Date(hu.year,hu.month,hu.day,(hu.ispm)?(hu.hour+12):hu.hour,hu.min,hu.sec,hu.ms);if(this.__yy){hC=new Date(hC.getUTCFullYear(),hC.getUTCMonth(),hC.getUTCDate(),hC.getUTCHours(),hC.getUTCMinutes(),hC.getUTCSeconds(),hC.getUTCMilliseconds());}
;if(hu.month!=hC.getMonth()||hu.year!=hC.getFullYear()){throw new Error(O+hA+fB);}
;return hC;}
,__yL:function(){if(this.__yB!=null){return;}
;this.__yB=[];var hK;var hI=0;var hM=dL;var hG=this.__iB;var hJ=fb;var i=0;while(i<hG.length){var hL=hG.charAt(i);switch(hJ){case A:if(hL==gf){if(i+1>=hG.length){i++ ;break;}
;var hH=hG.charAt(i+1);if(hH==gf){hM+=hL;i++ ;}
else {i++ ;hJ=ga;}
;}
else {hM+=hL;i++ ;}
;break;case fN:if(hL==hK){hI++ ;i++ ;}
else {this.__yB.push({type:fN,character:hK,size:hI});hK=null;hI=0;hJ=fb;}
;break;default:if((hL>=E&&hL<=eV)||(hL>=eS&&hL<=gc)){hK=hL;hJ=fN;}
else if(hL==gf){if(i+1>=hG.length){hM+=hL;i++ ;break;}
;var hH=hG.charAt(i+1);if(hH==gf){hM+=hL;i++ ;}
;i++ ;hJ=A;}
else {hJ=fb;}
;if(hJ!=fb){if(hM.length>0){this.__yB.push({type:fA,text:hM});hM=dL;}
;}
else {hM+=hL;i++ ;}
;break;};}
;if(hK!=null){this.__yB.push({type:fN,character:hK,size:hI});}
else if(hM.length>0){this.__yB.push({type:fA,text:hM});}
;}
,__yM:function(){if(this.__yz!=null){return;}
;var hQ=this.__iB;this.__yO();this.__yL();var hW=[];var hR=x;for(var hO=0;hO<this.__yB.length;hO++ ){var hX=this.__yB[hO];if(hX.type==fA){hR+=qx.lang.String.escapeRegexpChars(hX.text);}
else {var hP=hX.character;var hS=hX.size;var hU;for(var hY=0;hY<this.__yA.length;hY++ ){var hT=this.__yA[hY];if(this.__yN(hT,hP,hS)){hU=hT;break;}
;}
...
;this.__yz={regex:hN,"usedRules":hW,pattern:hR};}
,__yN:function(ib,ic,id){if(ic===z&&ib.pattern===fw){ib.regex=ib.regexFunc(id);return true;}
else if(ic===eF&&ib.pattern===ex){ib.regex=ib.regexFunc(id);return true;}
else {return ic==ib.pattern.charAt(0)&&id==ib.pattern.length;}
;}
,__yO:function(){var it=qx.util.format.DateFormat;var iw=qx.lang.String;if(this.__yA!=null){return;}
;var iu=this.__yA=[];var ip=qx.locale.Date.getAmMarker(this.__uV).toString()||it.AM_MARKER;var iD=qx.locale.Date.getPmMarker(this.__uV).toString()||it.PM_MARKER;var ig=this.__uV;var il=function(iG,iH){iH=parseInt(iH,10);if(iH>=0){if(iH<it.ASSUME_YEAR_2000_THRESHOLD){iH+=2000;}
...
},destruct:function(){if(this.__yw!=null){qx.locale.Manager.getInstance().removeBinding(this.__yw);}
;this.__yB=this.__yz=this.__yA=null;}
});}
)();
...
else {var bL=(bN==null)?-1:bN.getDate();for(var i=0;i<6*7;i++ ){var bO=this.__LU[i];if(bO.hasState(M)){if(bO.hasState(p)){bO.removeState(p);}
;}
...
;this.__LT=this.__LU=this.__LV=null;}
});}
)();
...
(function(){var a="changeModel",b="modelSelection[0]",c="qx.core.Object",d=":\n",e="qx.debug",f="_applyTarget",g="qx.data.controller.Form",h="changeTarget",j="value",k=" of ",l="Could not bind property ",m=".",n="No target is set.",o="qx.ui.form.Form",p="_applyModel";qx.Class.define(g,{extend:qx.core.Object,construct:function(q,s,r){qx.core.Object.call(this);this._selfUpdate=!!r;this.__qE={};if(q!=null){this.setModel(q);}
;if(s!=null){this.setTarget(s);}
;}
,properties:{model:{check:c,apply:p,event:a,nullable:true,dereference:true},target:{check:o,apply:f,event:h,nullable:true,init:null,dereference:true}},members:{__qF:null,__qE:null,addBindingOptions:function(name,v,w){this.__qE[name]=[v,w];if(this.getModel()==null||this.getTarget()==null){return;}
;var t=this.getTarget().getItems()[name];var u=this.__qI(t)?b:j;this.__qF.removeTarget(t,u,name);this.__qF.addTarget(t,u,name,!this._selfUpdate,v,w);}
...
;if(this.__qE[name]&&this.__qE[name][1]){B=this.__qE[name][1].converter(B);}
;E[F[i]]=B;}
else {if(!E[F[i]]){E[F[i]]={};}
;E=E[F[i]];}
;}
...
;var G=this.getTarget().getItems();for(var name in G){var J=G[name];var I=this.__qI(J)?b:j;var H=this.__qE[name];H=H&&this.__qE[name][1];qx.data.SingleValueBinding.updateTarget(J,I,this.getModel(),name,H);}
;}
,_applyTarget:function(L,K){if(K!=null){this.__qH(K);}
;if(this.getModel()==null){return;}
;if(L!=null){this.__qG();}
;}
,_applyModel:function(O,M){if(this.__qF!=null&&O==null){this.__qF.setModel(null);}
;if(this.__qF!=null&&this.getTarget()!=null){var N=this.getTarget().getItems();for(var name in N){var P=N[name];var Q=this.__qI(P)?b:j;this.__qF.removeTarget(P,Q,name);}
;}
;if(this.__qF!=null){this.__qF.setModel(O);}
;if(this.getTarget()==null){return;}
;if(O!=null){this.__qG();}
;}
,__qG:function(){if(this.__qF==null){this.__qF=new qx.data.controller.Object(this.getModel());}
;var U=this.getTarget().getItems();for(var name in U){var R=U[name];var S=this.__qI(R)?b:j;var T=this.__qE[name];try{if(T==null){this.__qF.addTarget(R,S,name,!this._selfUpdate);}
else {this.__qF.addTarget(R,S,name,!this._selfUpdate,T[0],T[1]);}
;}
catch(V){if(qx.core.Environment.get(e)){this.warn(l+name+k+this.getModel()+d+V.stack);}
;}
;}
;this.getTarget().redefineResetter();}
,__qH:function(W){if(this.__qF==null){return;}
;var ba=W.getItems();for(var name in ba){var Y=ba[name];var X=this.__qI(Y)?b:j;this.__qF.removeTarget(Y,X,name);}
;}
,__qI:function(bb){return qx.Class.hasInterface(bb.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(bb.constructor,qx.ui.form.IModelSelection);}
},destruct:function(){if(this.__qF){this.__qF.dispose();}
;}
});}
)();
