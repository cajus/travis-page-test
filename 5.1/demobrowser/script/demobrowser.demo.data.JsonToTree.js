...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.JsonToTree.3b5699455953.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="open",b="abstract",c="__bQ",d="qx.debug",e="'. It is not a child of this tree item.",f="addItem",g="visible",h="excluded",j="removeItem",k="Invalid child index: ",m="Cannot remove treeitem '",n="__eV",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:b,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__bQ=[];}
,properties:{parent:{check:o,nullable:true}},members:{__bQ:null,__eV:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
;}
;}
,getChildrenContainer:function(){if(!this.__eV){this.__eV=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?g:h});}
;return this.__eV;}
,hasChildrenContainer:function(){return this.__eV;}
,getParentChildrenContainer:function(){if(this.getParent()){return this.getParent().getChildrenContainer();}
else if(this.getLayoutParent()){return this.getLayoutParent();}
else {return null;}
;}
,getChildren:function(){return this.__bQ;}
,hasChildren:function(){return this.__bQ?this.__bQ.length>0:false;}
...
;}
,__zz:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);}
;}
...
;this.__bQ.push(Q);if(!N){this.__zz();}
;if(P){Q.recursiveAddToWidgetQueue();P.fireNonBubblingEvent(f,qx.event.type.Data,[Q]);}
;}
;if(P){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(X,T){if(qx.core.Environment.get(d)){this.assert(T<=this.__bQ.length&&T>=0,k+T);}
;if(T==this.__bQ.length){this.add(X);return;}
;var W=X.getParent();if(W){W.remove(X);}
;var U=this.getChildrenContainer();X.setParent(this);var V=this.hasChildren();var S=this.__bQ[T];U.addBefore(X,S);if(X.hasChildren()){U.addAfter(X.getChildrenContainer(),X);}
;qx.lang.Array.insertAt(this.__bQ,X,T);if(!V){this.__zz();}
;if(this.getTree()){X.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(Y,bb){if(qx.core.Environment.get(d)){this.assert(this.__bQ.indexOf(bb)>=0);}
;var ba=Y.getParent();if(ba){ba.remove(Y);}
;this.addAt(Y,this.__bQ.indexOf(bb));}
,addAfter:function(bd,bc){if(qx.core.Environment.get(d)){this.assert(this.__bQ.indexOf(bc)>=0);}
;var be=bd.getParent();if(be){be.remove(bd);}
;this.addAt(bd,this.__bQ.indexOf(bc)+1);}
,addAtBegin:function(bf){this.addAt(bf,0);}
,remove:function(bk){for(var i=0,l=arguments.length;i<l;i++ ){var bj=arguments[i];if(this.__bQ.indexOf(bj)==-1){this.warn(m+bj+e);return;}
...
;}
;qx.lang.Array.remove(this.__bQ,bj);bj.setParent(null);bg.remove(bj);}
;var bh=this.getTree();if(bh){bh.fireNonBubblingEvent(j,qx.event.type.Data,[bj]);}
;qx.ui.core.queue.Widget.add(this);}
,removeAt:function(bl){var bm=this.__bQ[bl];if(bm){this.remove(bm);}
;}
,removeAll:function(){var bn=this.__bQ.concat();for(var i=this.__bQ.length-1;i>=0;i-- ){this.remove(this.__bQ[i]);}
;return bn;}
},destruct:function(){this._disposeArray(c);this._disposeObjects(n);}
});}
)();
...
(function(){var a="Enter",b="_applyOpenMode",c="excluded",d="Space",f="Left",g="Boolean",h="changeOpenMode",j="_applyRootOpenClose",k="visible",l="changeSelection",m="qx.ui.tree.Tree",n="qx.event.type.Data",o="qx.ui.tree.core.AbstractTreeItem",p="tree",q="dbltap",r="_applyHideRoot",s="changeRoot",t="__eF",u="_applyRoot",v="keypress",w="none",x="pane",y="tap",z="Right";qx.Class.define(m,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__eF=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(x).add(this.__eF);this.initOpenMode();this.initRootOpenClose();this.addListener(l,this._onChangeSelection,this);this.addListener(v,this._onKeyPress,this);}
,events:{addItem:n,removeItem:n},properties:{openMode:{check:[y,q,w],init:q,apply:b,event:h,themeable:true},root:{check:o,init:null,nullable:true,event:s,apply:u},hideRoot:{check:g,init:false,apply:r},rootOpenClose:{check:g,init:false,apply:j},appearance:{refine:true,init:p},focusable:{refine:true,init:true}},members:{__eF:null,SELECTION_MANAGER:qx.ui.tree.selection.SelectionManager,getChildrenContainer:function(){return this.__eF;}
...
,_getContentPaddingTarget:function(){return this.__eF;}
...
(function(){var a="changeModel",b="_applyDelegate",c="qx.core.Object",d="_applyLabelPath",f="icon",g="change",h="label",k="' twice.",l="_applyTarget",m="changeTarget",n="_applyIconPath",o="Can not reverse bind the same target property '",p="_applyLabelOptions",q="_applyChildPath",r="_applyIconOptions",s="qx.data.controller.Tree",t="String",u="Can not bind the same target property '",v="_applyModel",w="get";qx.Class.define(s,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(y,z,A,x){qx.core.Object.call(this);this.__da={};this.__db=[];this.__dc={};if(A!=null){this.setChildPath(A);}
;if(x!=null){this.setLabelPath(x);}
;if(y!=null){this.setModel(y);}
;if(z!=null){this.setTarget(z);}
;}
,properties:{model:{check:c,apply:v,event:a,nullable:true,dereference:true},target:{apply:l,event:m,init:null,nullable:true,dereference:true},childPath:{check:t,apply:q,nullable:true},labelPath:{check:t,apply:d,nullable:true},iconPath:{check:t,apply:n,nullable:true},labelOptions:{apply:p,nullable:true},iconOptions:{apply:r,nullable:true},delegate:{apply:b,init:null,nullable:true}},members:{__dc:null,__da:null,__db:null,__dd:null,_applyDelegate:function(C,B){this._setConfigureItem(C,B);this._setCreateItem(C,B);this._setBindItem(C,B);}
,_applyIconOptions:function(E,D){this.__dl();}
,_applyLabelOptions:function(G,F){this.__dl();}
,_applyTarget:function(I,H){if(H!=undefined){this.__di(H);}
;if(this.getModel()!=null){this.__dg();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__dg();}
,_applyChildPath:function(M,L){this.__dd=L;this.__dg();this.__dd=null;}
,_applyIconPath:function(O,N){this.__dl();}
,_applyLabelPath:function(Q,P){this.__dg();}
,__de:function(R){var T=R.getTarget();var S=this.__dc[T.toHashCode()].treeNode;var U=this.__dc[T.toHashCode()].modelNode;this.__dh(S,U);this._updateSelection();}
,__df:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__dc[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__dc[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__df,this);var W=V.treeNode;this.__dc[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__dh(W,bd);this._updateSelection();}
...
;return be;}
,__dg:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__di();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__dm(this.getModel(),bg);this.__dh(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__dh:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__dc[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__de,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__df,this);this.__dc[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__dm(bj.getItem(i),bo);this.__dh(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__dk(bk,bn);}
;}
,__di:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__dj(bv);var bu=bv.getModel();if(bu){this.__dn(bu);}
;bv.destroy();this.__dc={};}
;}
,__dj:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__dj(bx[i]);}
;this.__dk(bx[i],bw);}
;}
,__dk:function(bz,bC){var bD=bz.getModel();var by=this.__dd||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__dc[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__dj(bz);delete this.__dc[bE.toHashCode()];}
;this.__dn(bD);bC.remove(bz);bz.destroy();}
,bindProperty:function(bG,bJ,bI,bF,bL){var bK=bL.bind(bG,bF,bJ,bI);if(this.__da[bJ]==null){this.__da[bJ]={};}
;var bH=this.__da[bJ];if(bH[bL.toHashCode()]){if(bH[bL.toHashCode()].id){throw new Error(u+bJ+k);}
;bH[bL.toHashCode()].id=bK;}
else {bH[bL.toHashCode()]={id:bK,reverseId:null,treeNode:bF};}
;if(!qx.lang.Array.contains(this.__db,bJ)){this.__db.push(bJ);}
;}
,bindPropertyReverse:function(bQ,bM,bO,bP,bS){var bR=bP.bind(bM,bS,bQ,bO);if(this.__da[bM]==null){this.__da[bM]={};}
;var bN=this.__da[bM];if(bN[bS.toHashCode()]){if(bN[bS.toHashCode()].reverseId){throw new Error(o+bQ+k);}
;bN[bS.toHashCode()].reverseId=bR;}
else {bN[bS.toHashCode()]={id:null,reverseId:bR,treeNode:bP};}
;if(!qx.lang.Array.contains(this.__db,bM)){this.__db.push(bM);}
;}
...
;}
,__dl:function(){var bW;for(var ca in this.__da){bW=ca;break;}
;for(var bX in this.__da[bW]){var bV=this.__da[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__dn(bY);this.__dm(bY,bV);}
;}
,__dm:function(cd,cb){var cc=this.getDelegate();if(cc!=null&&cc.bindItem!=null){cc.bindItem(this,cb,cd);}
else {this.bindDefaultProperties(cb,cd);}
;}
,__dn:function(cg){for(var i=0;i<this.__db.length;i++ ){var ce=this.__db[i];var cf=this.__da[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
;if(cf.reverseId){cf.treeNode.removeBinding(cf.reverseId);cf.reverseId=null;}
;delete this.__da[ce][cg.toHashCode()];}
;}
;}
...
;this._startSelectionModification();this.__di();this.__dg();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__dg();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__da=this.__dc=this.__db=null;}
});}
)();
