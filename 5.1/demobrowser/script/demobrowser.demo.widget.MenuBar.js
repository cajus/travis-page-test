...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.MenuBar.135cd22f9e21.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="-",c="PageUp",d="Escape",f="Boolean",g="+",h="qx.event.type.Data",j="_applyShortcut",k="PrintScreen",l="NumLock",m="short",n="keydown",o="5",p="8",q="execute",r="Meta",s="0",t="",u="2",v="Control",w="Shift",x="You can only specify one non modifier key!",y="3",z="Unidentified",A="/",B="Delete",C="String",D="changeEnabled",E="*",F="qx.bom.Shortcut",G="6",H="4",I="1",J="Alt",K="Not a valid key name for a shortcut: ",L="PageDown",M="Whitespaces are not allowed within shortcuts",N="_applyEnabled",O="keypress",P="7",Q="a",R="z",S="9";qx.Class.define(F,{extend:qx.core.Object,construct:function(T){qx.core.Object.call(this);this.__MK={};this.__ML=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":h},properties:{enabled:{init:true,check:f,event:D,apply:N},shortcut:{check:C,apply:j,nullable:true},autoRepeat:{check:f,init:false}},members:{__MK:t,__ML:t,execute:function(U){this.fireDataEvent(q,U);}
,__MM:function(event){if(this.getEnabled()&&this.__MN(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__Et:function(event){if(this.getEnabled()&&this.__MN(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__MM,this);qx.event.Registration.addListener(document.documentElement,O,this.__Et,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__MM,this);qx.event.Registration.removeListener(document.documentElement,O,this.__Et,this);}
;}
,_applyShortcut:function(bb,Y){if(bb){if(bb.search(/[\s]+/)!=-1){var bc=M;this.error(bc);throw new Error(bc);}
;this.__MK={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__ML=null;var X;var a=[];while(bb.length>0&&X!=-1){X=bb.search(/[-+]+/);a.push((bb.length==1||X==-1)?bb:bb.substring(0,X));bb=bb.substring(X+1);}
;var ba=a.length;for(var i=0;i<ba;i++ ){var bd=this.__MP(a[i]);switch(bd){case v:case w:case r:case J:this.__MK[bd]=true;break;case z:var bc=K+a[i];this.error(bc);throw bc;default:if(this.__ML){var bc=x;this.error(bc);throw bc;}
;this.__ML=bd;};}
;}
;return true;}
,__MN:function(e){var be=this.__ML;if(!be){return false;}
;if((!this.__MK.Shift&&e.isShiftPressed())||(this.__MK.Shift&&!e.isShiftPressed())||(!this.__MK.Control&&e.isCtrlPressed())||(this.__MK.Control&&!e.isCtrlPressed())||(!this.__MK.Meta&&e.isMetaPressed())||(this.__MK.Meta&&!e.isMetaPressed())||(!this.__MK.Alt&&e.isAltPressed())||(this.__MK.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__MO:{esc:d,ctrl:v,print:k,del:B,pageup:c,pagedown:L,numlock:l,numpad_0:s,numpad_1:I,numpad_2:u,numpad_3:y,numpad_4:H,numpad_5:o,numpad_6:G,numpad_7:P,numpad_8:p,numpad_9:S,numpad_divide:A,numpad_multiply:E,numpad_minus:b,numpad_plus:g},__MP:function(bg){var bf=qx.event.util.Keyboard;var bh=z;if(bf.isValidKeyIdentifier(bg)){return bg;}
;if(bg.length==1&&bg>=Q&&bg<=R){return bg.toUpperCase();}
;bg=bg.toLowerCase();var bh=this.__MO[bg]||qx.lang.String.firstUp(bg);if(bf.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bj=this.__ML;var bk=[];for(var bi in this.__MK){if(this.__MK[bi]){bk.push(qx.locale.Key.getKeyName(m,bi));}
;}
;if(bj){bk.push(qx.locale.Key.getKeyName(m,bj));}
;return bk.join(g);}
},destruct:function(){this.setEnabled(false);this.__MK=this.__ML=null;}
});}
)();
...
;var P=K.getMarginLeft()+K.getMarginRight();P=Math.max(P,this.getSpacing());var J=K.getSizeHint().width+P;this.__dW(K);G-=J;if(E&&E.getVisibility()!=o){E.setVisibility(o);G+=L;var H=E.getMarginLeft()+E.getMarginRight();G+=Math.max(H,this.getSpacing());}
;}
while(G>F);}
else if(this.__dT.length>0){do {var Q=this.__dT[0];if(Q){var P=Q.getMarginLeft()+Q.getMarginRight();P=Math.max(P,this.getSpacing());if(Q.getContentElement().getDomElement()==null){Q.syncAppearance();Q.invalidateLayoutCache();}
;var I=Q.getSizeHint().width;var O=false;if(this.__dT.length==1&&L>0){var N=P-this.getSpacing();var M=G-L+I+N;O=F>M;}
;if(F>G+I+P||O){this.__dV(Q);G+=I;if(E&&this.__dT.length==0){E.setVisibility(y);}
;}
else {return;}
;}
;}
while(F>=G&&this.__dT.length>0);}
;}
,__dV:function(R){R.setVisibility(o);this.__dT.shift();this.fireDataEvent(m,R);}
,__dW:function(S){if(!S){return;}
;this.__dT.unshift(S);S.setVisibility(y);this.fireDataEvent(A,S);}
,_getNextToHide:function(){for(var i=this.__dU.length-1;i>=0;i-- ){var T=this.__dU[i];if(T&&T.getVisibility&&T.getVisibility()==o){return T;}
;}
...
;}
;}
,setRemovePriority:function(X,W,Y){if(!Y&&this.__dU[W]!=undefined){throw new Error(v);}
;this.__dU[W]=X;}
...
;for(var i=0;i<this.__dT.length;i++ ){this.__dT[i].setVisibility(o);}
;this.__dT=[];}
;}
...
;bf.setVisibility(y);}
;}
,__dX:false,_setAllowMenuOpenHover:function(bg){this.__dX=bg;}
,_isAllowMenuOpenHover:function(){return this.__dX;}
...
(function(){var a="blur",b="Enter",c="__S",d="Escape",f="Space",g="keydown",h="Left",j="Down",k="Up",l="singleton",m="qx.debug",n="interval",o="roll",p="event.touch",q="__U",r="Object is no menu: ",s="pointerdown",t="qx.ui.menu.Manager",u="keyup",v="keypress",w="contextmenu",x="Right",y="__T";qx.Class.define(t,{type:l,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__S=[];var z=document.body;var A=qx.event.Registration;A.addListener(window.document.documentElement,s,this._onPointerDown,this,true);A.addListener(z,o,this._onRoll,this,true);A.addListener(z,g,this._onKeyUpDown,this,true);A.addListener(z,u,this._onKeyUpDown,this,true);A.addListener(z,v,this._onKeyPress,this,true);if(!qx.core.Environment.get(p)){qx.bom.Element.addListener(window,a,this.hideAll,this);}
;this.__T=new qx.event.Timer();this.__T.addListener(n,this._onOpenInterval,this);this.__U=new qx.event.Timer();this.__U.addListener(n,this._onCloseInterval,this);}
,members:{__V:null,__W:null,__T:null,__U:null,__S:null,_getChild:function(B,D,C,G){var F=B.getChildren();var length=F.length;var E;for(var i=D;i<length&&i>=0;i+=C){E=F[i];if(E.isEnabled()&&!E.isAnonymous()&&E.isVisible()){return E;}
;}
...
;H=H.getLayoutParent();}
;return false;}
,_isMenuOpener:function(J){var I=this.__S;for(var i=0;i<I.length;i++ ){if(I[i].getOpener()===J){return true;}
;}
;return false;}
...
;}
;var M=this.__S;M.push(L);L.setZIndex(1e6+M.length);}
,remove:function(N){if(qx.core.Environment.get(m)){if(!(N instanceof qx.ui.menu.Menu)){throw new Error(r+N);}
;}
;var O=this.__S;if(O){qx.lang.Array.remove(O,N);}
;}
,hideAll:function(){var P=this.__S;if(P){for(var i=P.length-1;i>=0;i-- ){P[i].exclude();}
;}
;}
,getActiveMenu:function(){var Q=this.__S;return Q.length>0?Q[Q.length-1]:null;}
,scheduleOpen:function(R){this.cancelClose(R);if(R.isVisible()){if(this.__V){this.cancelOpen(this.__V);}
;}
else if(this.__V!=R){this.__V=R;this.__T.restartWith(R.getOpenInterval());}
;}
,scheduleClose:function(S){this.cancelOpen(S);if(!S.isVisible()){if(this.__W){this.cancelClose(this.__W);}
;}
else if(this.__W!=S){this.__W=S;this.__U.restartWith(S.getCloseInterval());}
;}
,cancelOpen:function(T){if(this.__V==T){this.__T.stop();this.__V=null;}
;}
,cancelClose:function(U){if(this.__W==U){this.__U.stop();this.__W=null;}
;}
,_onOpenInterval:function(e){this.__T.stop();this.__V.open();this.__V=null;}
,_onCloseInterval:function(e){this.__U.stop();this.__W.exclude();this.__W=null;}
,preventContextMenuOnce:function(){qx.event.Registration.addListener(document.body,w,this.__X,this,true);}
,__X:function(e){var V=e.getTarget();V=qx.ui.core.Widget.getWidgetByElement(V,true);if(this._isInMenu(V)){e.stopPropagation();e.preventDefault();}
;qx.event.Registration.removeListener(document.body,w,this.__X,this,true);}
...
;if(this.__S.length>0&&!this._isInMenu(W)){this.hideAll();}
;}
,__Y:{"Enter":1,"Space":1},__ba:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var X=this.getActiveMenu();if(!X){return;}
;var Y=e.getKeyIdentifier();if(this.__ba[Y]||(this.__Y[Y]&&X.getSelectedButton())){e.stopPropagation();}
;}
,_onKeyPress:function(e){var bc=this.getActiveMenu();if(!bc){return;}
;var ba=e.getKeyIdentifier();var be=this.__ba[ba];var bb=this.__Y[ba];if(be){switch(ba){case k:this._onKeyPressUp(bc);break;case j:this._onKeyPressDown(bc);break;case h:this._onKeyPressLeft(bc);break;case x:this._onKeyPressRight(bc);break;case d:this.hideAll();break;};e.stopPropagation();e.preventDefault();}
...
;this.__cl();}
,__cl:function(){if(this.isVisible()){if(this.getBlockBackground()){var V=this.getZIndex();this._blocker.blockContent(V-1);}
;}
...
,__cm:function(){var bn=this._getMenuBounds();if(!bn){this.addListenerOnce(n,this.__cm,this);return;}
...
;this.__ck=bp;qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(bq){this.getChildControl(h);if(this.__ck){this.__ck.call(this);delete this.__ck;}
;}
...
;this.__cm();}
;}
,_onPointerOver:function(e){var bt=qx.ui.menu.Manager.getInstance();bt.cancelClose(this);var bu=e.getTarget();if(bu.isEnabled()&&bu instanceof qx.ui.menu.AbstractButton){this.setSelectedButton(bu);var bs=bu.getMenu&&bu.getMenu();if(bs){bs.setOpener(bu);bt.scheduleOpen(bs);this.__cj=bs;}
else {var bv=this.getOpenedButton();if(bv){bt.scheduleClose(bv.getMenu());}
;if(this.__cj){bt.cancelOpen(this.__cj);this.__cj=null;}
;}
;}
...
;if(this.__cj){bw.cancelOpen(this.__cj);}
;}
;}
...
(function(){var a="qx.ui.menu.Layout",b="Integer",c="_applyLayoutChange";qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:b,init:0,apply:c},spanColumn:{check:b,init:1,nullable:true,apply:c},iconColumnWidth:{check:b,init:0,themeable:true,apply:c},arrowColumnWidth:{check:b,init:0,themeable:true,apply:c}},members:{__cn:null,_computeSizeHint:function(){var q=this._getLayoutChildren();var o,g,k;var e=this.getSpanColumn();var j=this.__cn=[0,0,0,0];var n=this.getColumnSpacing();var m=0;var f=0;for(var i=0,l=q.length;i<l;i++ ){o=q[i];if(o.isAnonymous()){continue;}
...
,getColumnSizes:function(){return this.__cn||null;}
...
;var x=this.__cH(y[0]);var w=x.getColumnSizes();var s=x.getSpacingX();var z=qx.lang.Array.sum(w)+s*(w.length-1);if(z<o){w[1]+=o-z;}
...
;}
;}
,__cH:function(B){while(!(B instanceof qx.ui.menu.Menu)){B=B.getLayoutParent();}
;return B;}
...
(function(){var a="hovered",b="hover-button",c="pointerover",d="interval",f="__do",g="qx.ui.form.HoverButton",h="Integer",i="pointerout";qx.Class.define(g,{extend:qx.ui.basic.Atom,include:[qx.ui.core.MExecutable],implement:[qx.ui.form.IExecutable],construct:function(j,k){qx.ui.basic.Atom.call(this,j,k);this.addListener(c,this._onPointerOver,this);this.addListener(i,this._onPointerOut,this);this.__do=new qx.event.AcceleratingTimer();this.__do.addListener(d,this._onInterval,this);}
,properties:{appearance:{refine:true,init:b},interval:{check:h,init:80},firstInterval:{check:h,init:200},minTimer:{check:h,init:20},timerDecrease:{check:h,init:2}},members:{__do:null,_onPointerOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;this.__do.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.addState(a);}
,_onPointerOut:function(e){this.__do.stop();this.removeState(a);if(!this.isEnabled()||e.getTarget()!==this){return;}
;}
...
,__eq:function(){var y=this.getChildrenContainer().getChildren();y=y.filter(function(z){return z.getVisibility()==k;}
