...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Columns.00c40b6f7937.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,openNodeAndParents:function(bk){this.__jD(this.getModel(),bk);this.buildLookupTable();}
,closeNode:function(bl){if(qx.lang.Array.contains(this.__jw,bl)){qx.lang.Array.remove(this.__jw,bl);this.fireDataEvent(G,bl);this.buildLookupTable();}
;}
...
,isNodeOpen:function(bo){return qx.lang.Array.contains(this.__jw,bo);}
,_init:function(){this.__dY=new qx.data.Array();this.__jw=[];this.__jx=[];this._initLayer();}
...
,this);}
,getLookupTable:function(){return this.__dY;}
,_reverseLookup:function(bp){return bp;}
,_getDataFromRow:function(bq){return this.__dY.getItem(bq);}
,_getSelectables:function(){return this.__dY;}
,getOpenNodes:function(){return this.__jw;}
,isNode:function(br){return qx.ui.tree.core.Util.isNode(br,this.getChildProperty());}
,getLevel:function(bs){return this.__jx[bs];}
...
,_applyModel:function(bQ,bP){this.__jw=[];if(bQ!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bQ.constructor,qx.data.marshal.MEventBubbling)){this.warn(H+h+k);}
;}
;bQ.addListener(J,this._onChangeBubble,this);this.__jC(bQ);}
...
;}
;if(this.__dY.indexOf(bV)!=-1){this.__jA();}
;}
;}
,_onUpdated:function(event){if(this.__jy==null){this.__jy=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__jy.schedule();}
,_onOpen:function(event){var ca=event.getRow();var bY=this.__dY.getItem(ca);if(this.isNode(bY)){if(this.isNodeOpen(bY)){this.closeNode(bY);}
...
;break;};}
;}
,_beforeApplySelection:function(cg){if(cg.length===0&&this.getSelectionMode()===g){var cf=this.__jG();var ch=this.getLookupTable().indexOf(cf);if(ch>=0){cg.push(ch);}
;}
;}
,_afterApplySelection:function(){var ci=this.getSelection();if(ci.getLength()>0&&this.getSelectionMode()===g){this.__jF(ci.getItem(0));}
else {this.__jz=[];}
;}
,__jA:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this._itemWidth=0;var cm=[];this.__jx=[];var ck=-1;var cl=this.getModel();if(cl!=null){if(!this.isHideRoot()){ck++ ;cm.push(cl);this.__jx.push(ck);}
;if(this.isNodeOpen(cl)){var cj=this.__jB(cl,ck);cm=cm.concat(cj);}
;}
;if(!qx.lang.Array.equals(this.__dY.toArray(),cm)){this._provider.removeBindings();this.__dY.removeAll();this.__dY.append(cm);this.__jE();this._updateSelection();}
;}
,__jB:function(cn,cs){var cr=[];cs++ ;if(!this.isNode(cn)){return cr;}
...
;if(this.isNode(cu)){this.__jx.push(cs);cr.push(cu);if(this.isNodeOpen(cu)){var co=this.__jB(cu,cs);cr=cr.concat(co);}
;}
else {if(this.isShowLeafs()){this.__jx.push(cs);cr.push(cu);}
;}
;}
;cv.dispose();return cr;}
,__jC:function(cw){if(!qx.lang.Array.contains(this.__jw,cw)){this.__jw.push(cw);this.fireDataEvent(t,cw);}
;}
,__jD:function(cx,cy){if(cx===cy){this.__jC(cy);return true;}
;if(!this.isNode(cx)){return false;}
;var cB=cx.get(this.getChildProperty());if(cB==null){return false;}
;for(var i=0;i<cB.getLength();i++ ){var cA=cB.getItem(i);var cz=this.__jD(cA,cy);if(cz===true){this.__jC(cA);return true;}
;}
;return false;}
,__jE:function(){this.getPane().getRowConfig().setItemCount(this.__dY.getLength());this.getPane().fullUpdate();}
,getParent:function(cD){var cE=this.__dY.indexOf(cD);if(cE<0){return null;}
;var cF=this.__jx[cE];while(cE>0){cE-- ;var cC=this.__jx[cE];if(cC<cF){return this.__dY.getItem(cE);}
;}
;return null;}
,__jF:function(cG){this.__jz=[];var parent=this.getParent(cG);while(parent!=null){this.__jz.unshift(parent);parent=this.getParent(parent);}
;}
,__jG:function(){if(this.__jz==null){return this.getModel();}
;var cH=this.getLookupTable();var parent=this.__jz.pop();while(parent!=null){if(cH.contains(parent)){return parent;}
;parent=this.__jz.pop();}
...
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__jy!=null){this.__jy.cancel();this.__jy.dispose();}
;var cI=this.getModel();if(cI!=null){cI.removeListener(J,this._onChangeBubble,this);}
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__dY.dispose();this._layer=this._provider=this.__dY=this.__jw=this.__jy=null;}
});}
)();
...
(function(){var a="BindingIds",b="model",c="",d="changeDelegate",f="virtual-tree-file",g="label",h="appearance",i="icon",j="bindItem",k=".length",l="]",m="virtual-tree-folder",n="[",o=".",p="String",q="qx.ui.tree.core.MWidgetController";qx.Mixin.define(q,{construct:function(){this.__jR=[];}
,properties:{labelPath:{check:p,nullable:true},iconPath:{check:p,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},childProperty:{check:p,nullable:true},delegate:{event:d,init:null,nullable:true}},members:{__jR:null,bindDefaultProperties:function(u,s){this.bindProperty(c,b,null,u,s);this.bindProperty(this.getLabelPath(),g,this.getLabelOptions(),u,s);var r=this.__jS(s);var t=this._tree.getLookupTable();t=qx.data.SingleValueBinding.resolvePropertyChain(t,r);if(qx.util.OOUtil.hasProperty(t.constructor,this.getChildProperty())){this.bindProperty(this.getChildProperty()+k,h,{converter:function(){return m;}
},u,s);}
else {u.setAppearance(f);}
;if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),i,this.getIconOptions(),u,s);}
;}
,bindProperty:function(y,B,z,x,v){var A=this.__jS(v,y);var w=this._tree.getLookupTable();var C=w.bind(A,x,B,z);this.__dm(x,C);}
,bindPropertyReverse:function(J,F,H,I,D){var G=this.__jS(D,J);var E=this._tree.getLookupTable();var K=I.bind(F,E,G,H);this.__dm(I,K);}
,removeBindings:function(){while(this.__jR.length>0){var L=this.__jR.pop();this._removeBindingsFrom(L);}
;}
...
;}
,_removeBindingsFrom:function(P){var Q=this.__jT(P);while(Q.length>0){var R=Q.pop();try{this._tree.getLookupTable().removeBinding(R);}
catch(e){P.removeBinding(R);}
;}
;if(qx.lang.Array.contains(this.__jR,P)){qx.lang.Array.remove(this.__jR,P);}
;}
,__jS:function(T,U){var S=n+T+l;if(U!=null&&U!=c){S+=o+U;}
;return S;}
,__dm:function(W,X){var V=this.__jT(W);if(!qx.lang.Array.contains(V,X)){V.push(X);}
;if(!qx.lang.Array.contains(this.__jR,W)){this.__jR.push(W);}
;}
,__jT:function(Y){var ba=Y.getUserData(a);if(ba==null){ba=[];Y.setUserData(a,ba);}
;return ba;}
...
;var o=this._renderer.getCellWidget();o.setOpen(p&&this._tree.isNodeOpen(s));o.addListener(a,this.__jU,this);o.setUserData(b,this.getChildProperty());o.setUserData(g,this._tree.isShowLeafs());if(this._tree.getSelection().contains(s)){this._styleSelectabled(o);}
...
,poolCellWidget:function(u){u.removeListener(a,this.__jU,this);this._removeBindingsFrom(u);this._renderer.pool(u);this._onPool(u);}
...
,__jU:function(event){var K=event.getTarget();var L=K.getUserData(k);var J=this._tree.getLookupTable().getItem(L);if(event.getData()){this._tree.openNodeWithoutScrolling(J);}
...
,addSpacer:function(){if(!this.__KY){this.__KY=new qx.ui.core.Spacer();}
else {this._remove(this.__KY);}
;this._add(this.__KY);}
,addOpenButton:function(){this._add(this.getChildControl(l));}
,_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());}
;}
,addIcon:function(){var L=this.getChildControl(s);if(this.__KX){this._remove(L);}
;this._add(L);this.__KX=true;}
,addLabel:function(M){var N=this.getChildControl(D);if(this.__KW){this._remove(N);}
;if(M){this.setLabel(M);}
else {N.setValue(this.getLabel());}
;this._add(N);this.__KW=true;}
,_applyIcon:function(P,O){if(!this.__Lb()){this.__Lc(P);}
else if(!this.isOpen()){this.__Lc(P);}
;}
,_applyIconOpened:function(R,Q){if(this.isOpen()){if(this.__La()&&this.__Lb()){this.__Lc(R);}
else if(!this.__La()&&this.__Lb()){this.__Lc(R);}
;}
;}
...
;var X;if(W){X=this.__Lb()?this.getIconOpened():null;}
else {X=this.getIcon();}
;if(X){this.__Lc(X);}
;W?this.addState(t):this.removeState(t);}
,__La:function(){return qx.util.PropertyUtil.getUserValue(this,s);}
,__Lb:function(){return qx.util.PropertyUtil.getUserValue(this,u);}
,__Lc:function(Y){var ba=this.getChildControl(s,true);if(ba){ba.setSource(Y);}
;}
...
;}
;if(this.__KY){this.__KY.setWidth((this.getLevel()+1)*this.getIndent()-bf);}
;}
...
(function(){var a="source",b="changeLeadIcon",c="changeSize",d="changeMode",e="changeDate",f="checked",g="changeChecked",h="icon/16/places/user-desktop.png",i="value",j="size",k="leadIcon",l="date",m="mode",n="demobrowser.demo.virtual.tree.TreeItem",o="String",p="Boolean";qx.Class.define(n,{extend:qx.ui.tree.VirtualTreeItem,properties:{leadIcon:{check:o,event:b,nullable:true},checked:{check:p,event:g,nullable:true},size:{check:o,event:c,nullable:true},date:{check:o,event:e,nullable:true},mode:{check:o,event:d,nullable:true}},members:{__Op:null,__Oq:null,__Or:null,__Os:null,__uk:null,_addWidgets:function(){var q=this.__Op=new qx.ui.basic.Image();this.bind(k,q,a);q.setWidth(16);this.addWidget(q);this.addSpacer();this.addOpenButton();this.addIcon();this.setIcon(h);var r=this.__Oq=new qx.ui.form.CheckBox();this.bind(f,r,i);r.bind(i,this,f);r.setFocusable(false);r.setTriState(true);r.setMarginRight(4);this.addWidget(r);this.addLabel();this.addWidget(new qx.ui.core.Spacer(),{flex:1});var s=this.__Or=new qx.ui.basic.Label();this.bind(j,s,i);s.setWidth(50);this.addWidget(s);s=this.__Os=new qx.ui.basic.Label();this.bind(l,s,i);s.setWidth(150);this.addWidget(s);s=this.__uk=new qx.ui.basic.Label();this.bind(m,s,i);s.setWidth(80);this.addWidget(s);}
}});}
)();
