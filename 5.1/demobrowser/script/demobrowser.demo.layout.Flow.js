...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow.5f03c769108c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var x=new qx.ui.layout.LineSizeIterator(D,this.getSpacingX());var B=z.top;while(x.hasMoreLines()){var C=x.computeNextLine(A);this.__qC(C,B,A,z);B+=C.height+this.getSpacingY();}
;}
,__qC:function(L,K,I,E){var H=qx.ui.layout.Util;var F=E.left;if(this.getAlignX()!=l){F=E.left+I-L.width;if(this.getAlignX()==j){F=E.left+Math.round((I-L.width)/2);}
;}
...
;}
,_computeSizeHint:function(){return this.__qD(Infinity);}
,hasHeightForWidth:function(){return true;}
,getHeightForWidth:function(P){return this.__qD(P).height;}
...
;return S;}
,__qD:function(T){var U=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var V=0;var W=0;var X=0;while(U.hasMoreLines()){var Y=U.computeNextLine(T);X+=1;W=Math.max(W,Y.width);V+=Y.height;}
;return {width:W,height:V+this.getSpacingY()*(X-1)};}
}});}
)();
(function(){var a="No more lines to compute",b="qx.ui.layout.LineSizeIterator";qx.Class.define(b,{extend:Object,construct:function(d,c){this.__bQ=d;this.__mu=c;this.__mv=d.length>0;this.__mw=0;}
,members:{__bQ:null,__mu:null,__mv:null,__mw:null,computeNextLine:function(k){var k=k||Infinity;if(!this.__mv){throw new Error(a);}
;var p=this.__bQ;var n=0;var f=0;var o=[];var j=[];for(var i=this.__mw;i<p.length;i++ ){var m=p[i];var q=m.getSizeHint();var g=this.__mx(i);var h=q.width+g;var e=i==this.__mw;if(!e&&f+h>k){this.__mw=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);j.push(g);f+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__mw=i+1;break;}
;}
;if(i>=p.length){this.__mv=false;}
;return {height:n,width:f,children:o,gapsBefore:j};}
,__mx:function(r){var s=r==this.__mw;if(s){return this.__bQ[r].getMarginLeft();}
else {return Math.max(this.__bQ[r-1].getMarginRight(),this.__bQ[r].getMarginLeft(),this.__mu);}
;}
,hasMoreLines:function(){return this.__mv;}
}});}
)();
