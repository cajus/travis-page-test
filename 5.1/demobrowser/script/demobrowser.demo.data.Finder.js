...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Finder.88de00bef509.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="indexOf",b="addAfter",c="add",d="addBefore",e="_",f="addAt",g="hasChildren",h="removeAt",i="removeAll",j="getChildren",k="remove",l="qx.ui.core.MRemoteChildrenHandling";qx.Mixin.define(l,{members:{__wB:function(q,m,o,n){var p=this.getChildrenContainer();if(p===this){q=e+q;}
;return (p[q])(m,o,n);}
,getChildren:function(){return this.__wB(j);}
,hasChildren:function(){return this.__wB(g);}
,add:function(s,r){return this.__wB(c,s,r);}
,remove:function(t){return this.__wB(k,t);}
,removeAll:function(){return this.__wB(i);}
,indexOf:function(u){return this.__wB(a,u);}
,addAt:function(x,v,w){this.__wB(f,x,v,w);}
,addBefore:function(y,A,z){this.__wB(d,y,A,z);}
,addAfter:function(D,B,C){this.__wB(b,D,B,C);}
,removeAt:function(E){return this.__wB(h,E);}
}});}
)();
...
(function(){var a="qx.ui.window.Manager",b="__lc";qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__lc:null,setDesktop:function(c){this.__lc=c;this.updateStack();}
,getDesktop:function(){return this.__lc;}
,changeActiveWindow:function(d,e){if(d){this.bringToFront(d);d.setActive(true);}
;if(e){e.resetActive();}
;}
,_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(){this.__lc.forceUnblock();var h=this.__lc.getWindows();var g=this._minZIndex;var m=g+h.length*2;var j=g+h.length*4;var k=null;for(var i=0,l=h.length;i<l;i++ ){var f=h[i];if(!f.isVisible()){continue;}
;k=k||f;if(f.isModal()){f.setZIndex(j);this.__lc.blockContent(j-1);j+=2;k=f;}
else if(f.isAlwaysOnTop()){f.setZIndex(m);m+=2;}
else {f.setZIndex(g);g+=2;}
;if(!k.isModal()&&f.isActive()||f.getZIndex()>k.getZIndex()){k=f;}
;}
;this.__lc.setActiveWindow(k);}
,bringToFront:function(o){var n=this.__lc.getWindows();var p=qx.lang.Array.remove(n,o);if(p){n.push(o);this.updateStack();}
;}
,sendToBack:function(r){var q=this.__lc.getWindows();var s=qx.lang.Array.remove(q,r);if(s){q.unshift(r);this.updateStack();}
;}
},destruct:function(){this._disposeObjects(b);}
});}
)();
(function(){var a="pointerup",b="Boolean",c="losecapture",d="move",f="qx.ui.core.MMovable",g="__At",h="__As",i="The move handle could not be redefined!",j="pointerdown",k="maximized",l="pointermove",m="roll",n="move-frame";qx.Mixin.define(f,{properties:{movable:{check:b,init:true},useMoveFrame:{check:b,init:false}},members:{__As:null,__At:null,__Au:null,__Av:null,__Aw:null,__Ax:null,__Ay:null,__Az:false,__AA:null,__AB:0,_activateMoveHandle:function(o){if(this.__As){throw new Error(i);}
;this.__As=o;o.addListener(j,this._onMovePointerDown,this);o.addListener(a,this._onMovePointerUp,this);o.addListener(l,this._onMovePointerMove,this);o.addListener(c,this.__AF,this);}
,__AC:function(){var p=this.__At;if(!p){p=this.__At=new qx.ui.core.Widget();p.setAppearance(n);p.exclude();qx.core.Init.getApplication().getRoot().add(p);}
;return p;}
,__AD:function(){var location=this.getContentLocation();var r=this.getBounds();var q=this.__AC();q.setUserBounds(location.left,location.top,r.width,r.height);q.show();q.setZIndex(this.getZIndex()+1);}
,__AE:function(e){var t=this.__Au;var s=Math.max(t.left,Math.min(t.right,e.getDocumentLeft()));var v=Math.max(t.top,Math.min(t.bottom,e.getDocumentTop()));var u=this.__Av+s;var w=this.__Aw+v;return {viewportLeft:parseInt(u,10),viewportTop:parseInt(w,10),parentLeft:parseInt(u-this.__Ax,10),parentTop:parseInt(w-this.__Ay,10)};}
,_onMoveRoll:function(e){e.stop();}
,_onMovePointerDown:function(e){if(!this.getMovable()||this.hasState(k)){return;}
;this.addListener(m,this._onMoveRoll,this);var parent=this.getLayoutParent();var x=parent.getContentLocation();var z=parent.getBounds();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isBlocked()){this.__AA=parent.getBlockerColor();this.__AB=parent.getBlockerOpacity();parent.setBlockerColor(null);parent.setBlockerOpacity(1);parent.blockContent(this.getZIndex()-1);this.__Az=true;}
;}
;this.__Au={left:x.left,top:x.top,right:x.left+z.width,bottom:x.top+z.height};var y=this.getContentLocation();this.__Ax=x.left;this.__Ay=x.top;this.__Av=y.left-e.getDocumentLeft();this.__Aw=y.top-e.getDocumentTop();this.addState(d);this.__As.capture();if(this.getUseMoveFrame()){this.__AD();}
;e.stop();}
,_onMovePointerMove:function(e){if(!this.hasState(d)){return;}
;var B=this.__AE(e);if(this.getUseMoveFrame()){this.__AC().setDomPosition(B.viewportLeft,B.viewportTop);}
...
;this.removeState(d);var parent=this.getLayoutParent();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__Az){parent.unblock();parent.setBlockerColor(this.__AA);parent.setBlockerOpacity(this.__AB);this.__AA=null;this.__AB=0;this.__Az=false;}
;}
;this.__As.releaseCapture();var D=this.__AE(e);var C=this.getLayoutParent().getInsets();this.setLayoutProperties({left:D.parentLeft-(C.left||0),top:D.parentTop-(C.top||0)});if(this.getUseMoveFrame()){this.__AC().exclude();}
;e.stopPropagation();}
,__AF:function(e){if(!this.hasState(d)){return;}
;this.removeState(d);if(this.getUseMoveFrame()){this.__AC().exclude();}
;}
},destruct:function(){this._disposeObjects(g,h);this.__Au=null;}
});}
)();
(function(){var a="resetPaddingRight",b="setPaddingTop",c="_applyContentPadding",d="setPaddingBottom",e="resetThemed",f="contentPaddingRight",g="Integer",h="contentPaddingLeft",i="setThemedPaddingLeft",j="resetPaddingTop",k="shorthand",l="setThemedPaddingRight",m="setThemed",n="setPaddingRight",o="contentPaddingBottom",p="resetPaddingBottom",q="qx.ui.core.MContentPadding",r="resetPaddingLeft",s="setThemedPaddingTop",t="setPaddingLeft",u="setThemedPaddingBottom",v="contentPaddingTop";qx.Mixin.define(q,{properties:{contentPaddingTop:{check:g,init:0,apply:c,themeable:true},contentPaddingRight:{check:g,init:0,apply:c,themeable:true},contentPaddingBottom:{check:g,init:0,apply:c,themeable:true},contentPaddingLeft:{check:g,init:0,apply:c,themeable:true},contentPadding:{group:[v,f,o,h],mode:k,themeable:true}},members:{__Ld:{contentPaddingTop:b,contentPaddingRight:n,contentPaddingBottom:d,contentPaddingLeft:t},__Le:{contentPaddingTop:s,contentPaddingRight:l,contentPaddingBottom:u,contentPaddingLeft:i},__Lf:{contentPaddingTop:j,contentPaddingRight:a,contentPaddingBottom:p,contentPaddingLeft:r},_applyContentPadding:function(z,w,name,y){var A=this._getContentPaddingTarget();if(z==null){var x=this.__Lf[name];A[x]();}
else {if(y==m||y==e){var B=this.__Le[name];A[B](z);}
else {var B=this.__Ld[name];A[B](z);}
;}
;}
}});}
)();
(function(){var a="Boolean",b="w-resize",c="touch",d="sw-resize",f="n-resize",g="resizableRight",h="ne-resize",i="se-resize",j="Integer",k="e-resize",l="resizableLeft",m="move",n="shorthand",o="maximized",p="resize",q="nw-resize",r="pointerup",s="pointerdown",t="qx.ui.core.MResizable",u="losecapture",v="resize-frame",w="resizableBottom",x="s-resize",y="pointermove",z="resizableTop",A="pointerout";qx.Mixin.define(t,{construct:function(){var content=this.getContentElement();content.addListener(s,this.__AS,this,true);content.addListener(r,this.__AT,this);content.addListener(y,this.__AV,this);content.addListener(A,this.__AW,this);content.addListener(u,this.__AU,this);var B=content.getDomElement();if(B==null){B=window;}
;this.__AG=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:a,init:true},resizableRight:{check:a,init:true},resizableBottom:{check:a,init:true},resizableLeft:{check:a,init:true},resizable:{group:[z,g,w,l],mode:n},resizeSensitivity:{check:j,init:5},useResizeFrame:{check:a,init:true}},members:{__AG:null,__AH:null,__AI:null,__AJ:null,__AK:null,__AL:null,__AM:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__AH;if(!C){C=this.__AH=new qx.ui.core.Widget();C.setAppearance(v);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__AN:function(){var location=this.getContentLocation();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__AO:function(e){var F=this.__AI;var E=this.getSizeHint();var I=this.__AM;var H=this.__AL;var J=H.width;var G=H.height;var L=H.left;var top=H.top;var K;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(I.top,Math.min(I.bottom,e.getDocumentTop()))-this.__AK;if(F&this.RESIZE_TOP){G-=K;}
else {G+=K;}
;if(G<E.minHeight){G=E.minHeight;}
else if(G>E.maxHeight){G=E.maxHeight;}
;if(F&this.RESIZE_TOP){top+=H.height-G;}
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(I.left,Math.min(I.right,e.getDocumentLeft()))-this.__AJ;if(F&this.RESIZE_LEFT){J-=K;}
else {J+=K;}
...
,__AP:{'1':f,'2':x,'4':b,'8':k,'5':q,'6':d,'9':h,'10':i},__AQ:function(e){var location=this.getContentLocation();var O=this.getResizeSensitivity();var N=e.getDocumentLeft();var P=e.getDocumentTop();var M=this.__AR(location,N,P,O);if(M>0){M=M|this.__AR(location,N,P,O*2);}
;this.__AI=M;}
,__AR:function(location,T,R,S){var Q=0;if(this.getResizableTop()&&Math.abs(location.top-R)<S&&T>location.left-S&&T<location.right+S){Q+=this.RESIZE_TOP;}
...
;return Q;}
,__AS:function(e){if(!this.__AI||!this.getEnabled()||e.getPointerType()==c){return;}
;this.addState(p);this.__AJ=e.getDocumentLeft();this.__AK=e.getDocumentTop();var location=this.getContentLocation();var V=this.getBounds();this.__AL={top:location.top,left:location.left,width:location.right-location.left,height:location.bottom-location.top,bounds:qx.lang.Object.clone(V)};var parent=this.getLayoutParent();var W=parent.getContentLocation();var U=parent.getBounds();this.__AM={left:W.left,top:W.top,right:W.left+U.width,bottom:W.top+U.height};if(this.getUseResizeFrame()){this.__AN();}
;this.capture();e.stop();}
,__AT:function(e){if(!this.hasState(p)||!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var X=this.__AO(e);this.setWidth(X.width);this.setHeight(X.height);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:X.parentLeft,top:X.parentTop});}
;this.__AI=0;this.removeState(p);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
,__AU:function(e){if(!this.__AI){return;}
...
;}
,__AV:function(e){if(!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.hasState(p)){var ba=this.__AO(e);if(this.getUseResizeFrame()){var Y=this._getResizeFrame();Y.setUserBounds(ba.viewportLeft,ba.viewportTop,ba.width,ba.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(o)&&!this.__AG.isSessionActive()){this.__AQ(e);var bd=this.__AI;var bc=this.getApplicationRoot();if(bd){var bb=this.__AP[bd];this.setCursor(bb);bc.setGlobalCursor(bb);}
else if(this.getCursor()){this.resetCursor();bc.resetGlobalCursor();}
;}
;}
,__AW:function(e){if(e.getPointerType()==c){return;}
;if(this.getCursor()&&!this.hasState(p)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__AH!=null&&!qx.core.ObjectRegistry.inShutDown){this.__AH.destroy();this.__AH=null;}
;this.__AG=null;}
});}
)();
...
;var br=this.getLayoutProperties();this.__zd=br.left===undefined?0:br.left;this.__zc=br.top===undefined?0:br.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(F);this._updateCaptionBar();this.fireEvent(Q);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(b,qx.event.type.Event,[false,true])){var bs=this.getLayoutProperties();this.__zd=bs.left===undefined?0:bs.left;this.__zc=bs.top===undefined?0:bs.top;this.removeState(F);this.hide();this.fireEvent(v);}
;}
...
;var bt=this.__zd;var top=this.__zc;this.setLayoutProperties({edge:null,left:bt,top:top});this.removeState(F);this._updateCaptionBar();this.fireEvent(S);}
;}
...
(function(){var a="vertical",b="under",c="above",d="qx.ui.core.selection.Widget",e="left",f="right";qx.Class.define(d,{extend:qx.ui.core.selection.Abstract,construct:function(g){qx.ui.core.selection.Abstract.call(this);this.__eo=g;}
,members:{__eo:null,_isSelectable:function(h){return this._isItemSelectable(h)&&h.getLayoutParent()===this.__eo;}
,_selectableToHashCode:function(j){return j.$$hash;}
,_styleSelectable:function(k,m,n){n?k.addState(m):k.removeState(m);}
,_capture:function(){this.__eo.capture();}
,_releaseCapture:function(){this.__eo.releaseCapture();}
,_isItemSelectable:function(o){if(this._userInteraction){return o.isVisible()&&o.isEnabled();}
else {return o.isVisible();}
;}
,_getWidget:function(){return this.__eo;}
,_getLocation:function(){var p=this.__eo.getContentElement().getDomElement();return p?qx.bom.element.Location.get(p):null;}
,_getDimension:function(){return this.__eo.getInnerSize();}
...
,_scrollItemIntoView:function(w){this.__eo.scrollChildIntoView(w);}
,getSelectables:function(x){var y=false;if(!x){y=this._userInteraction;this._userInteraction=true;}
;var B=this.__eo.getChildren();var z=[];var A;for(var i=0,l=B.length;i<l;i++ ){A=B[i];if(this._isItemSelectable(A)){z.push(A);}
;}
;this._userInteraction=y;return z;}
,_getSelectableRange:function(C,D){if(C===D){return [C];}
;var H=this.__eo.getChildren();var E=[];var G=false;var F;for(var i=0,l=H.length;i<l;i++ ){F=H[i];if(F===C||F===D){if(G){E.push(F);break;}
else {G=true;}
;}
;if(G&&this._isItemSelectable(F)){E.push(F);}
;}
;return E;}
,_getFirstSelectable:function(){var I=this.__eo.getChildren();for(var i=0,l=I.length;i<l;i++ ){if(this._isItemSelectable(I[i])){return I[i];}
;}
;return null;}
,_getLastSelectable:function(){var J=this.__eo.getChildren();for(var i=J.length-1;i>0;i-- ){if(this._isItemSelectable(J[i])){return J[i];}
;}
;return null;}
,_getRelatedSelectable:function(N,O){var L=this.__eo.getOrientation()===a;var P=this.__eo.getChildren();var K=P.indexOf(N);var M;if((L&&O===c)||(!L&&O===e)){for(var i=K-1;i>=0;i-- ){M=P[i];if(this._isItemSelectable(M)){return M;}
;}
;}
...
;this.__u.removeItem(I);}
,selectRange:function(J,K){this.__u.selectItemRange(J,K);}
,resetSelection:function(){this.__u.clearSelection();}
...
else {var M=this.getSelection();if(!qx.lang.Array.equals(M,L)){this.__u.replaceSelection(L);}
;}
;}
,getSelection:function(){return this.__u.getSelection();}
,getSortedSelection:function(){return this.__u.getSortedSelection();}
,isSelectionEmpty:function(){return this.__u.isSelectionEmpty();}
,getSelectionContext:function(){return this.__u.getSelectionContext();}
,_getManager:function(){return this.__u;}
,getSelectables:function(N){return this.__u.getSelectables(N);}
,invertSelection:function(){this.__u.invertSelection();}
,_getLeadItem:function(){var O=this.__u.getMode();if(O===q||O===b){return this.__u.getSelectedItem();}
else {return this.__u.getLeadItem();}
;}
,_applySelectionMode:function(Q,P){this.__u.setMode(Q);}
,_applyDragSelection:function(S,R){this.__u.setDrag(S);}
,_applyQuickSelection:function(U,T){this.__u.setQuick(U);}
...
(function(){var a="Could not set the model selection. Maybe your models are not unique? ",b="qx.ui.form.MModelSelection",c="Please use an array as parameter.",d="change",f="qx.debug",g="__l",h="changeSelection",k="qx.event.type.Data";qx.Mixin.define(b,{construct:function(){this.__l=new qx.data.Array();this.__l.addListener(d,this.__o,this);this.addListener(h,this.__n,this);}
,events:{changeModelSelection:k},members:{__l:null,__m:false,__n:function(){if(this.__m){return;}
...
;}
;}
,__o:function(){this.__m=true;var r=this.getSelectables(true);var s=[];var q=this.__l.toArray();for(var i=0;i<q.length;i++ ){var u=q[i];for(var j=0;j<r.length;j++ ){var v=r[j];var p=v.getModel?v.getModel():null;if(u===p){s.push(v);break;}
;}
;}
;this.setSelection(s);this.__m=false;var t=this.getSelection();if(!qx.lang.Array.equals(t,s)){this.__n();}
;}
,getModelSelection:function(){return this.__l;}
,setModelSelection:function(w){if(!w){this.__l.removeAll();return;}
;if(qx.core.Environment.get(f)){this.assertArray(w,c);}
;w.unshift(this.__l.getLength());w.unshift(0);var x=this.__l.splice.apply(this.__l,w);x.dispose();}
...
(function(){var a="qx.ui.form.List",b="text",c="Boolean",d="one",f="addChildWidget",g="_applySpacing",h="Enter",j="Integer",k="action",m="keyinput",n="qx.event.type.Data",o="addItem",p="vertical",q="removeChildWidget",r="",s="_applyOrientation",t="__eF",u="single",v="keypress",w="list",x="horizontal",y="label",z="pane",A="removeItem";qx.Class.define(a,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__eF=this._createListItemContainer();this.__eF.addListener(f,this._onAddChild,this);this.__eF.addListener(q,this._onRemoveChild,this);this.getChildControl(z).add(this.__eF);if(B){this.setOrientation(x);}
else {this.initOrientation();}
;this.addListener(v,this._onKeyPress);this.addListener(m,this._onKeyInput);this.__eG=r;}
,events:{addItem:n,removeItem:n},properties:{appearance:{refine:true,init:w},focusable:{refine:true,init:true},orientation:{check:[x,p],init:p,apply:s},spacing:{check:j,init:0,apply:g,themeable:true},enableInlineFind:{check:c,init:true}},members:{__eG:null,__eH:null,__eF:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__eF;}
...
,_applyOrientation:function(F,C){var D=F===x;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__eF;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__eF.getLayout().setSpacing(H);}
...
;if(((new Date).valueOf()-this.__eH)>1000){this.__eG=r;}
;this.__eG+=e.getChar();var K=this.findItemByLabelFuzzy(this.__eG);if(K){this.setSelection([K]);}
;this.__eH=(new Date).valueOf();}
...
;if(this.getSelection()==null){this.__cz=new qx.data.Array();this.setSelection(this.__cz);}
;}
,properties:{selection:{check:b,event:f,apply:d,init:null}},events:{"changeSelection":h},members:{_modifingSelection:0,__cA:null,__cB:null,__cz:null,_applySelection:function(k,j){if(this.__cB!=undefined&&j!=undefined){j.removeListenerById(this.__cB);this.__cB=null;}
;if(k){this.__cB=k.addListener(a,this.__cC,this);}
;this._updateSelection();}
,__cC:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__cD()&&!this.__cE()){return;}
;if(this._inSelectionModification()){return;}
;var m=this.getTarget().getSelection();var l=this.getSelection();if(l==null){l=new qx.data.Array();this.__cz=l;this.setSelection(l);}
...
,_addChangeTargetListener:function(p,o){if(this.__cA!=undefined&&o!=undefined){o.removeListenerById(this.__cA);}
;if(p!=null){if(this.__cD()||this.__cE()){this.__cA=p.addListener(f,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()||!this.getSelection()){return;}
;this._startSelectionModification();if(this.__cD()){var s=[];for(var i=0;i<this.getSelection().length;i++ ){var t=this.getSelection().getItem(i);var u=this.__cG(t);if(u!=null){s.push(u);}
;}
...
;}
;}
else if(this.__cE()){var r=this.getSelection().getItem(this.getSelection().length-1);if(r!==undefined){this.__cF(r);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__cD:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.IMultiSelection);}
,__cE:function(){var w=this.getTarget().constructor;return qx.Class.implementsInterface(w,qx.ui.core.ISingleSelection);}
,__cF:function(x){var y=this.__cG(x);if(y==null){return;}
;if(this.__cD()){this.getTarget().addToSelection(y);}
else if(this.__cE()){this.getTarget().setSelection([y]);}
;}
,__cG:function(z){var A=this.getTarget().getSelectables(true);for(var i=0;i<A.length;i++ ){if(A[i].getModel()==z){return A[i];}
;}
;return null;}
...
},destruct:function(){if(this.__cz){this.__cz.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="_applyLabelOptions",c="_applyLabelPath",d="changeTarget",e="_applyModel",f="change",g="icon",h="ReverseBindingId",k="qx.data.controller.List",m="_applyIconPath",n="BindingId",o="_applyDelegate",p="",q="changeDelegate",r="_applyTarget",s="]",t="qx.data.IListData",u="model[",v="model",w="label",x="String",y="_applyIconOptions",z=".";qx.Class.define(k,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__dY=[];this.__db=[];this.__ea=[];this.__eb={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:t,apply:e,event:a,nullable:true,dereference:true},target:{apply:r,event:d,nullable:true,init:null,dereference:true},labelPath:{check:x,apply:c,nullable:true},iconPath:{check:x,apply:m,nullable:true},labelOptions:{apply:b,nullable:true},iconOptions:{apply:y,nullable:true},delegate:{apply:o,event:q,init:null,nullable:true}},members:{__ec:null,__dY:null,__eb:null,__db:null,__ea:null,__ed:null,__ee:null,update:function(){this.__eh();this.__dl();this._updateSelection();}
,_applyDelegate:function(E,D){this._setConfigureItem(E,D);this._setFilter(E,D);this._setCreateItem(E,D);this._setBindItem(E,D);}
,_applyIconOptions:function(G,F){this.__dl();}
,_applyLabelOptions:function(I,H){this.__dl();}
,_applyIconPath:function(K,J){this.__dl();}
,_applyLabelPath:function(M,L){this.__dl();}
,_applyModel:function(P,N){if(N!=undefined){if(this.__ec!=undefined){N.removeListenerById(this.__ec);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(P!=null){this.__ec=P.addListener(f,this.__eg,this);this.__em();this.__eh();if(N==null){this._changeTargetSelection();}
else {this.__ed=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var O=this.getTarget();if(O!=null){var length=O.getChildren().length;for(var i=0;i<length;i++ ){this.__ek();}
;}
;}
;}
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__dY.length;i++ ){this.__ej(this.__en(i));}
;}
;}
;}
,__ef:false,__eg:function(){if(this.__ef)return;this.__ef=true;this.__ee=true;qx.ui.core.queue.Widget.add(this);this.update();this.__ef=false;}
,syncWidget:function(){if(this.__ed){this._changeTargetSelection();}
;if(this.__ee){this._updateSelection();}
;this.__ee=this.__ed=null;}
,__eh:function(){if(this.getTarget()==null){return;}
;this.__em();var U=this.__dY.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__ej(this.__en(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__ek();}
;}
;this.__em();this.__ed=true;qx.ui.core.queue.Widget.add(this);}
,__ei:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__ec);this.__ec=V.addListener(f,this.__eg,this);}
;}
...
;return W;}
,__ej:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__ek:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bo,br,bp,bn,bm){if(bp!=null){var bp=qx.lang.Object.clone(bp);this.__eb[br]=bp.onUpdate;delete bp.onUpdate;}
else {bp={};this.__eb[br]=null;}
;bp.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bm);bp.ignoreConverter=v;var bq=u+bm+s;if(bo!=null&&bo!=p){bq+=z+bo;}
;var bs=this.bind(bq,bn,br,bp);bn.setUserData(br+n,bs);if(!qx.lang.Array.contains(this.__db,br)){this.__db.push(br);}
;}
,bindPropertyReverse:function(by,bv,bw,bx,bt){var bu=u+bt+s;if(by!=null&&by!=p){bu+=z+by;}
;var bz=bx.bind(bv,this,bu,bw);bx.setUserData(by+h,bz);if(!qx.lang.Array.contains(this.__ea,by)){this.__ea.push(by);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__db.length;i++ ){if(this.__eb[this.__db[i]]!=null){this.__eb[this.__db[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__db.length;i++ ){var bE=bD.getUserData(this.__db[i]+n);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__ea.length;i++ ){var bE=bD.getUserData(this.__ea[i]+h);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__dl:function(){if(this.getTarget()==null||this.getModel()==null){return;}
...
,_setFilter:function(bR,bP){if((bR==null||bR.filter==null)&&(bP!=null&&bP.filter!=null)){this.__el();}
...
;var bQ=this.__dY;this.__em();if(bQ.length>this.__dY.length){for(var j=bQ.length;j>this.__dY.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bQ.length<this.__dY.length){for(var j=bQ.length;j<this.__dY.length;j++ ){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__en(i));}
;this.__ei();this._endSelectionModification();this._updateSelection();}
,__el:function(){this.__em();this.__eh();this.__dl();this.__ee=true;qx.ui.core.queue.Widget.add(this);}
,__em:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__dY=[];for(var i=0;i<bW.getLength();i++ ){if(bV==null||bV(bW.getItem(i))){this.__dY.push(i);}
;}
;}
,__en:function(bY){return this.__dY[bY];}
},destruct:function(){this.__dY=this.__eb=this.__db=null;this.__ea=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
