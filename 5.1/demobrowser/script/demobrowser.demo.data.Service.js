...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Service.ab1d7a0d1fb1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var M=this.__Lq(),L=this.__Lk,N=this;if(this.timeout>0){this.__Cs=window.setTimeout(this.__zi,this.timeout);}
;if(this.__Ls(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
...
;this.__zk=true;this.__Lr();this._emit(m);return this;}
...
;if(this.__Ls(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__sH){return null;}
;if(this.__Ls(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__Ll=P;}
,dispose:function(){var Q=this.__Lm;if(!this.__sH){if(Q){Q.onload=Q.onreadystatechange=null;this.__Lr();}
;if(this.__Cs){window.clearTimeout(this.__Cs);}
;this.__sH=true;}
;}
,isDisposed:function(){return !!this.__sH;}
,_getUrl:function(){return this.__sC;}
,_getScriptElement:function(){return this.__Lm;}
,_onTimeout:function(){this.__Lo();if(!this.__Lp()){this._emit(f);}
;this._emit(B);if(!this.__Lp()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__Lm,R=this.__Ll,T=this;if(this.__zk){return;}
;if(this.__Ls(x)===w&&this.__Ls(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__Ls(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__Ls(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__Ls(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__Cs){window.clearTimeout(this.__Cs);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__Lo();this._emit(f);this._emit(g);}
,__Lm:null,__Lk:null,__sC:n,__Li:null,__Lj:null,__zi:null,__Cs:null,__zk:null,__sH:null,__Ln:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__Lr();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__Lo:function(){this.__Lr();this.readyState=4;this.status=0;this.statusText=null;}
,__Lp:function(){var W=this.__Ls(x)===w&&this.__Ls(l)<9;var V=this.__Ls(x)===s;return !(W||V);}
,__Lq:function(){var X=this.__Lm=document.createElement(i);X.src=this.__sC;X.onerror=this.__Lj;X.onload=this.__Li;if(this.__Ls(x)===w&&this.__Ls(l)<9){X.onreadystatechange=this.__Li;}
;return X;}
,__Lr:function(){var Y=this.__Lm;if(Y&&Y.parentNode){this.__Lk.removeChild(Y);}
;}
,__Ls:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__zw();}
,members:{responseJson:null,__gH:null,__zo:null,__zp:null,__zq:null,__zr:null,__zs:null,__sH:null,__zt:e,open:function(t,p){if(this.__sH){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__zq=false;q=this.__zo||b;s=this.__zp||this.__zt+n+this.__gH+m;if(!this.__zp){this.constructor[this.__gH]=this;}
else {if(!window[this.__zp]){this.__zr=true;window[this.__zp]=function(u){o.callback(u);}
;}
else {if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,k+this.__zp+j);}
;}
;}
;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,h+s);}
;r[q]=s;this.__zs=p=qx.util.Uri.appendParamsToUrl(p,r);this.__zv(c,[t,p]);}
,callback:function(v){if(this.__sH){return;}
;this.__zq=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__gH]=undefined;this.__zu();}
,setCallbackParam:function(w){this.__zo=w;return this;}
,setCallbackName:function(name){this.__zp=name;return this;}
,setPrefix:function(x){this.__zt=x;}
,getGeneratedUrl:function(){return this.__zs;}
,dispose:function(){this.__zu();this.__zv(d);}
,_onNativeLoad:function(){this.status=this.__zq?200:500;this.__zv(i);}
,__zu:function(){if(this.__zr&&window[this.__zp]){window[this.__zp]=undefined;this.__zr=false;}
;}
,__zv:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__zw:function(){this.__gH=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
...
(function(){var a="changeModel",b="qx.core.Object",c="qx.data.controller.Object",d="get",e="[",f="reset",g="_applyModel",h="last";qx.Class.define(c,{extend:qx.core.Object,construct:function(j){qx.core.Object.call(this);this.__da={};this.__FL=[];if(j!=null){this.setModel(j);}
;}
,properties:{model:{check:b,event:a,apply:g,nullable:true,dereference:true}},members:{__FL:null,__da:null,_applyModel:function(o,l){for(var i=0;i<this.__FL.length;i++ ){var t=this.__FL[i][0];var q=this.__FL[i][1];var n=this.__FL[i][2];var r=this.__FL[i][3];var p=this.__FL[i][4];var m=this.__FL[i][5];if(l!=undefined&&!l.isDisposed()){this.__Ly(t,q,n,l);}
;if(o!=undefined){this.__Lx(t,q,n,r,p,m);}
...
;}
;}
,addTarget:function(z,w,u,x,y,v){this.__FL.push([z,w,u,x,y,v]);this.__Lx(z,w,u,x,y,v);}
,__Lx:function(F,D,B,G,E,A){if(this.getModel()==null){return;}
;var C=this.getModel().bind(B,F,D,E);var I=null;if(G){I=F.bind(D,this.getModel(),B,A);}
;var H=F.toHashCode();if(this.__da[H]==undefined){this.__da[H]=[];}
;this.__da[H].push([C,I,D,B,E,A]);}
,removeTarget:function(K,J,L){this.__Ly(K,J,L,this.getModel());for(var i=0;i<this.__FL.length;i++ ){if(this.__FL[i][0]==K&&this.__FL[i][1]==J&&this.__FL[i][2]==L){this.__FL.splice(i,1);}
;}
;}
,__Ly:function(R,O,N,M){if(!(R instanceof qx.core.Object)){return;}
;var P=this.__da[R.toHashCode()];if(P==undefined||P.length==0){return;}
