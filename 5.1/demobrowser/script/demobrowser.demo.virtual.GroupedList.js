...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.GroupedList.9d42618fa290.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
};T.setDelegate(V);T.setSelection(this.__fW.getSelection());return U;}
,createThirdExample:function(){var bc=new qx.ui.container.Composite(new qx.ui.layout.Canvas());var be=new qx.ui.basic.Label(u).set({font:v});bc.add(be);var bb=this.__Ol=new qx.ui.list.List().set({height:280,width:150,labelPath:e,labelOptions:{converter:function(bg,bf){return bf?bg+D+bf.getLastname():B;}
...
};bb.setDelegate(bd);bb.setSelection(this.__fW.getSelection());return bc;}
},destruct:function(){this.__fW.dispose();this.__Ok.dispose();this.__Ol.dispose();this.__fW=this.__Ok=this.__Ol=null;}
});}
)();
...
(function(){var a="changeModel",b="application/json",c="json",d="aborted",e="loaded",f="qx.event.type.Data",g="error",h="__iI",i="fail",j="receiving",k="queued",l="changeState",m="success",n="String",o="changePhase",p="sending",q="_marshaler",r="completed",s="failed",t="qx.data.store.Json",u="configured",v="changeUrl",w="timeout",x="_applyUrl";qx.Class.define(t,{extend:qx.core.Object,construct:function(y,z){qx.core.Object.call(this);this._marshaler=new qx.data.marshal.Json(z);this._delegate=z;if(y!=null){this.setUrl(y);}
;}
,events:{"loaded":f,"error":f},properties:{model:{nullable:true,event:a},state:{check:[u,k,p,j,r,d,w,s],init:u,event:l},url:{check:n,apply:x,event:v,nullable:true}},members:{_marshaler:null,_delegate:null,__iI:null,_applyUrl:function(B,A){if(B!=null){B=qx.util.AliasManager.getInstance().resolve(B);B=qx.util.ResourceManager.getInstance().toUri(B);this._createRequest(B);}
;}
,_getRequest:function(){return this.__iI;}
,_setRequest:function(C){this.__iI=C;}
,_createRequest:function(D){if(this.__iI){this.__iI.dispose();this.__iI=null;}
...
;this.fireDataEvent(e,this.getModel());if(this.__iI){this.__iI.dispose();this.__iI=null;}
;}
,reload:function(){var R=this.getUrl();if(R!=null){this._createRequest(R);}
;}
},destruct:function(){if(this.__iI!=null){this._disposeObjects(h);}
...
},members:{__d:null,__e:function(z){return Object.keys(z).sort().join(a);}
,toClass:function(A,B){this.__f(A,B,null,0);}
,__f:function(N,I,C,K){if(!qx.lang.Type.isObject(N)||!!N.$$isString||N instanceof qx.core.Object){if(N instanceof Array||qx.Bootstrap.getClass(N)==h){for(var i=0;i<N.length;i++ ){this.__f(N[i],I,C+o+i+m,K+1);}
;}
;return;}
;var E=this.__e(N);if(this.__j(E,C,K)){return;}
;for(var M in N){this.__f(N[M],I,M,K+1);}
;if(qx.Class.isDefined(s+E)){return;}
;if(this.__d&&this.__d.getModelClass&&this.__d.getModelClass(E,N,C,K)!=null){return;}
;var O={};var F={__g:this.__g};for(var M in N){if(this.__d&&this.__d.getPropertyMapping){M=this.__d.getPropertyMapping(M,E);}
...
;if(this.__d&&this.__d.getValidationRule){var H=this.__d.getValidationRule(E,M);if(H){O[M].validate=k+M;F[k+M]=H;}
;}
;}
;if(this.__d&&this.__d.getModelSuperClass){var L=this.__d.getModelSuperClass(E,C,K)||qx.core.Object;}
else {var L=qx.core.Object;}
;var G=[];if(this.__d&&this.__d.getModelMixins){var J=this.__d.getModelMixins(E,C,K);if(!qx.lang.Type.isArray(J)){if(J!=null){G=[J];}
;}
else {G=J;}
;}
;if(I){G.push(qx.data.marshal.MEventBubbling);}
;var D={extend:L,include:G,properties:O,members:F,destruct:this.__h};qx.Class.define(s+E,D);}
,__h:function(){var P=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var Q in P){this.__g(this.get(P[Q].name));}
;}
,__g:function(R){if(!(R instanceof qx.core.Object)){return;}
;if(R.isDisposed()){return;}
;R.dispose();}
,__i:function(S,W,X,V){var Y;if(this.__d&&this.__d.getModelClass){Y=this.__d.getModelClass(S,W,X,V);}
;if(Y!=null){return (new Y());}
else {var U=s+S;var T=qx.Class.getByName(U);if(!T){throw new Error(b+U+l);}
;return (new T());}
;}
,__j:function(ba,bd,bb){var bc=this.__d;return bc&&bc.ignore&&bc.ignore(ba,bd,bb);}
,toModel:function(be){return this.__k(be,null,0);}
,__k:function(bq,bg,bm){var bk=qx.lang.Type.isObject(bq);var bf=bq instanceof Array||qx.Bootstrap.getClass(bq)==h;if((!bk&&!bf)||!!bq.$$isString||bq instanceof qx.core.Object){return bq;}
else if(this.__j(this.__e(bq),bg,bm)){return bq;}
else if(bf){var bo=qx.data.Array;if(this.__d&&this.__d.getArrayClass){var bl=this.__d.getArrayClass(bg,bm);bo=bl||bo;}
;var br=new bo();br.setAutoDisposeItems(true);for(var i=0;i<bq.length;i++ ){br.push(this.__k(bq[i],bg+o+i+m,bm+1));}
;return br;}
else if(bk){var bh=this.__e(bq);var bp=this.__i(bh,bq,bg,bm);for(var bn in bq){var bi=bn;if(this.__d&&this.__d.getPropertyMapping){bi=this.__d.getPropertyMapping(bn,bh);}
;var bs=bi.replace(/-|\.|\s+/g,d);if((qx.core.Environment.get(t))&&qx.core.Environment.get(e)){if(bs!=bi){this.warn(c+bn+n+bi+v);}
;}
;bi=bs;var bj=p+qx.lang.String.firstUp(bi);if(bp[bj]){bp[bj](this.__k(bq[bn],bn,bm+1));}
;}
;return bp;}
;throw new Error(j);}
...
;if((n instanceof qx.core.Object)&&qx.Class.hasMixin(n.constructor,qx.data.marshal.MEventBubbling)){var o=qx.lang.Function.bind(this.__HT,this,name);var q=n.addListener(g,o,this);var p=n.getUserData(d+this.$$hash);if(p==null){p=[];n.setUserData(d+this.$$hash,p);}
;p.push(q);}
;}
,__HT:function(name,e){var y=e.getData();var u=y.value;var s=y.old;if(qx.Class.hasInterface(e.getTarget().constructor,qx.data.IListData)){if(y.name.indexOf){var x=y.name.indexOf(h)!=-1?y.name.indexOf(h):y.name.length;var v=y.name.indexOf(f)!=-1?y.name.indexOf(f):y.name.length;if(v==0){var t=name+y.name;}
...
else if(arguments.length>1){this.__nO=[];for(var i=0;i<arguments.length;i++ ){this.__nO.push(arguments[i]);}
;}
else if(typeof u==q){this.__nO=new Array(u);}
else if(u instanceof Array){this.__nO=qx.lang.Array.clone(u);}
else {this.__nO=[];this.dispose();throw new Error(l);}
;for(var i=0;i<this.__nO.length;i++ ){this._applyEventPropagation(this.__nO[i],null,i);}
;this.__nP();if(qx.core.Environment.get(k)){this[0]=h;}
;}
,properties:{autoDisposeItems:{check:e,init:false}},events:{"change":t,"changeLength":t},members:{__nO:null,concat:function(v){if(v){var w=this.__nO.concat(v);}
else {var w=this.__nO.concat();}
;return new qx.data.Array(w);}
,join:function(x){return this.__nO.join(x);}
,pop:function(){var y=this.__nO.pop();this.__nP();this._registerEventChaining(null,y,this.length-1);this.fireDataEvent(r,{value:[],name:this.length+f,old:[y],item:this});this.fireDataEvent(o,{start:this.length-1,end:this.length-1,type:g,removed:[y],added:[]},null);return y;}
,push:function(z){for(var i=0;i<arguments.length;i++ ){this.__nO.push(arguments[i]);this.__nP();this._registerEventChaining(arguments[i],null,this.length-1);this.fireDataEvent(r,{value:[arguments[i]],name:(this.length-1)+f,old:[],item:this});this.fireDataEvent(o,{start:this.length-1,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,reverse:function(){if(this.length==0){return;}
;var A=this.__nO.concat();this.__nO.reverse();this.__nQ(0,this.length);this.fireDataEvent(o,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(r,{value:this.__nO,name:n+(this.__nO.length-1),old:A,item:this});}
,shift:function(){if(this.length==0){return;}
;var B=this.__nO.shift();this.__nP();this._registerEventChaining(null,B,this.length-1);this.__nQ(0,this.length);this.fireDataEvent(r,{value:[],name:p,old:[B],item:this});this.fireDataEvent(o,{start:0,end:this.length-1,type:g,removed:[B],added:[]},null);return B;}
,slice:function(D,C){return new qx.data.Array(this.__nO.slice(D,C));}
,splice:function(I,K,M){var Q=this.__nO.length;var L=this.__nO.splice.apply(this.__nO,arguments);if(this.__nO.length!=Q){this.__nP();}
...
;this.__nQ(I+(arguments.length-2)-K,this.length);if(O||G){var J=[];for(var i=2;i<arguments.length;i++ ){J[i-2]=arguments[i];}
...
;var R=this.__nO.concat();this.__nO.sort.apply(this.__nO,arguments);if(qx.lang.Array.equals(this.__nO,R)===true){return;}
;this.__nQ(0,this.length);this.fireDataEvent(o,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(r,{value:this.__nO,name:n+(this.length-1),old:R,item:this});}
,unshift:function(T){for(var i=arguments.length-1;i>=0;i-- ){this.__nO.unshift(arguments[i]);this.__nP();this.__nQ(0,this.length);this.fireDataEvent(r,{value:[this.__nO[0]],name:p,old:[this.__nO[1]],item:this});this.fireDataEvent(o,{start:0,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,toArray:function(){return this.__nO;}
,getItem:function(U){return this.__nO[U];}
,setItem:function(V,X){var W=this.__nO[V];if(W===X){return;}
;this.__nO[V]=X;this._registerEventChaining(X,W,V);if(this.length!=this.__nO.length){this.__nP();}
...
,indexOf:function(Y){return this.__nO.indexOf(Y);}
,lastIndexOf:function(ba){return this.__nO.lastIndexOf(ba);}
,toString:function(){if(this.__nO!=null){return this.__nO.toString();}
;return f;}
,contains:function(bb){return this.__nO.indexOf(bb)!==-1;}
...
;var bo=this.getLength();var bn=this.__nO.concat();this.__nO.length=0;this.__nP();this.fireDataEvent(r,{value:[],name:n+(bo-1),old:bn,item:this});this.fireDataEvent(o,{start:0,end:bo-1,type:g,removed:bn,added:[]},null);return bn;}
...
;var bq=this.__nO.length;Array.prototype.push.apply(this.__nO,bp);for(var i=0;i<bp.length;i++ ){this._registerEventChaining(bp[i],null,bq+i);}
;var bq=this.length;this.__nP();var name=bq==(this.length-1)?bq:bq+a+(this.length-1);this.fireDataEvent(r,{value:bp,name:name+f,old:[],item:this});this.fireDataEvent(o,{start:bq,end:this.length-1,type:b,added:bp,removed:[]},null);}
...
,forEach:function(bx,by){for(var i=0;i<this.__nO.length;i++ ){bx.call(by,this.__nO[i],i,this);}
;}
,filter:function(bz,self){return new qx.data.Array(this.__nO.filter(bz,self));}
,map:function(bA,self){return new qx.data.Array(this.__nO.map(bA,self));}
,some:function(bB,self){return this.__nO.some(bB,self);}
,every:function(bC,self){return this.__nO.every(bC,self);}
,reduce:function(bE,bD){return this.__nO.reduce(bE,bD);}
,reduceRight:function(bG,bF){return this.__nO.reduceRight(bG,bF);}
,__nP:function(){var bH=this.length;this.length=this.__nO.length;this.fireDataEvent(s,this.length,bH);}
,__nQ:function(bJ,bI){for(var i=bJ;i<bI;i++ ){this._registerEventChaining(this.__nO[i],this.__nO[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__nO.length;i++ ){var bK=this.__nO[i];this._applyEventPropagation(null,bK,i);if(this.isAutoDisposeItems()&&bK&&bK instanceof qx.core.Object){bK.dispose();}
;}
;this.__nO=null;}
});}
)();
...
;this.__nR={};var S=this._transport=this._createTransport();this._setPhase(H);this.__ze=qx.lang.Function.bind(this._onReadyStateChange,this);this.__zf=qx.lang.Function.bind(this._onLoad,this);this.__zg=qx.lang.Function.bind(this._onLoadEnd,this);this.__zh=qx.lang.Function.bind(this._onAbort,this);this.__zi=qx.lang.Function.bind(this._onTimeout,this);this.__zj=qx.lang.Function.bind(this._onError,this);S.onreadystatechange=this.__ze;S.onload=this.__zf;S.onloadend=this.__zg;S.onabort=this.__zh;S.ontimeout=this.__zi;S.onerror=this.__zj;}
...
,nullable:true},authentication:{check:k,nullable:true}},members:{__ze:null,__zf:null,__zg:null,__zh:null,__zi:null,__zj:null,__sI:null,__zk:null,__zl:null,__nR:null,__zm:null,_transport:null,_createTransport:function(){throw new Error(F);}
...
;this.__zk=true;this.__zl=t;this._transport.abort();}
...
;}
,_getAllRequestHeaders:function(){var bd={};qx.lang.Object.mergeWith(bd,this._getConfiguredRequestHeaders());qx.lang.Object.mergeWith(bd,this.__zn());qx.lang.Object.mergeWith(bd,this.__zm);qx.lang.Object.mergeWith(bd,this.__nR);return bd;}
,__zn:function(){var bf=this.getAuthentication(),be={};if(bf){bf.getAuthHeaders().forEach(function(bg){be[bg.key]=bg.value;}
);return be;}
;}
,setRequestHeader:function(bh,bi){this.__nR[bh]=bi;}
,getRequestHeader:function(bj){return this.__nR[bj];}
,removeRequestHeader:function(bk){if(this.__nR[bk]){delete this.__nR[bk];}
;}
,getTransport:function(){return this._transport;}
,getReadyState:function(){return this._transport.readyState;}
,getPhase:function(){return this.__zl;}
...
;this.fireEvent(E);if(this.__zk){return;}
;if(bp===3){this._setPhase(q);}
;if(this.isDone()){this.__sJ();}
;}
,__sJ:function(){if(qx.core.Environment.get(f)){this.debug(B+this.getStatus());}
...
,_setPhase:function(br){var bs=this.__zl;if(qx.core.Environment.get(n)){qx.core.Assert.assertString(br);qx.core.Assert.assertMatch(br,/^(unsent)|(opened)|(sent)|(loading)|(load)|(success)|(abort)|(timeout)|(statusError)$/);}
;this.__zl=br;this.fireDataEvent(c,br,bs);}
...
;var A=z[h+qx.lang.String.firstUp(name)]();if(qx.lang.Type.isArray(A)){var D=qx.data&&qx.data.IListData&&qx.Class.hasInterface(A&&A.constructor,qx.data.IListData);for(var i=0;i<A.length;i++ ){var F=D?A.getItem(i):A[i];E+=this.__Lh(name,F,C);}
;}
else if(qx.lang.Type.isDate(A)&&y!=null){E+=this.__Lh(name,y.format(A),C);}
else {E+=this.__Lh(name,A,C);}
;}
;return E.substring(0,E.length-1);}
,__Lh:function(name,I,G){if(I&&I.$$type==o){I=I.classname;}
...
(function(){var a="Retry open native request with method: ",b="activex",c="No XHR support available.",d="If-None-Match",f="xhr",g="If-Modified-Since",h="engine.version",i="onunload",j="GET",k="-1",l="qx.debug.io",m="Undefined",n="Open native request with method: ",o="error",p="loadend",q="Blob",r="HTMLDocument",s="progress",t="load",u="Received native readyState: ",v="abort",w=", url: ",x="String",y="browser.documentmode",z="",A="readystatechange",B="Microsoft.XMLHTTP",C="Already disposed",D="browser.version",E="opera",F="qx.bom.request.Xhr",G="on",H="timeout",I="gecko",J="If-Match",K="mshtml",L="engine.name",M="Microsoft.XMLDOM",N="file:",O="Send native request",P="FormData",Q="If-Range",R="Content-Type",S="io.xhr",T="Not enough arguments",U="ArrayBuffer",V="undefined",W=", async: ",X="Native XHR object doesn't support overrideMimeType.";qx.Bootstrap.define(F,{extend:Object,construct:function(){var Y=qx.Bootstrap.bind(this.__Kp,this);if(qx.event&&qx.event.GlobalError&&qx.event.GlobalError.observeMethod){this.__Kc=qx.event.GlobalError.observeMethod(Y);}
else {this.__Kc=Y;}
;this.__Kd=qx.Bootstrap.bind(this.__Kn,this);this.__Ke=qx.Bootstrap.bind(this.__Ko,this);this.__zi=qx.Bootstrap.bind(this.__Kt,this);this.__Km();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__Kf=qx.Bootstrap.bind(this.__Kw,this);window.attachEvent(i,this.__Kf);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":F,"error":F,"loadend":F,"timeout":F,"abort":F,"load":F,"progress":F},members:{readyState:0,responseText:z,responseXML:null,status:0,statusText:z,timeout:0,progress:null,open:function(be,ba,bb,bd,bc){this.__Ky();if(typeof ba===V){throw new Error(T);}
else if(typeof be===V){be=j;}
;this.__zk=false;this.__Kg=false;this.__Kh=false;this.__sC=ba;if(typeof bb==V){bb=true;}
;this.__Ki=bb;if(!this.__Kx()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__Km();}
;this.__Kk.onreadystatechange=this.__Kc;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,n+be+w+ba+W+bb);}
;this.__Kk.open(be,ba,bb,bd,bc);}
catch(bf){if(!qx.util.Request.isCrossDomain(ba)){throw bf;}
;if(!this.__Ki){this.__Kj=bf;}
;if(this.__Ki){if(window.XDomainRequest){this.readyState=4;this.__Kk=new XDomainRequest();this.__Kk.onerror=qx.Bootstrap.bind(function(){this._emit(A);this._emit(o);this._emit(p);}
,this);if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,a+be+w+ba+W+bb);}
;this.__Kk.open(be,ba,bb,bd,bc);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__sH){return;}
;this.readyState=4;this._emit(A);this._emit(o);this._emit(p);}
,this));}
;}
;if(qx.core.Environment.get(L)===K&&qx.core.Environment.get(y)<9&&this.__Kk.readyState>0){this.__Kk.setRequestHeader(g,k);}
;if(qx.core.Environment.get(L)===I&&parseInt(qx.core.Environment.get(h),10)<2&&!this.__Ki){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(A);}
;}
,setRequestHeader:function(bg,bh){this.__Ky();if(bg==J||bg==g||bg==d||bg==Q){this.__Kh=true;}
;this.__Kk.setRequestHeader(bg,bh);return this;}
,send:function(bj){this.__Ky();if(!this.__Ki&&this.__Kj){throw this.__Kj;}
;if(qx.core.Environment.get(L)===E&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__mH=window.setTimeout(this.__zi,this.timeout);}
;bj=typeof bj==V?null:bj;var bi=qx.Bootstrap.getClass(bj);bj=(bj!==null&&this.__Kl.indexOf(bi)===-1)?bj.toString():bj;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,O);}
;this.__Kk.send(bj);}
catch(bl){if(!this.__Ki){throw bl;}
;if(this._getProtocol()===N){this.readyState=2;this.__Kq();var bk=this;window.setTimeout(function(){if(bk.__sH){return;}
;bk.readyState=3;bk.__Kq();bk.readyState=4;bk.__Kq();}
);}
;}
;if(qx.core.Environment.get(L)===I&&!this.__Ki){this.__Kp();}
;this.__Kg=true;return this;}
,abort:function(){this.__Ky();this.__zk=true;this.__Kk.abort();if(this.__Kk&&this.readyState!==qx.bom.request.Xhr.DONE){this.readyState=this.__Kk.readyState;}
;return this;}
...
,getResponseHeader:function(bo){this.__Ky();if(qx.core.Environment.get(y)===9&&this.__Kk.aborted){return z;}
;return this.__Kk.getResponseHeader(bo);}
,getAllResponseHeaders:function(){this.__Ky();if(qx.core.Environment.get(y)===9&&this.__Kk.aborted){return z;}
;return this.__Kk.getAllResponseHeaders();}
,overrideMimeType:function(bp){this.__Ky();if(this.__Kk.overrideMimeType){this.__Kk.overrideMimeType(bp);}
else {throw new Error(X);}
;return this;}
,getRequest:function(){return this.__Kk;}
,dispose:function(){if(this.__sH){return false;}
;window.clearTimeout(this.__mH);if(window.detachEvent){window.detachEvent(i,this.__Kf);}
;try{this.__Kk.onreadystatechange;}
catch(br){return false;}
;var bq=function(){}
;this.__Kk.onreadystatechange=bq;this.__Kk.onload=bq;this.__Kk.onerror=bq;this.__Kk.onprogress=bq;this.abort();this.__Kk=null;this.__sH=true;return true;}
...
,_getProtocol:function(){var bt=this.__sC;var bu=/^(\w+:)\/\//;if(bt!==null&&bt.match){var bv=bt.match(bu);if(bv&&bv[1]){return bv[1];}
;}
;return window.location.protocol;}
,__Kk:null,__Ki:null,__Kc:null,__Kd:null,__Ke:null,__Kf:null,__zi:null,__Kg:null,__sC:null,__zk:null,__sF:null,__sH:null,__mH:null,__Kj:null,__Kh:null,__Kl:null,__Km:function(){this.__Kk=this._createNativeXhr();this.__Kk.onreadystatechange=this.__Kc;if(qx.Bootstrap.getClass(this.__Kk.onabort)!==m){this.__Kk.onabort=this.__Kd;}
;if(qx.Bootstrap.getClass(this.__Kk.onprogress)!==m){this.__Kk.onprogress=this.__Ke;this.progress={lengthComputable:false,loaded:0,total:0};}
;this.__sH=this.__Kg=this.__zk=false;this.__Kl=[U,q,r,x,P];}
,__Kn:function(){if(!this.__zk){this.abort();}
;}
,__Ko:function(e){this.progress.lengthComputable=e.lengthComputable;this.progress.loaded=e.loaded;this.progress.total=e.total;this._emit(s);}
,__Kp:function(){var bw=this.__Kk,bx=true;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,u+bw.readyState);}
;if(this.readyState==bw.readyState){return;}
;this.readyState=bw.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__zk&&!this.__Kg){return;}
;if(!this.__Ki&&(bw.readyState==2||bw.readyState==3)){return;}
...
;if(bx){this.__Ku();this.__Kv();}
;}
;this.__Kq();if(this.readyState==qx.bom.request.Xhr.DONE){if(bw){bw.onreadystatechange=function(){}
;}
;}
;}
,__Kq:function(){if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__mH);}
;this._emit(A);if(this.readyState===qx.bom.request.Xhr.DONE){this.__Kr();}
;}
,__Kr:function(){if(this.__sF){this._emit(H);if(qx.core.Environment.get(L)===E){this._emit(o);}
;this.__sF=false;}
else {if(this.__zk){this._emit(v);}
else {if(this.__Ks()){this._emit(o);}
else {this._emit(t);}
;}
;}
;this._emit(p);}
,__Ks:function(){var bz;if(this._getProtocol()===N){bz=!this.responseText;}
else {bz=!this.statusText&&this.status!==204;}
;return bz;}
,__Kt:function(){var bA=this.__Kk;this.readyState=qx.bom.request.Xhr.DONE;this.__sF=true;bA.aborted=true;bA.abort();this.responseText=z;this.responseXML=null;this.__Kq();}
,__Ku:function(){var bB=this.readyState===qx.bom.request.Xhr.DONE;if(this._getProtocol()===N&&this.status===0&&bB){if(!this.__Ks()){this.status=200;}
;}
;if(this.status===1223){this.status=204;}
;if(qx.core.Environment.get(L)===E){if(bB&&this.__Kh&&!this.__zk&&this.status===0){this.status=304;}
;}
;}
,__Kv:function(){if(qx.core.Environment.get(L)==K&&(this.getResponseHeader(R)||z).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bC=new window.ActiveXObject(M);bC.async=false;bC.validateOnParse=false;bC.loadXML(this.responseText);this.responseXML=bC;}
;}
,__Kw:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__Kx:function(){var name=qx.core.Environment.get(L);var bD=qx.core.Environment.get(D);return !(name==K&&bD<9||name==I&&bD<3.5);}
,__Ky:function(){if(this.__sH){throw new Error(C);}
;}
...
;}
,statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},members:{__sB:null,parse:function(h,g){var f=this._getParser(g);if(typeof f===a){if(h!==c){return f.call(this,h);}
;}
;return h;}
,setParser:function(i){if(typeof qx.util.ResponseParser.PARSER[i]===a){return this.__sB=qx.util.ResponseParser.PARSER[i];}
;if(qx.core.Environment.get(d)){qx.core.Assert.assertFunction(i);}
;return this.__sB=i;}
,_getParser:function(k){var j=this.__sB,m=c,l=c;if(j){return j;}
...
(function(){var a="mouse",b="scrollbar-y",c="qx.ui.core.scroll.MRoll",d="pointerdown",f="scrollbar-x",g="roll";qx.Mixin.define(c,{members:{__R:null,_addRollHandling:function(){this.addListener(g,this._onRoll,this);this.addListener(d,this._onPointerDownForRoll,this);}
...
,_onPointerDownForRoll:function(e){this.__R=e.getPointerId();}
,_onRoll:function(e){if(e.getPointerType()==a){return;}
;if(this.__R&&e.getMomentum()){qx.event.Registration.getManager(e.getOriginalTarget()).getHandler(qx.event.handler.Gesture).gestureCancel(this.__R);e.stopMomentum();this.__R=null;return;}
;this.__R=null;var k=this._isChildControlVisible(f);var l=this._isChildControlVisible(b);var q=l?this.getChildControl(b,true):null;var p=k?this.getChildControl(f,true):null;var m=e.getDelta().y;var h=e.getDelta().x;var j=!l;var o=!k;if(q){if(m!==0){q.scrollBy(parseInt(m,10));}
...
;r.addListener(k,this.__ch,this);r.addListener(i,this.__ci,this);this.__ce=[n,o];this.__cf=[m,g];}
,properties:{dragScrollThresholdX:{check:p,init:30},dragScrollThresholdY:{check:p,init:30},dragScrollSlowDownFactor:{check:b,init:0.1}},members:{__cg:null,__ce:null,__cf:null,_findScrollableParent:function(s){var t=s;if(t===null){return null;}
...
;}
,_getAxis:function(A){if(this.__ce.indexOf(A)!==-1){return a;}
else if(this.__cf.indexOf(A)!==-1){return q;}
else {throw new Error(l+A+f);}
;}
,_getThresholdByEdgeType:function(B){if(this.__ce.indexOf(B)!==-1){return this.getDragScrollThresholdX();}
else if(this.__cf.indexOf(B)!==-1){return this.getDragScrollThresholdY();}
;}
...
;var P=S.getBounds(),O=T===a?P.width:P.height,R=this._calculateScrollAmount(O,Q);if(this._isScrollbarExceedingMaxPos(S,T,R)){this.__cg.stop();}
;S.scrollBy(R);}
,__ch:function(e){if(this.__cg){this.__cg.stop();}
...
;bd=this._getAxis(X);if(this._isScrollbarVisible(U,bd)){bb=this._calculateThresholdExceedance(bc[X],this._getThresholdByEdgeType(X));if(this.__cg){this.__cg.dispose();}
;this.__cg=new qx.event.Timer(50);this.__cg.addListener(h,function(be,bg,bf){this._scrollBy(be,bg,bf);}
.bind(this,U,bd,bb));this.__cg.start();e.stopPropagation();return;}
else {U=this._findScrollableParent(U);}
;}
;}
,__ci:function(e){if(this.__cg){this.__cg.stop();}
;}
...
(function(){var a="qx.ui.core.scroll.NativeScrollBar",b="px",c="PositiveNumber",d="frame",f="end",g="box-sizing",h="Integer",i="track",j="browser.name",k="hidden",l="content-box",m="pointerup",n="vertical",o="_applyOrientation",p="scrollAnimationEnd",q="appear",r="__co",s="pointerdown",t="PositiveInteger",u='qx.event.type.Event',v="mshtml",w="engine.name",x="horizontal",y="Number",z="edge",A="_applyPosition",B="scrollbar",C="_applyMaximum",D="pointermove",E="native",F="scroll";qx.Class.define(a,{extend:qx.ui.core.Widget,implement:qx.ui.core.scroll.IScrollBar,construct:function(G){qx.ui.core.Widget.call(this);this.addState(E);this.getContentElement().addListener(F,this._onScroll,this);this.addListener(s,this._stopPropagation,this);this.addListener(m,this._stopPropagation,this);this.addListener(D,this._stopPropagation,this);this.addListener(q,this._onAppear,this);this.getContentElement().add(this._getScrollPaneElement());this.getContentElement().setStyle(g,l);if(G!=null){this.setOrientation(G);}
else {this.initOrientation();}
;this.addListener(i,function(e){e.stopPropagation();}
,this);}
,events:{scrollAnimationEnd:u},properties:{appearance:{refine:true,init:B},orientation:{check:[x,n],init:x,apply:o},maximum:{check:t,apply:C,init:100},position:{check:y,init:0,apply:A,event:F},singleStep:{check:h,init:20},knobFactor:{check:c,nullable:true}},members:{__bV:null,__co:null,__cp:null,__cq:null,_getScrollPaneElement:function(){if(!this.__co){this.__co=new qx.html.Element();}
;return this.__co;}
...
,_getContentHint:function(){var L=qx.bom.element.Scroll.getScrollbarWidth();return {width:this.__bV?100:L,maxWidth:this.__bV?null:L,minWidth:this.__bV?null:L,height:this.__bV?L:100,maxHeight:this.__bV?L:null,minHeight:this.__bV?L:null};}
...
,_applyPosition:function(P){var content=this.getContentElement();if(this.__bV){content.scrollToX(P);}
else {content.scrollToY(P);}
;}
,_applyOrientation:function(S,Q){var R=this.__bV=S===x;this.set({allowGrowX:R,allowShrinkX:R,allowGrowY:!R,allowShrinkY:!R});if(R){this.replaceState(n,x);}
...
,_updateScrollBar:function(){var U=this.__bV;var V=this.getBounds();if(!V){return;}
...
,scrollTo:function(Y,X){this.stopScrollAnimation();if(X){var ba=this.getPosition();this.__cq=new qx.bom.AnimationFrame();this.__cq.on(d,function(bc){var bb=parseInt(bc/X*(Y-ba)+ba);this.updatePosition(bb);}
,this);this.__cq.on(f,function(){this.setPosition(Math.max(0,Math.min(this.getMaximum(),Y)));this.__cq=null;this.fireEvent(p);}
,this);this.__cq.startSequence(X);}
...
,stopScrollAnimation:function(){if(this.__cq){this.__cq.cancelSequence();this.__cq=null;}
;}
,_onScroll:function(e){var bj=this.getContentElement();var bk=this.__bV?bj.getScrollX():bj.getScrollY();this.setPosition(bk);}
...
,this);}
,events:{"scrollAnimationEnd":l},properties:{appearance:{refine:true,init:A},orientation:{check:[x,r],init:x,apply:s},maximum:{check:w,apply:B,init:100},position:{check:i,init:0,apply:z,event:F},singleStep:{check:f,init:20},pageStep:{check:f,init:10,apply:u},knobFactor:{check:c,apply:j,nullable:true}},members:{__eQ:2,__eR:0,_computeSizeHint:function(){var H=qx.ui.core.Widget.prototype._computeSizeHint.call(this);if(this.getOrientation()===x){this.__eR=H.minWidth;H.minWidth=0;}
else {this.__eR=H.minHeight;H.minHeight=0;}
;return H;}
,renderLayout:function(M,top,J,I){var L=qx.ui.core.Widget.prototype.renderLayout.call(this,M,top,J,I);var K=this.getOrientation()===x;if(this.__eR>=(K?J:I)){this.getChildControl(n).setVisibility(o);this.getChildControl(D).setVisibility(o);}
...
,_onResizeSlider:function(e){var bg=this.getChildControl(a).getChildControl(m);var bj=bg.getSizeHint();var bh=false;var bi=this.getChildControl(a).getInnerSize();if(this.getOrientation()==r){if(bi.height<bj.minHeight+this.__eQ){bh=true;}
;}
else {if(bi.width<bj.minWidth+this.__eQ){bh=true;}
;}
;if(bh){bg.exclude();}
else {bg.show();}
;}
...
(function(){var a="",b="qx.ui.form.MForm",c="Boolean",d="_applyValid",f="changeLocale",g="changeRequired",h="changeValid",i="qx.dynlocale",j="changeInvalidMessage",k="String",l="invalid";qx.Mixin.define(b,{construct:function(){if(qx.core.Environment.get(i)){qx.locale.Manager.getInstance().addListener(f,this.__HW,this);}
;}
...
,__HW:qx.core.Environment.select(i,{"true":function(e){var o=this.getInvalidMessage();if(o&&o.translate){this.setInvalidMessage(o.translate());}
;var p=this.getRequiredInvalidMessage();if(p&&p.translate){this.setRequiredInvalidMessage(p.translate());}
;}
,"false":null})},destruct:function(){if(qx.core.Environment.get(i)){qx.locale.Manager.getInstance().removeListener(f,this.__HW,this);}
;}
});}
)();
...
,_onPointerDown:function(e){if(this.__fi){return;}
;var bw=this.__bV;var bu=this.getChildControl(P);var bv=bw?G:L;var bA=bw?e.getDocumentLeft():e.getDocumentTop();var bC=this.getDecorator();bC=qx.theme.manager.Decoration.getInstance().resolve(bC);if(bw){var bz=bC?bC.getInsets().left:0;var by=(this.getPaddingLeft()||0)+bz;}
else {var bz=bC?bC.getInsets().top:0;var by=(this.getPaddingTop()||0)+bz;}
;var bB=this.__ff=qx.bom.element.Location.get(this.getContentElement().getDomElement())[bv];bB+=by;var bx=this.__fg=qx.bom.element.Location.get(bu.getContentElement().getDomElement())[bv];if(e.getTarget()===bu){this.__fi=true;if(!this.__fn){this.__fn=new qx.event.Timer(100);this.__fn.addListener(r,this._fireValue,this);}
;this.__fn.start();this.__fj=bA+bB-bx;bu.addState(c);}
else {this.__fk=true;this.__fl=bA<=bx?-1:1;this.__fr(e);this._onInterval();if(!this.__do){this.__do=new qx.event.Timer(100);this.__do.addListener(r,this._onInterval,this);}
;this.__do.start();}
;this.addListener(w,this._onPointerMove);this.capture();e.stopPropagation();}
,_onPointerUp:function(e){if(this.__fi){this.releaseCapture();delete this.__fi;this.__fn.stop();this._fireValue();delete this.__fj;this.getChildControl(P).removeState(c);if(e.getType()===B){var bE;var bF;var bD;if(this.__bV){bE=e.getDocumentLeft()-(this._valueToPosition(this.getValue())+this.__ff);bD=qx.bom.element.Location.get(this.getContentElement().getDomElement())[L];bF=e.getDocumentTop()-(bD+this.getChildControl(P).getBounds().top);}
else {bE=e.getDocumentTop()-(this._valueToPosition(this.getValue())+this.__ff);bD=qx.bom.element.Location.get(this.getContentElement().getDomElement())[G];bF=e.getDocumentLeft()-(bD+this.getChildControl(P).getBounds().left);}
;if(bF<0||bF>this.__fh||bE<0||bE>this.__fh){this.getChildControl(P).removeState(b);}
;}
;}
else if(this.__fk){this.__do.stop();this.releaseCapture();delete this.__fk;delete this.__fl;delete this.__fm;}
;this.removeListener(w,this._onPointerMove);if(e.getType()===B){e.stopPropagation();}
;}
,_onPointerMove:function(e){if(this.__fi){var bH=this.__bV?e.getDocumentLeft():e.getDocumentTop();var bG=bH-this.__fj;this.slideTo(this._positionToValue(bG));}
else if(this.__fk){this.__fr(e);}
;e.stopPropagation();}
,_onInterval:function(e){var bI=this.getValue()+(this.__fl*this.getPageStep());if(bI<this.getMinimum()){bI=this.getMinimum();}
else if(bI>this.getMaximum()){bI=this.getMaximum();}
;var bJ=this.__fl==-1;if((bJ&&bI<=this.__fm)||(!bJ&&bI>=this.__fm)){bI=this.__fm;}
;this.slideTo(bI);}
,_onUpdate:function(e){var bL=this.getInnerSize();var bM=this.getChildControl(P).getBounds();var bK=this.__bV?E:x;this._updateKnobSize();this.__fq=bL[bK]-bM[bK];this.__fh=bM[bK];this._updateKnobPosition();}
,__bV:false,__fq:0,__fr:function(e){var bN=this.__bV;var bU=bN?e.getDocumentLeft():e.getDocumentTop();var bX=this.__ff;var bO=this.__fg;var bY=this.__fh;var bV=bU-bX;if(bU>=bO){bV-=bY;}
;var bR=this._positionToValue(bV);var bP=this.getMinimum();var bQ=this.getMaximum();if(bR<bP){bR=bP;}
else if(bR>bQ){bR=bQ;}
else {var bW=this.getValue();var bT=this.getPageStep();var bS=this.__fl<0?I:Q;bR=bW+(Math[bS]((bR-bW)/bT)*bT);}
;if(this.__fm==null||(this.__fl==-1&&bR<=this.__fm)||(this.__fl==1&&bR>=this.__fm)){this.__fm=bR;}
;}
,_positionToValue:function(cb){var ca=this.__fq;if(ca==null||ca==0){return 0;}
;var cd=cb/ca;if(cd<0){cd=0;}
else if(cd>1){cd=1;}
;var cc=this.getMaximum()-this.getMinimum();return this.getMinimum()+Math.round(cc*cd);}
,_valueToPosition:function(ch){var ce=this.__fq;if(ce==null){return 0;}
...
,_setKnobPosition:function(ck){var ci=this.getChildControl(P);var cj=this.getDecorator();cj=qx.theme.manager.Decoration.getInstance().resolve(cj);var content=ci.getContentElement();if(this.__bV){if(cj&&cj.getPadding()){ck+=cj.getPadding().left;}
...
;if(this.__bV){this.getChildControl(P).setWidth(Math.round(cm*cl.width));}
...
,slideTo:function(cu,ct){this.stopSlideAnimation();if(ct){this.__ft(cu,ct);}
else {this.updatePosition(cu);}
;}
,updatePosition:function(cv){this.setValue(this.__fs(cv));}
,stopSlideAnimation:function(){if(this.__cq){this.__cq.cancelSequence();this.__cq=null;}
;}
,__fs:function(cw){if(cw<this.getMinimum()){cw=this.getMinimum();}
...
;return cw;}
,__ft:function(cy,cx){cy=this.__fs(cy);var cz=this.getValue();this.__cq=new qx.bom.AnimationFrame();this.__cq.on(i,function(cA){this.setValue(parseInt(cA/cx*(cy-cz)+cz));}
,this);this.__cq.on(j,function(){this.setValue(cy);this.__cq=null;this.fireEvent(O);}
,this);this.__cq.startSequence(cx);}
,_applyOrientation:function(cD,cC){var cB=this.getChildControl(P);this.__bV=cD===N;if(this.__bV){this.removeState(C);cB.removeState(C);this.addState(N);cB.addState(N);cB.setLayoutProperties({top:0,right:null,bottom:0});}
...
else {if(this.__bV){this.getChildControl(P).resetWidth();}
else {this.getChildControl(P).resetHeight();}
;}
;}
,_applyValue:function(cH,cG){if(cH!=null){this._updateKnobPosition();if(this.__fi){this.__fp=[cH,cG];}
else {this.fireEvent(h,qx.event.type.Data,[cH,cG]);}
;}
else {this.resetValue();}
;}
,_fireValue:function(){if(!this.__fp){return;}
;var cI=this.__fp;this.__fp=null;this.fireEvent(h,qx.event.type.Data,cI);}
...
(function(){var a="press",b="hovered",c="qx.ui.form.RepeatButton",d="release",f="Enter",g="pressed",h="interval",i="__do",j="qx.event.type.Event",k="Space",l="abandoned",m="Integer",n="execute";qx.Class.define(c,{extend:qx.ui.form.Button,construct:function(o,p){qx.ui.form.Button.call(this,o,p);this.__do=new qx.event.AcceleratingTimer();this.__do.addListener(h,this._onInterval,this);}
,events:{"execute":j,"press":j,"release":j},properties:{interval:{check:m,init:100},firstInterval:{check:m,init:500},minTimer:{check:m,init:20},timerDecrease:{check:m,init:2}},members:{__KR:null,__do:null,press:function(){if(this.isEnabled()){if(!this.hasState(g)){this.__KS();}
;this.removeState(l);this.addState(g);}
;}
,release:function(q){if(!this.isEnabled()){return;}
;if(this.hasState(g)){if(!this.__KR){this.execute();}
;}
;this.removeState(g);this.removeState(l);this.__KT();}
...
;}
,_onPointerOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;if(this.hasState(l)){this.removeState(l);this.addState(g);this.__do.start();}
;this.addState(b);}
,_onPointerOut:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;this.removeState(b);if(this.hasState(g)){this.removeState(g);this.addState(l);this.__do.stop();}
;}
,_onPointerDown:function(e){if(!e.isLeftPressed()){return;}
;this.capture();this.__KS();e.stopPropagation();}
,_onPointerUp:function(e){this.releaseCapture();if(!this.hasState(l)){this.addState(b);if(this.hasState(g)&&!this.__KR){this.execute();}
;}
;this.__KT();e.stopPropagation();}
,_onTap:function(e){}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case f:case k:if(this.hasState(g)){if(!this.__KR){this.execute();}
;this.removeState(g);this.removeState(l);e.stopPropagation();this.__KT();}
;};}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case f:case k:this.removeState(l);this.addState(g);e.stopPropagation();this.__KS();};}
,_onInterval:function(e){this.__KR=true;this.fireEvent(n);}
,__KS:function(){this.fireEvent(a);this.__KR=false;this.__do.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.removeState(l);this.addState(g);}
,__KT:function(){this.fireEvent(d);this.__do.stop();this.removeState(l);this.removeState(g);}
},destruct:function(){this._disposeObjects(i);}
});}
)();
(function(){var a="Integer",b="__do",c="interval",d="qx.event.type.Event",e="qx.event.AcceleratingTimer";qx.Class.define(e,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__do=new qx.event.Timer(this.getInterval());this.__do.addListener(c,this._onInterval,this);}
,events:{"interval":d},properties:{interval:{check:a,init:100},firstInterval:{check:a,init:500},minimum:{check:a,init:20},decrease:{check:a,init:2}},members:{__do:null,__KU:null,start:function(){this.__do.setInterval(this.getFirstInterval());this.__do.start();}
,stop:function(){this.__do.stop();this.__KU=null;}
,_onInterval:function(){this.__do.stop();if(this.__KU==null){this.__KU=this.getInterval();}
;this.__KU=Math.max(this.getMinimum(),this.__KU-this.getDecrease());this.__do.setInterval(this.__KU);this.__do.start();this.fireEvent(c);}
...
(function(){var a="' must be defined!",b="height",c="vAlign",d="hAlign",e="' is not supported by the Grid layout!",f="bottom",g="Invalid parameter 'column'",h="Integer",m="The property '",n="'",o="_applyLayoutChange",p="qx.debug",q="Value must be positive",r="center",s="qx.ui.layout.Grid",t="middle",u="maxHeight",v="Cannot add widget '",w="width",z=") for '",A="'!. ",B="top",C="minHeight",D="right",E="' in this cell (",F=", ",G="The layout properties 'row' and 'column' of the child widget '",H="minWidth",I="flex",J="left",K="maxWidth",L="Invalid parameter 'row'",M="There is already a widget '";qx.Class.define(s,{extend:qx.ui.layout.Abstract,construct:function(O,N){qx.ui.layout.Abstract.call(this);this.__vT=[];this.__vU=[];if(O){this.setSpacingX(O);}
;if(N){this.setSpacingY(N);}
;}
,properties:{spacingX:{check:h,init:0,apply:o},spacingY:{check:h,init:0,apply:o}},members:{__jb:null,__vT:null,__vU:null,__vV:null,__vW:null,__vX:null,__vY:null,__wa:null,__wb:null,verifyLayoutProperty:qx.core.Environment.select(p,{"true":function(P,name,Q){var R={"row":1,"column":1,"rowSpan":1,"colSpan":1};this.assert(R[name]==1,m+name+e);this.assertInteger(Q);this.assert(Q>=0,q);}
,"false":null}),__wc:function(){var Y=[];var T=[];var W=[];var V=-1;var S=-1;var X=this._getLayoutChildren();for(var i=0,l=X.length;i<l;i++ ){var ba=X[i];var bb=ba.getLayoutProperties();var bc=bb.row;var U=bb.column;bb.colSpan=bb.colSpan||1;bb.rowSpan=bb.rowSpan||1;if(bc==null||U==null){throw new Error(G+ba+a);}
...
;}
;this.__jb=Y;this.__vV=T;this.__vW=W;this.__vX=V;this.__vY=S;this.__wa=null;this.__wb=null;delete this._invalidChildrenCache;}
,_setRowData:function(bg,be,bf){var bd=this.__vT[bg];if(!bd){this.__vT[bg]={};this.__vT[bg][be]=bf;}
else {bd[be]=bf;}
;}
,_setColumnData:function(bh,bj,bk){var bi=this.__vU[bh];if(!bi){this.__vU[bh]={};this.__vU[bh][bj]=bk;}
else {bi[bj]=bk;}
;}
...
,getColumnAlign:function(bp){var bq=this.__vU[bp]||{};return {vAlign:bq.vAlign||B,hAlign:bq.hAlign||J};}
...
,getRowAlign:function(bv){var bu=this.__vT[bv]||{};return {vAlign:bu.vAlign||B,hAlign:bu.hAlign||J};}
,getCellWidget:function(bx,bw){if(this._invalidChildrenCache){this.__wc();}
;var bx=this.__jb[bx]||{};return bx[bw]||null;}
,getRowCount:function(){if(this._invalidChildrenCache){this.__wc();}
;return this.__vX+1;}
,getColumnCount:function(){if(this._invalidChildrenCache){this.__wc();}
;return this.__vY+1;}
,getCellAlign:function(bF,bz){var bE=B;var bC=J;var bD=this.__vT[bF];var bA=this.__vU[bz];var by=this.__jb[bF][bz];if(by){var bB={vAlign:by.getAlignY(),hAlign:by.getAlignX()};}
else {bB={};}
...
,getColumnFlex:function(bI){var bJ=this.__vU[bI]||{};return bJ.flex!==undefined?bJ.flex:0;}
,setRowFlex:function(bL,bK){this._setRowData(bL,I,bK);this._applyLayoutChange();return this;}
,getRowFlex:function(bO){var bM=this.__vT[bO]||{};var bN=bM.flex!==undefined?bM.flex:0;return bN;}
,setColumnMaxWidth:function(bP,bQ){this._setColumnData(bP,K,bQ);this._applyLayoutChange();return this;}
,getColumnMaxWidth:function(bR){var bS=this.__vU[bR]||{};return bS.maxWidth!==undefined?bS.maxWidth:Infinity;}
,setColumnWidth:function(bT,bU){this._setColumnData(bT,w,bU);this._applyLayoutChange();return this;}
,getColumnWidth:function(bV){var bW=this.__vU[bV]||{};return bW.width!==undefined?bW.width:null;}
,setColumnMinWidth:function(bX,bY){this._setColumnData(bX,H,bY);this._applyLayoutChange();return this;}
,getColumnMinWidth:function(ca){var cb=this.__vU[ca]||{};return cb.minWidth||0;}
,setRowMaxHeight:function(cd,cc){this._setRowData(cd,u,cc);this._applyLayoutChange();return this;}
,getRowMaxHeight:function(cf){var ce=this.__vT[cf]||{};return ce.maxHeight||Infinity;}
,setRowHeight:function(cg,ch){this._setRowData(cg,b,ch);this._applyLayoutChange();return this;}
,getRowHeight:function(cj){var ci=this.__vT[cj]||{};return ci.height!==undefined?ci.height:null;}
,setRowMinHeight:function(cl,ck){this._setRowData(cl,C,ck);this._applyLayoutChange();return this;}
,getRowMinHeight:function(cn){var cm=this.__vT[cn]||{};return cm.minHeight||0;}
,__wd:function(cp){var co=cp.getSizeHint();var cr=cp.getMarginLeft()+cp.getMarginRight();var cq=cp.getMarginTop()+cp.getMarginBottom();var cs={height:co.height+cq,width:co.width+cr,minHeight:co.minHeight+cq,minWidth:co.minWidth+cr,maxHeight:co.maxHeight+cq,maxWidth:co.maxWidth+cr};return cs;}
,_fixHeightsRowSpan:function(cN){var cz=this.getSpacingY();for(var i=0,l=this.__vW.length;i<l;i++ ){var cF=this.__vW[i];var cC=this.__wd(cF);var cv=cF.getLayoutProperties();var cB=cv.row;var cK=cz*(cv.rowSpan-1);var ct=cK;var cw={};for(var j=0;j<cv.rowSpan;j++ ){var cA=cv.row+j;var cJ=cN[cA];var cL=this.getRowFlex(cA);if(cL>0){cw[cA]={min:cJ.minHeight,value:cJ.height,max:cJ.maxHeight,flex:cL};}
...
;}
;}
,_fixWidthsColSpan:function(cR){var cS=this.getSpacingX();for(var i=0,l=this.__vV.length;i<l;i++ ){var cO=this.__vV[i];var cQ=this.__wd(cO);var cU=cO.getLayoutProperties();var cP=cU.column;var db=cS*(cU.colSpan-1);var cT=db;var cV={};var cX;for(var j=0;j<cU.colSpan;j++ ){var cW=cU.column+j;var da=cR[cW];var cY=this.getColumnFlex(cW);if(cY>0){cV[cW]={min:da.minWidth,value:da.width,max:da.maxWidth,flex:cY};}
...
;}
;}
,_getRowHeights:function(){if(this.__wa!=null){return this.__wa;}
;var dm=[];var df=this.__vX;var de=this.__vY;for(var dn=0;dn<=df;dn++ ){var dg=0;var di=0;var dh=0;for(var dl=0;dl<=de;dl++ ){var dd=this.__jb[dn][dl];if(!dd){continue;}
;var dj=dd.getLayoutProperties().rowSpan||0;if(dj>1){continue;}
;var dk=this.__wd(dd);if(this.getRowFlex(dn)>0){dg=Math.max(dg,dk.minHeight);}
...
;if(this.__vW.length>0){this._fixHeightsRowSpan(dm);}
;this.__wa=dm;return dm;}
,_getColWidths:function(){if(this.__wb!=null){return this.__wb;}
;var dt=[];var dq=this.__vY;var ds=this.__vX;for(var dy=0;dy<=dq;dy++ ){var dw=0;var dv=0;var dr=Infinity;for(var dz=0;dz<=ds;dz++ ){var dp=this.__jb[dz][dy];if(!dp){continue;}
;var du=dp.getLayoutProperties().colSpan||0;if(du>1){continue;}
;var dx=this.__wd(dp);dv=Math.max(dv,dx.minWidth);dw=Math.max(dw,dx.width);}
...
;if(this.__vV.length>0){this._fixWidthsColSpan(dt);}
;this.__wb=dt;return dt;}
...
,renderLayout:function(ej,dO,ei){if(this._invalidChildrenCache){this.__wc();}
;var ed=qx.ui.layout.Util;var dQ=this.getSpacingX();var dW=this.getSpacingY();var eh=this._getColWidths();var ek=this._getColumnFlexOffsets(ej);var dR=[];var em=this.__vY;var dP=this.__vX;var el;for(var en=0;en<=em;en++ ){el=ek[en]?ek[en].offset:0;dR[en]=eh[en].width+el;}
...
;var er=0;for(var en=0;en<=em;en++ ){var top=0;for(var dX=0;dX<=dP;dX++ ){var ef=this.__jb[dX][en];if(!ef){top+=et[dX]+dW;continue;}
...
;er+=dR[en]+dQ;}
;}
,invalidateLayoutCache:function(){qx.ui.layout.Abstract.prototype.invalidateLayoutCache.call(this);this.__wb=null;this.__wa=null;}
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__wc();}
...
},destruct:function(){this.__jb=this.__vT=this.__vU=this.__vV=this.__vW=this.__wb=this.__wa=null;}
});}
)();
...
,events:{update:n,scrollAnimationEnd:n},properties:{scrollX:{check:j,apply:g,transform:d,event:f,init:0},scrollY:{check:m,apply:h,transform:k,event:b,init:0}},members:{__hW:null,add:function(t){var u=this._getChildren()[0];if(u){this._remove(u);u.removeListener(a,this._onUpdate,this);}
...
;this.stopScrollAnimation();if(N){var P=this.getScrollX();this.__hW=new qx.bom.AnimationFrame();this.__hW.on(p,function(){this.setScrollX(Q);this.__hW=null;this.fireEvent(c);}
,this);this.__hW.on(i,function(S){var R=parseInt(S/N*(Q-P)+P);this.setScrollX(R);}
,this);this.__hW.startSequence(N);}
else {this.setScrollX(Q);}
;}
,scrollToY:function(W,T){var U=this.getScrollMaxY();if(W<0){W=0;}
else if(W>U){W=U;}
;this.stopScrollAnimation();if(T){var V=this.getScrollY();this.__hW=new qx.bom.AnimationFrame();this.__hW.on(p,function(){this.setScrollY(W);this.__hW=null;this.fireEvent(c);}
,this);this.__hW.on(i,function(Y){var X=parseInt(Y/T*(W-V)+V);this.setScrollY(X);}
,this);this.__hW.startSequence(T);}
...
,stopScrollAnimation:function(){if(this.__hW){this.__hW.cancelSequence();this.__hW=null;}
;}
...
(function(){var a="scrollY",b="The method 'getItemRight' is not implemented!",c="update",d="scrollX",f="The method 'getItemLeft' is not implemented!",g="The method 'getItemBottom' is not implemented!",h="The method 'getItemTop' is not implemented!",i="pane",j="os.scrollBarOverlayed",k="qx.ui.virtual.core.Scroller";qx.Class.define(k,{extend:qx.ui.core.scroll.AbstractScrollArea,construct:function(o,l,n,m){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__ny=new qx.ui.virtual.core.Pane(o,l,n,m);this.__ny.addListener(c,this._computeScrollbars,this);this.__ny.addListener(d,this._onScrollPaneX,this);this.__ny.addListener(a,this._onScrollPaneY,this);if(qx.core.Environment.get(j)){this._add(this.__ny,{edge:0});}
else {this._add(this.__ny,{row:0,column:0});}
;}
,properties:{width:{refine:true,init:null},height:{refine:true,init:null}},members:{__ny:null,getPane:function(){return this.__ny;}
,_createChildControlImpl:function(q,p){if(q==i){return this.__ny;}
...
,_onScrollBarX:function(e){this.__ny.setScrollX(e.getData());}
,_onScrollBarY:function(e){this.__ny.setScrollY(e.getData());}
},destruct:function(){this.__ny.dispose();this.__ny=null;}
});}
)();
(function(){var a="scrollY",b="cellTap",c="qx.event.type.Data",d="__ym",f="change",g="qx.debug",h="qx.event.type.Event",j="__ye",k="cellContextmenu",l="dbltap",m="resize",n="cellDbltap",o="scrollX",p="appear",q="pointerdown",r="qx.ui.virtual.core.Pane",s="qx.ui.virtual.core.CellEvent",t="update",u="contextmenu",v="__yl",w="tap",x="__yf";qx.Class.define(r,{extend:qx.ui.core.Widget,construct:function(B,y,A,z){qx.ui.core.Widget.call(this);this.__ye=new qx.ui.virtual.core.Axis(A,B);this.__yf=new qx.ui.virtual.core.Axis(z,y);this.__yg=0;this.__yh=0;this.__yi=0;this.__yj=0;this.__yk={};this.__sN={};this.__yl=new qx.ui.container.Composite();this.__yl.setUserBounds(0,0,0,0);this._add(this.__yl);this.__ym=[];this.__ye.addListener(f,this.fullUpdate,this);this.__yf.addListener(f,this.fullUpdate,this);this.addListener(m,this._onResize,this);this.addListenerOnce(p,this._onAppear,this);this.addListener(q,this._onPointerDown,this);this.addListener(w,this._onTap,this);this.addListener(l,this._onDbltap,this);this.addListener(u,this._onContextmenu,this);}
,events:{cellTap:s,cellContextmenu:s,cellDbltap:s,update:h,scrollX:c,scrollY:c},properties:{width:{refine:true,init:400},height:{refine:true,init:300}},members:{__ye:null,__yf:null,__yg:null,__yh:null,__yi:null,__yj:null,__yk:null,__sN:null,__yl:null,__ym:null,__yn:null,__cn:null,__yo:null,__yp:null,getRowConfig:function(){return this.__ye;}
,getColumnConfig:function(){return this.__yf;}
,getChildren:function(){return [this.__yl];}
,addLayer:function(C){if(qx.core.Environment.get(g)){this.assertInterface(C,qx.ui.virtual.core.ILayer);}
;this.__ym.push(C);C.setUserBounds(0,0,0,0);this.__yl.add(C);}
,getLayers:function(){return this.__ym;}
,getVisibleLayers:function(){var D=[];for(var i=0;i<this.__ym.length;i++ ){var E=this.__ym[i];if(E.isVisible()){D.push(E);}
;}
;return D;}
,getScrollMaxX:function(){var F=this.getInnerSize();if(F){return Math.max(0,this.__yf.getTotalSize()-F.width);}
;return 0;}
,getScrollMaxY:function(){var G=this.getInnerSize();if(G){return Math.max(0,this.__ye.getTotalSize()-G.height);}
;return 0;}
,setScrollY:function(J){var H=this.getScrollMaxY();if(J<0){J=0;}
else if(J>H){J=H;}
;if(this.__yg!==J){var I=this.__yg;this.__yg=J;this._deferredUpdateScrollPosition();this.fireDataEvent(a,J,I);}
;}
,getScrollY:function(){return this.__yg;}
,setScrollX:function(M){var K=this.getScrollMaxX();if(M<0){M=0;}
else if(M>K){M=K;}
;if(M!==this.__yh){var L=this.__yh;this.__yh=M;this._deferredUpdateScrollPosition();this.fireDataEvent(o,M,L);}
;}
,getScrollX:function(){return this.__yh;}
,getScrollSize:function(){return {width:this.__yf.getTotalSize(),height:this.__ye.getTotalSize()};}
...
,this,0);}
,this);return;}
;var R=this.__ye.getItemPosition(P);var O=R+this.__ye.getItemSize(P);var N=this.getScrollY();if(R<N){this.setScrollY(R);}
...
,this,0);}
,this);return;}
;var U=this.__yf.getItemPosition(S);var T=U+this.__yf.getItemSize(S);var W=this.getScrollX();if(U<W){this.setScrollX(U);}
...
;bf=this.__ye.getItemAtPosition(this.getScrollY()+bc-bd.top);be=this.__yf.getItemAtPosition(this.getScrollX()+bb-bd.left);if(!bf||!be){return null;}
...
;var bn=this.__yh+bl.width;var bo=this.__yj-bn;if(this.__yh-this.__yk.left<Math.min(this.__yh,bj)||this.__yk.right-bn<Math.min(bo,bk)){var bp=Math.min(this.__yh,bm);var bh=Math.min(bo,bi);this._setLayerWindow(bg,this.__yh-bp,this.__yg,bl.width+bp+bh,bl.height,false);}
;}
,prefetchY:function(bz,bv,br,bu){var bq=this.getVisibleLayers();if(bq.length==0){return;}
;var bw=this.getBounds();if(!bw){return;}
;var bs=this.__yg+bw.height;var bt=this.__yi-bs;if(this.__yg-this.__yk.top<Math.min(this.__yg,bz)||this.__yk.bottom-bs<Math.min(bt,br)){var by=Math.min(this.__yg,bv);var bx=Math.min(bt,bu);this._setLayerWindow(bq,this.__yh,this.__yg-by,bw.width,bw.height+by+bx,false);}
;}
,_onResize:function(){if(this.getContentElement().getDomElement()){this.__yn=true;this._updateScrollPosition();this.__yn=null;this.fireEvent(t);}
;}
,_onAppear:function(){this.fullUpdate();}
,_onPointerDown:function(e){this.__yp=this.getCellAtPosition(e.getDocumentLeft(),e.getDocumentTop());}
,_onTap:function(e){this.__yq(e,b);}
,_onContextmenu:function(e){this.__yq(e,k);}
,_onDbltap:function(e){this.__yq(e,n);}
,__yq:function(e,bC){var bB=this.getCellAtPosition(e.getDocumentLeft(),e.getDocumentTop());if(!bB){return;}
;var bA=this.__yp;if(bA==null||bA.row!==bB.row||bA.column!==bB.column){return;}
;this.fireNonBubblingEvent(bC,qx.ui.virtual.core.CellEvent,[this,e,bB.row,bB.column]);}
,syncWidget:function(bD){if(this.__sN._fullUpdate){this._fullUpdate();}
else if(this.__sN._updateScrollPosition){this._updateScrollPosition();}
;this.__sN={};}
,_setLayerWindow:function(bE,bT,top,bP,bL,bV){var bK=this.__ye.getItemAtPosition(top);if(bK){var bN=bK.index;var bR=this.__ye.getItemSizes(bN,bL+bK.offset);var bM=qx.lang.Array.sum(bR);var bU=top-bK.offset;var bQ=top-bK.offset+bM;}
else {var bN=0;var bR=[];var bM=0;var bU=0;var bQ=0;}
;var bF=this.__yf.getItemAtPosition(bT);if(bF){var bJ=bF.index;var bH=this.__yf.getItemSizes(bJ,bP+bF.offset);var bO=qx.lang.Array.sum(bH);var bS=bT-bF.offset;var bI=bT-bF.offset+bO;}
else {var bJ=0;var bH=[];var bO=0;var bS=0;var bI=0;}
;this.__yk={top:bU,bottom:bQ,left:bS,right:bI};this.__yl.setUserBounds((this.getPaddingLeft()||0)+(this.__yk.left-this.__yh),(this.getPaddingTop()||0)+(this.__yk.top-this.__yg),bO,bM);this.__cn=bH;this.__yo=bR;for(var i=0;i<this.__ym.length;i++ ){var bG=this.__ym[i];bG.setUserBounds(0,0,bO,bM);if(bV){bG.fullUpdate(bN,bJ,bR,bH);}
else {bG.updateLayerWindow(bN,bJ,bR,bH);}
;}
;}
,__yr:function(){if(this.__yn){return;}
;var bW=this.getScrollSize();if(this.__yi!==bW.height||this.__yj!==bW.width){this.__yi=bW.height;this.__yj=bW.width;this.fireEvent(t);}
;}
,fullUpdate:function(){this.__sN._fullUpdate=1;qx.ui.core.queue.Widget.add(this);}
,isUpdatePending:function(){return !!this.__sN._fullUpdate;}
,_fullUpdate:function(){var bX=this.getVisibleLayers();if(bX.length==0){this.__yr();return;}
;var bY=this.getBounds();if(!bY){return;}
;this._setLayerWindow(bX,this.__yh,this.__yg,bY.width,bY.height,true);this.__yr();}
,_deferredUpdateScrollPosition:function(){this.__sN._updateScrollPosition=1;qx.ui.core.queue.Widget.add(this);}
,_updateScrollPosition:function(){var ca=this.getVisibleLayers();if(ca.length==0){this.__yr();return;}
;var cc=this.getBounds();if(!cc){return;}
;var cb={top:this.__yg,bottom:this.__yg+cc.height,left:this.__yh,right:this.__yh+cc.width};if(this.__yk.top<=cb.top&&this.__yk.bottom>=cb.bottom&&this.__yk.left<=cb.left&&this.__yk.right>=cb.right){this.__yl.setUserBounds((this.getPaddingLeft()||0)+(this.__yk.left-cb.left),(this.getPaddingTop()||0)+(this.__yk.top-cb.top),this.__yk.right-this.__yk.left,this.__yk.bottom-this.__yk.top);}
else {this._setLayerWindow(ca,this.__yh,this.__yg,cc.width,cc.height,false);}
;this.__yr();}
},destruct:function(){this._disposeArray(d);this._disposeObjects(j,x,v);this.__yk=this.__sN=this.__cn=this.__yo=null;}
});}
)();
...
,events:{"change":e},members:{__ys:null,getDefaultItemSize:function(){return this.defaultItemSize;}
,setDefaultItemSize:function(k){if(this.defaultItemSize!==k){this.defaultItemSize=k;this.__ys=null;this.fireNonBubblingEvent(c);}
;}
,getItemCount:function(){return this.itemCount;}
,setItemCount:function(l){if(this.itemCount!==l){this.itemCount=l;this.__ys=null;this.fireNonBubblingEvent(c);}
;}
...
;this.__ys=null;this.fireNonBubblingEvent(c);}
,getItemSize:function(o){return this.customSizes[o]||this.defaultItemSize;}
,resetItemSizes:function(){this.customSizes={};this.__ys=null;this.fireNonBubblingEvent(c);}
,__yt:function(){if(this.__ys){return this.__ys;}
;var s=this.defaultItemSize;var q=this.itemCount;var y=[];for(var v in this.customSizes){var p=parseInt(v,10);if(p<q){y.push(p);}
;}
;if(y.length==0){var u=[{startIndex:0,endIndex:q-1,firstItemSize:s,rangeStart:0,rangeEnd:q*s-1}];this.__ys=u;return u;}
...
;var z=u[u.length-1];var w=(q-z.startIndex-1)*s;z.rangeEnd=z.rangeStart+z.firstItemSize+w-1;z.endIndex=q-1;this.__ys=u;return u;}
,__yu:function(F){var B=this.__ys||this.__yt();var C=0;var A=B.length-1;while(true){var E=C+((A-C)>>1);var D=B[E];if(D.rangeEnd<F){C=E+1;}
else if(D.rangeStart>F){A=E-1;}
else {return D;}
;}
;}
,getItemAtPosition:function(K){if(K<0||K>=this.getTotalSize()){return null;}
;var J=this.__yu(K);var L=J.rangeStart;var G=J.startIndex;var I=J.firstItemSize;if(L+I>K){return {index:G,offset:K-L};}
else {var H=this.defaultItemSize;return {index:G+1+Math.floor((K-L-I)/H),offset:(K-L-I)%H};}
;}
,__yv:function(M){var O=this.__ys||this.__yt();var P=0;var N=O.length-1;while(true){var R=P+((N-P)>>1);var Q=O[R];if(Q.endIndex<M){P=R+1;}
else if(Q.startIndex>M){N=R-1;}
else {return Q;}
;}
;}
,getItemPosition:function(S){if(S<0||S>=this.itemCount){return null;}
;var T=this.__yv(S);if(T.startIndex==S){return T.rangeStart;}
else {return T.rangeStart+T.firstItemSize+(S-T.startIndex-1)*this.defaultItemSize;}
;}
,getTotalSize:function(){var U=this.__ys||this.__yt();return U[U.length-1].rangeEnd+1;}
...
;}
;return W;}
},destruct:function(){this.customSizes=this.__ys=null;}
});}
)();
...
(function(){var a="one",b="Boolean",c="qx.ui.virtual.selection.MModel",d="qx.data.Array",f="multi",g="selected",h="_applySelection",j="change",k="_applyDragSelection",l="single",m="_applyQuickSelection",n="changeSelection",o="_applySelectionMode",p="additive",q="qx.event.type.Data";qx.Mixin.define(c,{construct:function(){this._initSelectionManager();this.__Me=new qx.data.Array();this.initSelection(this.__Me);}
,properties:{selection:{check:d,event:n,apply:h,nullable:false,deferredInit:true},selectionMode:{check:[l,f,p,a],init:l,apply:o},dragSelection:{check:b,init:false,apply:k},quickSelection:{check:b,init:false,apply:m}},events:{"changeSelection":q},members:{_manager:null,__Mf:false,__Mg:false,__Me:null,_initSelectionManager:function(){var self=this;var r={isItemSelectable:function(s){return self._provider.isSelectable(s);}
...
,_onChangeSelection:function(e){if(this.__Mg==true){return;}
;this.__Mf=true;var G=this.getSelection();var I=[];for(var i=0;i<G.getLength();i++ ){var H=G.getItem(i);var J=this._getSelectables();var F=-1;if(J!=null){F=J.indexOf(H);}
...
;this.__Mh();if(this._afterApplySelection!=null&&qx.lang.Type.isFunction(this._afterApplySelection)){this._afterApplySelection();}
;this.__Mf=false;}
,_onManagerChangeSelection:function(e){if(this.__Mf==true){return;}
;this.__Mg=true;this.__Mh();this.__Mg=false;}
,__Mh:function(){if(this.__Mj()){return;}
;var M=this._manager.getSelection();var N=[];for(var i=0;i<M.length;i++ ){var O=this._getDataFromRow(M[i]);if(O!=null){N.push(O);}
;}
;this.__Mi(N);}
,__Mi:function(R){var Q=this.getSelection();if(R.length>0){var P=[0,Q.getLength()];P=P.concat(R);var S=Q.splice.apply(Q,P);S.dispose();}
else {Q.removeAll();}
;}
,__Mj:function(){var U=this.getSelection();var W=this._manager.getSelection();if(U.getLength()!==W.length){return false;}
...
,getSortedSelection:function(){var bU=this.getSelectables();var bT=qx.lang.Object.getValues(this.__dx);bT.sort(function(a,b){return bU.indexOf(a)-bU.indexOf(b);}
);return bT;}
,isItemSelected:function(bV){var bW=this._selectableToHashCode(bV);return this.__dx[bW]!==undefined;}
...
;this._fireChange();}
,_setLeadItem:function(cb){var ca=this.__dO;if(ca!==null){this._styleSelectable(ca,u,false);}
;if(cb!==null){this._styleSelectable(cb,u,true);}
;this.__dO=cb;}
,getLeadItem:function(){return this.__dO;}
,_setAnchorItem:function(cd){var cc=this.__dP;if(cc!=null){this._styleSelectable(cc,I,false);}
...
;this._fireChange();}
,handlePointerOver:function(event){if(this.__dR!=null&&this.__dR!=this._getScroll().top){this.__dR=null;return;}
...
;var cD=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cA=event.isShiftPressed();if(this.isItemSelected(cB)&&!cA&&!cD&&!this.getDrag()){this.__dQ=cB;this._userInteraction=false;return;}
else {this.__dQ=null;}
;this._scrollItemIntoView(cB);var cC=this.getMode();if(this.getDrag()&&cC!==bu&&cC!==h&&!cA&&!cD&&event.getPointerType()==F){this._setAnchorItem(cB);this._setLeadItem(cB);this.__dE=this._getLocation();this.__dB=this._getScroll();this.__dF=event.getDocumentLeft()+this.__dB.left;this.__dG=event.getDocumentTop()+this.__dB.top;this.__dH=true;this._capture();}
;this._fireChange(bD);this._userInteraction=false;}
,handleTap:function(event){this._userInteraction=true;var cH=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cE=event.isShiftPressed();if(!cH&&!cE&&this.__dQ!=null){this._userInteraction=false;var cF=this._getSelectableFromPointerEvent(event);if(cF===null||!this.isItemSelected(cF)){return;}
;}
...
,handlePointerMove:function(event){if(!this.__dH){return;}
;this.__dI=event.getDocumentLeft();this.__dJ=event.getDocumentTop();this._userInteraction=true;var cI=this.__dI+this.__dB.left;if(cI>this.__dF){this.__dK=1;}
else if(cI<this.__dF){this.__dK=-1;}
else {this.__dK=0;}
;var cJ=this.__dJ+this.__dB.top;if(cJ>this.__dG){this.__dL=1;}
else if(cJ<this.__dG){this.__dL=-1;}
else {this.__dL=0;}
;var location=this.__dE;if(this.__dI<location.left){this.__dy=this.__dI-location.left;}
...
;if(!this.__dA){this.__dA=new qx.event.Timer(100);this.__dA.addListener(G,this._onInterval,this);}
;this.__dA.start();this._autoSelect();event.stopPropagation();this._userInteraction=false;}
...
;delete this.__dH;delete this.__dC;delete this.__dD;this._releaseCapture();if(this.__dA){this.__dA.stop();}
;}
,_onInterval:function(e){this._scrollBy(this.__dy,this.__dz);this.__dB=this._getScroll();this._autoSelect();}
,_autoSelect:function(){var cT=this._getDimension();var cM=Math.max(0,Math.min(this.__dI-this.__dE.left,cT.width))+this.__dB.left;var cL=Math.max(0,Math.min(this.__dJ-this.__dE.top,cT.height))+this.__dB.top;if(this.__dC===cM&&this.__dD===cL){return;}
;this.__dC=cM;this.__dD=cL;var cQ=this._getAnchorItem();var cO=cQ;var cR=this.__dK;var cU,cN;while(cR!==0){cU=cR>0?this._getRelatedSelectable(cO,t):this._getRelatedSelectable(cO,W);if(cU!==null){cN=this._getSelectableLocationX(cU);if((cR>0&&cN.left<=cM)||(cR<0&&cN.right>=cM)){cO=cU;continue;}
;}
;break;}
;var cS=this.__dL;var cV,cP;while(cS!==0){cV=cS>0?this._getRelatedSelectable(cO,z):this._getRelatedSelectable(cO,bB);if(cV!==null){cP=this._getSelectableLocationY(cV);if((cS>0&&cP.top<=cL)||(cS<0&&cP.bottom>=cL)){cO=cV;continue;}
;}
;break;}
...
;this._fireChange(bw);}
,__ba:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dd,dc;var df=event.getKeyIdentifier();var de=this.getMode();var cY=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var da=event.isShiftPressed();var db=false;if(df===A&&cY){if(de!==bu&&de!==h){this._selectAllItems();db=true;}
;}
...
;db=true;}
;}
else if(this.__ba[df]){db=true;if(de===bu||de==h){dd=this._getSelectedItem();}
...
;}
;break;};this.__dR=this._getScroll().top;this._scrollItemIntoView(dc);}
;}
...
;}
,_clearSelection:function(){var di=this.__dx;for(var dj in di){this._removeFromSelection(di[dj]);}
;this.__dx={};}
,_selectItemRange:function(dk,dp,dm){var dq=this._getSelectableRange(dk,dp);if(!dm){var dn=this.__dx;var dr=this.__dS(dq);for(var dl in dn){if(!dr[dl]){this._removeFromSelection(dn[dl]);}
;}
;}
...
;}
,__dS:function(dw){var dx={};var dv;for(var i=0,l=dw.length;i<l;i++ ){dv=dw[i];dx[this._selectableToHashCode(dv)]=dv;}
;return dx;}
,_getSelectedItem:function(){for(var dy in this.__dx){return this.__dx[dy];}
;return null;}
,_setSelectedItem:function(dA){if(this._isSelectable(dA)){var dz=this.__dx;var dB=this._selectableToHashCode(dA);if(!dz[dB]||(dz.length>=2)){this._clearSelection();this._addToSelection(dA);}
;}
;}
,_addToSelection:function(dC){var dD=this._selectableToHashCode(dC);if(this.__dx[dD]==null&&this._isSelectable(dC)){this.__dx[dD]=dC;this._styleSelectable(dC,S,true);this.__dM=true;}
;}
,_toggleInSelection:function(dE){var dF=this._selectableToHashCode(dE);if(this.__dx[dF]==null){this.__dx[dF]=dE;this._styleSelectable(dE,S,true);}
else {delete this.__dx[dF];this._styleSelectable(dE,S,false);}
;this.__dM=true;}
,_removeFromSelection:function(dG){var dH=this._selectableToHashCode(dG);if(this.__dx[dH]!=null){delete this.__dx[dH];this._styleSelectable(dG,S,false);this.__dM=true;}
;}
...
;}
;var dP=dK[0];var dI=dO;var dM=this.__dx;for(var dN in dM){if(dJ[dN]){delete dJ[dN];}
...
;if(!dL){return false;}
;this._scrollItemIntoView(dI);this._setLeadItem(dP);this._setAnchorItem(dP);this.__dM=true;this._fireChange();}
,_fireChange:function(dQ){if(this.__dM){this.__dN=dQ||null;this.fireDataEvent(bA,this.getSelection());delete this.__dM;}
;}
...
;return dS;}
;return null;}
},destruct:function(){this._disposeObjects(Q);this.__dx=this.__dQ=this.__dP=null;this.__dO=null;}
});}
)();
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__dY=[];this.__Ml=[];this.__Mm={};this.__Mn=false;this.__Mo=false;this.__Mp=false;this.getPane().addListener(d,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__dY;}
,_lookup:function(Q){return this.__dY[Q];}
,_lookupGroup:function(R){return this.__Ml.indexOf(R);}
,_reverseLookup:function(S){if(S<0){return -1;}
;return this.__dY.indexOf(S);}
...
,_applyGroupRowHeight:function(ba,Y){this.__Mq();}
...
,_applyDelegate:function(bo,bn){this._provider.setDelegate(bo);this.__em();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this._provider.removeBindings();this.__em();this._applyDefaultSelection();}
,__jE:function(){this.getPane().getRowConfig().setItemCount(this.__dY.length);this.getPane().fullUpdate();}
,__Mq:function(){var bp=this.getPane().getRowConfig();var bq=this.getGroupItemHeight();bp.resetItemSizes();if(bq){for(var i=0,l=this.__dY.length;i<l; ++i){if(this.__dY[i]==-1){bp.setItemSize(i,bq);}
;}
;}
;}
,__em:function(){this.__dY=[];this.__Ml=[];this.__Mm={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var br=this.getModel();if(br!=null){this._runDelegateFilter(br);this._runDelegateSorter(br);this._runDelegateGroup(br);}
;this._updateSelection();this.__Mq();this.__jE();}
,_runDelegateFilter:function(bt){var bs=qx.util.Delegate.getMethod(this.getDelegate(),D);for(var i=0,l=bt.length;i<l; ++i){if(bs==null||bs(bt.getItem(i))){this.__dY.push(i);}
;}
;}
,_runDelegateSorter:function(bu){if(this.__dY.length==0){return;}
;var bv=qx.util.Delegate.getMethod(this.getDelegate(),t);if(bv!=null){this.__dY.sort(function(a,b){return bv(bu.getItem(a),bu.getItem(b));}
);}
;}
,_runDelegateGroup:function(bz){var bA=qx.util.Delegate.getMethod(this.getDelegate(),v);if(bA!=null){for(var i=0,l=this.__dY.length;i<l; ++i){var bw=this.__dY[i];var by=this.getModel().getItem(bw);var bx=bA(by);this.__Mr(bx,bw);}
;this.__dY=this.__Ms();}
;}
,__Mr:function(bC,bB){if(bC==null){this.__Mp=true;bC=A;}
;var name=this.__Mt(bC);if(this.__Mm[name]==null){this.__Mm[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bC);}
;}
;this.__Mm[name].push(bB);}
,__Ms:function(){this.__Mu();var bG=[];var bI=0;var bE=this.getGroups();for(var i=0;i<bE.getLength();i++ ){var bD=bE.getItem(i);bG.push(-1);this.__Ml.push(bI);bI++ ;var bH=this.__Mt(bD);var bF=this.__Mm[bH];if(bF!=null){for(var k=0;k<bF.length;k++ ){bG.push(bF[k]);bI++ ;}
;}
;}
;return bG;}
,__Mt:function(bK){var name=null;if(!qx.lang.Type.isString(bK)){var bJ=this.getGroups().indexOf(bK);this.__Mo=true;name=v;if(bJ==-1){name+=this.getGroups().getLength();}
else {name+=bJ;}
;}
else {this.__Mn=true;var name=bK;}
;return name;}
,__Mu:function(){if(this.__Mo&&this.__Mp||this.__Mo&&this.__Mn){throw new Error(r+z);}
;}
...
;this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__dY=this.__Ml=this.__Mm=null;if(this.__Mk){this.__Mk.dispose();}
;}
});}
)();
(function(){var a="qx.ui.virtual.layer.Abstract",b="abstract",c="Abstract method '_fullUpdate' called!";qx.Class.define(a,{extend:qx.ui.core.Widget,type:b,implement:[qx.ui.virtual.core.ILayer],construct:function(){qx.ui.core.Widget.call(this);this.__sN={};}
,properties:{anonymous:{refine:true,init:true}},members:{__sN:null,__BQ:null,__Mv:null,__Mw:null,__yo:null,__cn:null,getFirstRow:function(){return this.__Mv;}
,getFirstColumn:function(){return this.__Mw;}
,getRowSizes:function(){return this.__yo||[];}
,getColumnSizes:function(){return this.__cn||[];}
,syncWidget:function(e){if(!this.getContentElement().getDomElement()){return;}
;if(this.__sN.fullUpdate||this.__sN.updateLayerWindow&&this.__sN.updateLayerData){this._fullUpdate.apply(this,this.__BQ);}
else if(this.__sN.updateLayerWindow){this._updateLayerWindow.apply(this,this.__BQ);}
else if(this.__sN.updateLayerData&&this.__yo){this._updateLayerData();}
;if(this.__sN.fullUpdate||this.__sN.updateLayerWindow){var d=this.__BQ;this.__Mv=d[0];this.__Mw=d[1];this.__yo=d[2];this.__cn=d[3];}
;this.__sN={};}
,_updateLayerData:function(){this._fullUpdate(this.__Mv,this.__Mw,this.__yo,this.__cn);}
,_fullUpdate:function(g,i,h,f){throw new Error(c);}
,_updateLayerWindow:function(k,m,l,j){this._fullUpdate(k,m,l,j);}
,updateLayerData:function(){this.__sN.updateLayerData=true;qx.ui.core.queue.Widget.add(this);}
,fullUpdate:function(p,q,o,n){this.__BQ=arguments;this.__sN.fullUpdate=true;qx.ui.core.queue.Widget.add(this);}
,updateLayerWindow:function(t,u,s,r){this.__BQ=arguments;this.__sN.updateLayerWindow=true;qx.ui.core.queue.Widget.add(this);}
},destruct:function(){this.__sN=this.__BQ=this.__yo=this.__cn=null;}
});}
)();
...
;this.__Mx={};this.__My={};}
,properties:{colorEven:{nullable:true,check:c,apply:b,themeable:true},colorOdd:{nullable:true,check:c,apply:a,themeable:true}},members:{__Mz:null,__MA:null,__Mx:null,__My:null,setColor:function(h,g){if(g){this.__Mx[h]=qx.theme.manager.Color.getInstance().resolve(g);}
else {delete (this.__Mx[h]);}
;}
,clearCustomColors:function(){this.__Mx={};this.updateLayerData();}
,getColor:function(i){var j=this.__Mx[i];if(j){return j;}
else {return i%2==0?this.__Mz:this.__MA;}
;}
,_applyColorEven:function(l,k){if(l){this.__Mz=qx.theme.manager.Color.getInstance().resolve(l);}
else {this.__Mz=null;}
;this.updateLayerData();}
,_applyColorOdd:function(n,m){if(n){this.__MA=qx.theme.manager.Color.getInstance().resolve(n);}
else {this.__MA=null;}
;this.updateLayerData();}
,setBackground:function(o,p){if(p){this.__My[o]=qx.theme.manager.Decoration.getInstance().resolve(p);}
else {delete (this.__My[o]);}
;this.updateLayerData();}
,getBackground:function(q){return this.__My[q];}
},destruct:function(){this.__Mx=this.__My=null;}
});}
)();
...
;}
,setColor:function(J,I){qx.ui.virtual.layer.AbstractBackground.prototype.setColor.call(this,J,I);if(this.__pl(J)){this.updateLayerData();}
;}
,setBackground:function(K,L){qx.ui.virtual.layer.AbstractBackground.prototype.setBackground.call(this,K,L);if(this.__pl(K)){this.updateLayerData();}
;}
,__pl:function(M){var O=this.getFirstRow();var N=O+this.getRowSizes().length-1;return M>=O&&M<=N;}
}});}
)();
...
(function(){var a="cell.type",b="model[",c="groups[",d="model",f="",g="changeDelegate",h="label",i="qx.ui.list.core.MWidgetController",j="icon",k="]",l="value",m="BindingIds",n=".",o="group",p="String";qx.Mixin.define(i,{construct:function(){this.__jR=[];}
...
;}
,bindProperty:function(u,x,v,t,s){var y=t.getUserData(a);var w=this.__jS(s,u,y);if(v){v.ignoreConverter=d;}
;var z=this._list.bind(w,t,x,v);this.__dm(t,z);}
,bindPropertyReverse:function(D,B,E,F,A){var G=F.getUserData(a);var C=this.__jS(A,D,G);var H=F.bind(B,this._list,C,E);this.__dm(F,H);}
,removeBindings:function(){while(this.__jR.length>0){var I=this.__jR.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__jT(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__jR,T)){qx.lang.Array.remove(this.__jR,T);}
;}
,__jS:function(X,ba,Y){var W=b+X+k;if(Y==o){W=c+X+k;}
;if(ba!=null&&ba!=f){W+=n+ba;}
;return W;}
,__dm:function(bc,bd){var bb=this.__jT(bc);if(!qx.lang.Array.contains(bb,bd)){bb.push(bd);}
;if(!qx.lang.Array.contains(this.__jR,bc)){this.__jR.push(bc);}
;}
,__jT:function(be){var bf=be.getUserData(m);if(bf==null){bf=[];be.setUserData(m,bf);}
;return bf;}
...
,styleSelectabled:function(v){var u=this.__MB(v);this._styleSelectabled(u);}
,styleUnselectabled:function(x){var w=this.__MB(x);this._styleUnselectabled(w);}
...
;}
,_styleSelectabled:function(A){this.__MC(A,{selected:1});}
,_styleUnselectabled:function(B){this.__MC(B,{});}
...
,__MB:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__MC:function(I,H){if(I==null){return;}
...
;this._cellProvider=h;this.__tw=[];}
,properties:{anonymous:{refine:true,init:false}},events:{updated:c},members:{__tw:null,getRenderedCellWidget:function(q,n){if(this._getChildren().length===0){return null;}
...
;}
,_getSpacer:function(){var r=this.__tw.pop();if(!r){r=new qx.ui.core.Spacer();r.setUserData(g,1);}
;return r;}
...
;}
;}
,_fullUpdate:function(B,v,A,z){var C=this._cellProvider;var H=this._getChildren().concat();for(var i=0;i<H.length;i++ ){var F=H[i];if(F.getUserData(g)){this.__tw.push(F);}
...
;}
;}
;var L=this._cellProvider;var P=this._getChildren().concat();for(var i=0;i<P.length;i++ ){if(!bb[i]){var W=P[i];if(W.getUserData(g)){this.__tw.push(W);}
...
;this._cellProvider=this.__tw=null;}
});}
)();
...
;}
;j.setUserData(c,h);}
,getCellWidget:function(o,m){var n=this.__MD();this.updateStates(n,m);this.updateData(n,o);return n;}
,pool:function(p){this.__Cn.push(p);}
,_cleanupPool:function(){var q=this.__Cn.pop();while(q){q.destroy();q=this.__Cn.pop();}
;}
,__MD:function(){var r=this.__Cn.shift();if(r==null){r=this._createWidget();this.fireDataEvent(a,r);}
;return r;}
