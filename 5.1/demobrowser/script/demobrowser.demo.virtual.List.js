...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.List.73235a884374.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var dc=qx.data.marshal.Json.createModel(cV);var cQ=this.__Om=new qx.ui.list.List(dc).set({scrollbarX:ci,selectionMode:cx,height:280,width:150,labelPath:cH,iconPath:H,iconOptions:{converter:function(de){return bS+de+cm;}
...
;}
;var cU=new qx.ui.form.RadioButton(cg);var cL=new qx.ui.form.RadioButton(D);var cW=new qx.ui.form.RadioButton(W);var db=new qx.ui.form.RadioButton(cC);cU.setUserData(ct,cz);cL.setUserData(ct,cx);cW.setUserData(ct,t);db.setUserData(ct,c);cM.add(cU,{left:160,top:20});cM.add(cL,{left:160,top:40});cM.add(cW,{left:160,top:60});cM.add(db,{left:160,top:80});var cR=new qx.ui.form.RadioButton(P);var dd=new qx.ui.form.RadioButton(b);var cK=new qx.ui.form.RadioButton(I);cR.setUserData(ct,cH);dd.setUserData(ct,H);cK.setUserData(ct,co);cM.add(cR,{left:160,top:120});cM.add(dd,{left:160,top:140});cM.add(cK,{left:160,top:160});var cS=this.__On=new qx.ui.form.CheckBox(G);var cY=this.__Oo=new qx.ui.form.CheckBox(Q).set({enabled:false});cM.add(cS,{left:160,top:200});cM.add(cY,{left:160,top:220});var da=new qx.ui.form.RadioGroup(cU,cL,cW,db);da.setSelection([cL]);da.addListener(m,this.onSelectionModeChange,this);var cO=new qx.ui.form.RadioGroup(cR,dd,cK);cO.setSelection([cK]);cO.addListener(m,this.onShowModeChange,this);cS.addListener(g,this.onDragCheckChange,this);cY.addListener(g,this.onQuickCheckChange,this);return cM;}
...
,onSelectionModeChange:function(e){var dy=e.getData()[0].getUserData(ct);this.__Om.setSelectionMode(dy);if(dy==cz||dy==c){this.__On.setEnabled(false);this.__Oo.setEnabled(true);}
else if(dy==cx||dy==bV){this.__On.setEnabled(true);this.__Oo.setEnabled(false);}
;}
,onShowModeChange:function(e){var dz=e.getData()[0].getUserData(ct);this.setShowMode(dz);}
,onDragCheckChange:function(e){if(e.getData()){var dA=this.__Om.getSelectionMode();if(dA==cz||dA==c){this.debug(M);}
;}
;this.__Om.setDragSelection(e.getData());}
,onQuickCheckChange:function(e){if(e.getData()){var dB=this.__Om.getSelectionMode();if(dB==cx||dB==t){this.debug(A);}
;}
...
(function(){var a=" array contains ",b="qx.ui.core.MSingleSelectionHandling",c=" items!",d="changeSelection",f="changeSelected",g="Could only select one item, but the selection",h="__u",i="qx.event.type.Data";qx.Mixin.define(b,{events:{"changeSelection":i},members:{__u:null,getSelection:function(){var j=this.__v().getSelected();if(j){return [j];}
else {return [];}
;}
,setSelection:function(k){switch(k.length){case 0:this.resetSelection();break;case 1:this.__v().setSelected(k[0]);break;default:throw new Error(g+a+k.length+c);};}
,resetSelection:function(){this.__v().resetSelected();}
,isSelected:function(l){return this.__v().isSelected(l);}
,isSelectionEmpty:function(){return this.__v().isSelectionEmpty();}
,getSelectables:function(m){return this.__v().getSelectables(m);}
,_onChangeSelected:function(e){var o=e.getData();var n=e.getOldData();o==null?o=[]:o=[o];n==null?n=[]:n=[n];this.fireDataEvent(d,o,n);}
,__v:function(){if(this.__u==null){var p=this;this.__u=new qx.ui.core.SingleSelectionManager({getItems:function(){return p._getItems();}
,isItemSelectable:function(q){if(p._isItemSelectable){return p._isItemSelectable(q);}
else {return q.isVisible();}
;}
});this.__u.addListener(f,this._onChangeSelected,this);}
;this.__u.setAllowEmptySelection(this._isAllowEmptySelection());return this.__u;}
},destruct:function(){this._disposeObjects(h);}
});}
)();
(function(){var a=", because it is not a child element!",b="Could not check if ",c="__y",d="Boolean",e="qx.ui.core.SingleSelectionManager",f="qx.debug",g="Invalid selectionProvider!",h="__A",j=" is selected,",k="Could not select ",l="__z",m="changeSelected",n=" because it is not a child element!",o="qx.event.type.Data";qx.Class.define(e,{extend:qx.core.Object,construct:function(p){qx.core.Object.call(this);if(qx.core.Environment.get(f)){qx.core.Assert.assertInterface(p,qx.ui.core.ISingleSelectionProvider,g);}
;this.__y=p;}
,events:{"changeSelected":o},properties:{allowEmptySelection:{check:d,init:true,apply:h}},members:{__z:null,__y:null,getSelected:function(){return this.__z;}
,setSelected:function(q){if(!this.__C(q)){throw new Error(k+q+a);}
;this.__B(q);}
,resetSelected:function(){this.__B(null);}
,isSelected:function(r){if(!this.__C(r)){throw new Error(b+r+j+n);}
;return this.__z===r;}
,isSelectionEmpty:function(){return this.__z==null;}
,getSelectables:function(t){var s=this.__y.getItems();var u=[];for(var i=0;i<s.length;i++ ){if(this.__y.isItemSelectable(s[i])){u.push(s[i]);}
;}
;if(!t){for(var i=u.length-1;i>=0;i-- ){if(!u[i].getEnabled()){u.splice(i,1);}
;}
;}
;return u;}
,__A:function(w,v){if(!w){this.__B(this.__z);}
;}
,__B:function(x){var A=this.__z;var y=x;if(y!=null&&A===y){return;}
;if(!this.isAllowEmptySelection()&&y==null){var z=this.getSelectables(true)[0];if(z){y=z;}
;}
;this.__z=y;this.fireDataEvent(m,y,A);}
,__C:function(B){var C=this.__y.getItems();for(var i=0;i<C.length;i++ ){if(C[i]===B){return true;}
;}
;return false;}
},destruct:function(){if(this.__y.toHashCode){this._disposeObjects(c);}
else {this.__y=null;}
...
(function(){var a="_applyAllowEmptySelection",b="_applyInvalidMessage",c="qx.ui.form.RadioGroup",d="Boolean",f="_applyValid",g="",h="changeRequired",j="changeValid",k="changeEnabled",m="changeInvalidMessage",n="changeSelection",o="changeValue",p="_applyEnabled",q="String",r="__HU";qx.Class.define(c,{extend:qx.core.Object,implement:[qx.ui.core.ISingleSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MSingleSelectionHandling,qx.ui.form.MModelSelection],construct:function(s){qx.core.Object.call(this);this.__HU=[];this.addListener(n,this.__cI,this);if(s!=null){this.add.apply(this,arguments);}
;}
,properties:{enabled:{check:d,apply:p,event:k,init:true},wrap:{check:d,init:true},allowEmptySelection:{check:d,init:false,apply:a},valid:{check:d,init:true,apply:f,event:j},required:{check:d,init:false,event:h},invalidMessage:{check:q,init:g,event:m,apply:b},requiredInvalidMessage:{check:q,nullable:true,event:m}},members:{__HU:null,getItems:function(){return this.__HU;}
,add:function(u){var v=this.__HU;var t;for(var i=0,l=arguments.length;i<l;i++ ){t=arguments[i];if(qx.lang.Array.contains(v,t)){continue;}
...
;}
,remove:function(w){var x=this.__HU;if(qx.lang.Array.contains(x,w)){qx.lang.Array.remove(x,w);if(w.getGroup()===this){w.resetGroup();}
;w.removeListener(o,this._onItemChangeChecked,this);if(w.getValue()){this.resetSelection();}
;}
;}
,getChildren:function(){return this.__HU;}
...
;}
,_applyInvalidMessage:function(A,z){for(var i=0;i<this.__HU.length;i++ ){this.__HU[i].setInvalidMessage(A);}
;}
,_applyValid:function(C,B){for(var i=0;i<this.__HU.length;i++ ){this.__HU[i].setValid(C);}
;}
,_applyEnabled:function(F,E){var D=this.__HU;if(F==null){for(var i=0,l=D.length;i<l;i++ ){D[i].resetEnabled();}
;}
...
;}
,selectNext:function(){var J=this.getSelection()[0];var K=this.__HU;var I=K.indexOf(J);if(I==-1){return;}
...
,selectPrevious:function(){var M=this.getSelection()[0];var N=this.__HU;var L=N.indexOf(M);if(L==-1){return;}
...
,_isItemSelectable:function(O){return this.__HU.indexOf(O)!=-1;}
,__cI:function(e){var Q=e.getData()[0];var P=e.getOldData()[0];if(P){P.setValue(false);}
