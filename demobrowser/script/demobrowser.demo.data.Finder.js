...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Finder.305fcf5cec17.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="qx.ui.form.List",b="text",c="Boolean",d="one",f="addChildWidget",g="_applySpacing",h="Enter",j="Integer",k="action",m="keyinput",n="qx.event.type.Data",o="addItem",p="vertical",q="removeChildWidget",r="",s="_applyOrientation",t="single",u="keypress",v="list",w="__vR",x="horizontal",y="label",z="pane",A="removeItem";qx.Class.define(a,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__vR=this._createListItemContainer();this.__vR.addListener(f,this._onAddChild,this);this.__vR.addListener(q,this._onRemoveChild,this);this.getChildControl(z).add(this.__vR);if(B){this.setOrientation(x);}
else {this.initOrientation();}
;this.addListener(u,this._onKeyPress);this.addListener(m,this._onKeyInput);this.__yU=r;}
,events:{addItem:n,removeItem:n},properties:{appearance:{refine:true,init:v},focusable:{refine:true,init:true},orientation:{check:[x,p],init:p,apply:s},spacing:{check:j,init:0,apply:g,themeable:true},enableInlineFind:{check:c,init:true}},members:{__yU:null,__yV:null,__vR:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__vR;}
...
,_applyOrientation:function(F,C){var D=F===x;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__vR;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__vR.getLayout().setSpacing(H);}
...
;if(((new Date).valueOf()-this.__yV)>1000){this.__yU=r;}
;this.__yU+=e.getChar();var K=this.findItemByLabelFuzzy(this.__yU);if(K){this.setSelection([K]);}
;this.__yV=(new Date).valueOf();}
...
;if(this.getSelection()==null){this.__yX=new qx.data.Array();this.setSelection(this.__yX);}
;}
,properties:{selection:{check:b,event:f,apply:d,init:null}},events:{"changeSelection":h},members:{_modifingSelection:0,__yY:null,__za:null,__yX:null,_applySelection:function(k,j){if(this.__za!=undefined&&j!=undefined){j.removeListenerById(this.__za);this.__za=null;}
;if(k){this.__za=k.addListener(a,this.__zb,this);}
;this._updateSelection();}
,__zb:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__zc()&&!this.__zd()){return;}
;if(this._inSelectionModification()){return;}
;var m=this.getTarget().getSelection();var l=this.getSelection();if(l==null){l=new qx.data.Array();this.__yX=l;this.setSelection(l);}
...
,_addChangeTargetListener:function(p,o){if(this.__yY!=undefined&&o!=undefined){o.removeListenerById(this.__yY);}
;if(p!=null){if(this.__zc()||this.__zd()){this.__yY=p.addListener(f,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()||!this.getSelection()){return;}
;this._startSelectionModification();if(this.__zc()){var s=[];for(var i=0;i<this.getSelection().length;i++ ){var t=this.getSelection().getItem(i);var u=this.__zf(t);if(u!=null){s.push(u);}
;}
...
;}
;}
else if(this.__zd()){var r=this.getSelection().getItem(this.getSelection().length-1);if(r!==undefined){this.__ze(r);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__zc:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.IMultiSelection);}
,__zd:function(){var w=this.getTarget().constructor;return qx.Class.implementsInterface(w,qx.ui.core.ISingleSelection);}
,__ze:function(x){var y=this.__zf(x);if(y==null){return;}
;if(this.__zc()){this.getTarget().addToSelection(y);}
else if(this.__zd()){this.getTarget().setSelection([y]);}
;}
,__zf:function(z){var A=this.getTarget().getSelectables(true);for(var i=0;i<A.length;i++ ){if(A[i].getModel()==z){return A[i];}
;}
;return null;}
...
(function(){var a="changeModel",b="_applyLabelOptions",c="_applyLabelPath",d="changeTarget",e="_applyModel",f="change",g="icon",h="ReverseBindingId",k="qx.data.controller.List",m="_applyIconPath",n="BindingId",o="_applyDelegate",p="",q="changeDelegate",r="_applyTarget",s="]",t="qx.data.IListData",u="model[",v="model",w="label",x="String",y="_applyIconOptions",z=".";qx.Class.define(k,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__sn=[];this.__zg=[];this.__zh=[];this.__zi={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:t,apply:e,event:a,nullable:true,dereference:true},target:{apply:r,event:d,nullable:true,init:null,dereference:true},labelPath:{check:x,apply:c,nullable:true},iconPath:{check:x,apply:m,nullable:true},labelOptions:{apply:b,nullable:true},iconOptions:{apply:y,nullable:true},delegate:{apply:o,event:q,init:null,nullable:true}},members:{__zj:null,__sn:null,__zi:null,__zg:null,__zh:null,__zk:null,__zl:null,update:function(){this.__zo();this.__zs();this._updateSelection();}
,_applyDelegate:function(E,D){this._setConfigureItem(E,D);this._setFilter(E,D);this._setCreateItem(E,D);this._setBindItem(E,D);}
,_applyIconOptions:function(G,F){this.__zs();}
,_applyLabelOptions:function(I,H){this.__zs();}
,_applyIconPath:function(K,J){this.__zs();}
,_applyLabelPath:function(M,L){this.__zs();}
,_applyModel:function(P,N){if(N!=undefined){if(this.__zj!=undefined){N.removeListenerById(this.__zj);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(P!=null){this.__zj=P.addListener(f,this.__zn,this);this.__xt();this.__zo();if(N==null){this._changeTargetSelection();}
else {this.__zk=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var O=this.getTarget();if(O!=null){var length=O.getChildren().length;for(var i=0;i<length;i++ ){this.__zr();}
;}
;}
;}
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__sn.length;i++ ){this.__zq(this.__zu(i));}
;}
;}
;}
,__zm:false,__zn:function(){if(this.__zm)return;this.__zm=true;this.__zl=true;qx.ui.core.queue.Widget.add(this);this.update();this.__zm=false;}
,syncWidget:function(){if(this.__zk){this._changeTargetSelection();}
;if(this.__zl){this._updateSelection();}
;this.__zl=this.__zk=null;}
,__zo:function(){if(this.getTarget()==null){return;}
;this.__xt();var U=this.__sn.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__zq(this.__zu(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__zr();}
;}
;this.__xt();this.__zk=true;qx.ui.core.queue.Widget.add(this);}
,__zp:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__zj);this.__zj=V.addListener(f,this.__zn,this);}
;}
...
;return W;}
,__zq:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__zr:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bo,br,bp,bn,bm){if(bp!=null){var bp=qx.lang.Object.clone(bp);this.__zi[br]=bp.onUpdate;delete bp.onUpdate;}
else {bp={};this.__zi[br]=null;}
;bp.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bm);bp.ignoreConverter=v;var bq=u+bm+s;if(bo!=null&&bo!=p){bq+=z+bo;}
;var bs=this.bind(bq,bn,br,bp);bn.setUserData(br+n,bs);if(!qx.lang.Array.contains(this.__zg,br)){this.__zg.push(br);}
;}
,bindPropertyReverse:function(by,bv,bw,bx,bt){var bu=u+bt+s;if(by!=null&&by!=p){bu+=z+by;}
;var bz=bx.bind(bv,this,bu,bw);bx.setUserData(by+h,bz);if(!qx.lang.Array.contains(this.__zh,by)){this.__zh.push(by);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__zg.length;i++ ){if(this.__zi[this.__zg[i]]!=null){this.__zi[this.__zg[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__zg.length;i++ ){var bE=bD.getUserData(this.__zg[i]+n);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__zh.length;i++ ){var bE=bD.getUserData(this.__zh[i]+h);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__zs:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++ ){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__zu(i));}
;this.__zp();}
...
,_setFilter:function(bR,bP){if((bR==null||bR.filter==null)&&(bP!=null&&bP.filter!=null)){this.__zt();}
...
;var bQ=this.__sn;this.__xt();if(bQ.length>this.__sn.length){for(var j=bQ.length;j>this.__sn.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
...
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__zu(i));}
;this.__zp();this._endSelectionModification();this._updateSelection();}
,__zt:function(){this.__xt();this.__zo();this.__zs();this.__zl=true;qx.ui.core.queue.Widget.add(this);}
,__xt:function(){var bW=this.getModel();if(bW==null){return;}
...
;}
;}
,__zu:function(bY){return this.__sn[bY];}
},destruct:function(){this.__sn=this.__zi=this.__zg=null;this.__zh=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
...
(function(){var a="changeModel",b="application/json",c="json",d="aborted",e="loaded",f="qx.event.type.Data",g="error",h="fail",i="receiving",j="queued",k="changeState",l="success",m="String",n="__vr",o="changePhase",p="sending",q="_marshaler",r="completed",s="failed",t="qx.data.store.Json",u="configured",v="changeUrl",w="timeout",x="_applyUrl";qx.Class.define(t,{extend:qx.core.Object,construct:function(y,z){qx.core.Object.call(this);this._marshaler=new qx.data.marshal.Json(z);this._delegate=z;if(y!=null){this.setUrl(y);}
;}
,events:{"loaded":f,"error":f},properties:{model:{nullable:true,event:a},state:{check:[u,j,p,i,r,d,w,s],init:u,event:k},url:{check:m,apply:x,event:v,nullable:true}},members:{_marshaler:null,_delegate:null,__vr:null,_applyUrl:function(B,A){if(B!=null){B=qx.util.AliasManager.getInstance().resolve(B);B=qx.util.ResourceManager.getInstance().toUri(B);this._createRequest(B);}
;}
,_getRequest:function(){return this.__vr;}
,_setRequest:function(C){this.__vr=C;}
,_createRequest:function(D){if(this.__vr){this.__vr.dispose();this.__vr=null;}
...
;this.fireDataEvent(e,this.getModel());if(this.__vr){this.__vr.dispose();this.__vr=null;}
;}
,reload:function(){var R=this.getUrl();if(R!=null){this._createRequest(R);}
;}
},destruct:function(){if(this.__vr!=null){this._disposeObjects(n);}
