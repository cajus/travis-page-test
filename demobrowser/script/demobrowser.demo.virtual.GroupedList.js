...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.GroupedList.db2f83888e4b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
};T.setDelegate(V);T.setSelection(this.__DS.getSelection());return U;}
,createThirdExample:function(){var bc=new qx.ui.container.Composite(new qx.ui.layout.Canvas());var be=new qx.ui.basic.Label(u).set({font:v});bc.add(be);var bb=this.__On=new qx.ui.list.List().set({height:280,width:150,labelPath:e,labelOptions:{converter:function(bg,bf){return bf?bg+D+bf.getLastname():B;}
...
};bb.setDelegate(bd);bb.setSelection(this.__DS.getSelection());return bc;}
},destruct:function(){this.__DS.dispose();this.__Om.dispose();this.__On.dispose();this.__DS=this.__Om=this.__On=null;}
});}
)();
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__sn=[];this.__xn=[];this.__xo={};this.__xp=false;this.__xq=false;this.__xr=false;this.getPane().addListener(d,this._onResize,this);this._initBackground();this._initLayer();}
...
,_applyGroupRowHeight:function(ba,Y){this.__xs();}
...
,_applyDelegate:function(bo,bn){this._provider.setDelegate(bo);this.__xt();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this._provider.removeBindings();this.__xt();this._applyDefaultSelection();}
,__sv:function(){this.getPane().getRowConfig().setItemCount(this.__sn.length);this.getPane().fullUpdate();}
,__xs:function(){var bp=this.getPane().getRowConfig();var bq=this.getGroupItemHeight();bp.resetItemSizes();if(bq){for(var i=0,l=this.__sn.length;i<l; ++i){if(this.__sn[i]==-1){bp.setItemSize(i,bq);}
;}
;}
;}
,__xt:function(){this.__sn=[];this.__xn=[];this.__xo={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var br=this.getModel();if(br!=null){this._runDelegateFilter(br);this._runDelegateSorter(br);this._runDelegateGroup(br);}
;this._updateSelection();this.__xs();this.__sv();}
...
;}
,_runDelegateGroup:function(bz){var bA=qx.util.Delegate.getMethod(this.getDelegate(),v);if(bA!=null){for(var i=0,l=this.__sn.length;i<l; ++i){var bw=this.__sn[i];var by=this.getModel().getItem(bw);var bx=bA(by);this.__xu(bx,bw);}
;this.__sn=this.__xv();}
;}
...
;this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__sn=this.__xn=this.__xo=null;if(this.__xm){this.__xm.dispose();}
;}
});}
)();
