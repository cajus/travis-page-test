...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.2e4a14f04809.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;if(this.__JO==null){this.__JO=by;}
;this.__jO[by]=bx;}
...
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__JT[bG]=bH;this.__JU[bH]=bG;this.__JS[bG]=bD;if(!bE){return;}
;if(typeof (bE)==j){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__JV[bF]){this.__JV[bF]={};}
;this.__JV[bF][bG]=true;if(!this.__JW[bG]){this.__JW[bG]=[];}
;this.__JW[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__JT[bL];if(this.__JW[bL]){for(var i=0;i<this.__JW[bL].length;i++ ){bJ=this.__JW[bL][i];delete this.__JV[bJ][bL];bM=true;for(bK in this.__JV[bJ]){bM=false;break;}
;if(bM){delete this.__JV[bJ];}
;}
;delete this.__JW[bL];}
;delete this.__JU[bI];delete this.__JT[bL];delete this.__JS[bL];}
,getObject:function(bN){return this.__JS[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__JU[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__JV[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__JT){var bT=this.__JT[bR];var bS=this.getObject(bR);this.debug(bR+c+bT);this.debug(br+bT+c+this.__JU[bT]);this.debug(br+bR+c+this.getObject(bR));this.debug(br+this.getObject(bR)+c+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__jO,"startState":this.__JO,"eventQueue":this.__JQ,"blockedEvents":this.__JR,"savedStates":this.__JP,"friendlyToObject":this.__JS,"friendlyToHash":this.__JT,"hashToFriendly":this.__JU,"groupToFriendly":this.__JV,"friendlyToGroups":this.__JW});}
,start:function(){this.__JY=false;var bV=this.__JO;if(bV==null){throw new Error(R);}
...
,pushState:function(bW){if(this.__JP.length>=this.getMaxSavedStates()){throw new Error(I);}
;if(bW===true){this.__JP.push(this.getState());}
else if(bW){this.__JP.push(bW);}
else {this.__JP.push(this.getPreviousState());}
;}
,popState:function(){if(this.__JP.length==0){throw new Error(n);}
;return this.__JP.pop();}
,postponeEvent:function(event){this.__JR.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__JQ.push(event);}
else {this.__JQ.unshift(event);}
...
;}
;}
,eventListener:function(event){if(this.__JY){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__Ka();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__JY){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__Ka:function(){if(this.__JX){return;}
;this.__JX=true;while(this.__JQ.length>0){var event=this.__JQ.pop();var ch=this.__Kb(event);if(ch){event.dispose();}
;}
;this.__JX=false;}
,__Kb:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:G));}
...
;this.__JR.unshift(event);return false;default:if(typeof (cp)==j){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+A);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(z+ck+y+t+s+bp);};cl=cm.getNextState();if(typeof (cl)==j){if(!cl in this.__jO){throw new Error(m+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__JP.length==0){throw new Error(x+bm);}
;cl=this.__JP.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__JY=true;this.setNextState(null);break;default:throw new Error(o);};}
...
;if(this.__JY){if(cr){this.debug(this.getName()+u+b);}
...
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__JR.length;i++ ){e=this.__JR.pop();this.__JQ.unshift(e);}
;if(cs){this.debug(this.getName()+u+cj+c+this.getName()+u+ck);}
;return true;}
;if(cs){this.debug(this.getName()+u+ck+g+event.getType()+l+K);}
;return true;}
},destruct:function(){this._disposeArray(v);this._disposeArray(L);this.__JP=this.__jO=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="__JG",h="  var groupObjects = ",k="__JI",l="parameters",m="__JM",n=")",o=" fsm.getObject('",p="groups",q="Invalid function parameters: ",r="Invalid onexit type: ",s="  for (var i = 0; i < groupObjects.length; i++)",t="qx.util.fsm.Transition",u="context",v="string",w="autoActionsAfterOnentry",x="Invalid numeric value in events object: ",y='"',z="Invalid state name",A=" value: ",B="');",C="State ",D="autoActionsBeforeOnexit",E="autoActionsBeforeOnentry",F="}",G="__JK",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L="): ",M=": ",N="expected array, found ",O="Invalid friendly name in 'objects' list: ",P="autoActionsAfterOnexit",Q="  fsm.debug(ex);",R="(",S="Invalid onentry type: ",T="    fsm.getObject(objName).",U=")(fsm)",V="events",W="undefined",X="Invalid ",Y="__JL",ba="try",bb="    var objName = groupObjects[i];",bc="Invalid value in events object: ",bd="').",be="objects",bf="expected object, found ",bg="__JH",bh="The events object must be provided in new state info",bi="catch(ex)",bj="  {",bk="    fsm.getGroupObjects('",bl="Invalid value in events object ",bm="Invalid transition: not an instance of ",bn="  }",bo="__JF",bp="Invalid 'objects' list: expected array, got ",bq="Invalid numeric value in events object ",br="qx.util.fsm.State",bs="onentry",bt=";",bu="Invalid function request parameter type: ",bv="Invalid 'groups' list: expected array, got ",bw="fsm",bx=",",by="__JJ",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(br,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(u,bE);for(var bC in bF){switch(bC){case bs:this.setOnentry(this.__JN(bF[bC],bE));break;case I:this.setOnexit(this.__JN(bF[bC],bE));break;case E:this.setAutoActionsBeforeOnentry(bF[bC]);break;case w:this.setAutoActionsAfterOnentry(bF[bC]);break;case D:this.setAutoActionsBeforeOnexit(bF[bC]);break;case P:this.setAutoActionsAfterOnexit(bF[bC]);break;case V:this.setEvents(bF[bC]);break;case u:break;default:this.setUserData(bC,bF[bC]);this.debug(C+bD+M+bz+bC);break;};}
...
;for(j=0;j<g.length;j++ ){func+=h+bk+g[j]+B+s+bj+bb+T+funcFragment+bt+bn;}
;}
;}
;}
;func+=F+bi+J+Q+F;return qx.lang.Function.bind(new Function(bw,func),context);}
},properties:{name:{transform:bo,nullable:true},onentry:{transform:d,nullable:true,init:function(bG,event){}
},onexit:{transform:bg,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:by,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:G,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:Y,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:m,nullable:true,init:function(bL,event){}
},events:{transform:k,nullable:true}},members:{__JF:function(bM){if(typeof (bM)!=v||bM.length<1){throw new Error(z);}
;return bM;}
,__JG:function(bN){switch(typeof (bN)){case W:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(u));default:throw new Error(S+typeof (bN));};}
,__JH:function(bP){switch(typeof (bP)){case W:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(u));default:throw new Error(r+typeof (bP));};}
,__JI:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
...
;}
;return bT;}
,__JJ:function(bU){return qx.util.fsm.State._commonTransformAutoActions(E,bU,this.getUserData(u));}
,__JK:function(bV){return qx.util.fsm.State._commonTransformAutoActions(w,bV,this.getUserData(u));}
,__JL:function(bW){return qx.util.fsm.State._commonTransformAutoActions(D,bW,this.getUserData(u));}
,__JM:function(bX){return qx.util.fsm.State._commonTransformAutoActions(P,bX,this.getUserData(u));}
,__JN:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+t);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="__Kd",c="autoActionsBeforeOntransition",d="__Kf",e="or one of the Fsm.StateChange constants",g="autoActionsAfterOntransition",h="context",i="string",j="predicate",k="nextState must be an explicit state name, ",l="Transition ",m=": ",n="String",o="__Kg",p="__Ke",q="number",r="Invalid transition nextState type: ",s="Invalid transition nextState value: ",t="Adding user-provided field to transition: ",u="__Kc",v="ontransition",w="Invalid transition predicate type: ",x="boolean",y="nextState",z="Invalid ontransition type: ",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(h,E);for(var C in D){switch(C){case j:this.setPredicate(this.__JN(D[C],E));break;case y:this.setNextState(D[C]);break;case c:this.setAutoActionsBeforeOntransition(this.__JN(D[C],E));break;case g:this.setAutoActionsAfterOntransition(this.__JN(D[C],E));break;case v:this.setOntransition(this.__JN(D[C],E));break;case h:break;default:this.setUserData(C,D[C]);this.debug(l+F+m+t+C);break;};}
;}
,properties:{name:{check:n,nullable:true},predicate:{init:function(G,event){return true;}
,transform:u},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:b},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:p},autoActionsAfterOntransition:{init:function(I,event){}
,transform:d},ontransition:{init:function(J,event){}
,transform:o}},members:{__Kc:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
...
,__Kd:function(N){switch(typeof (N)){case i:return N;case q:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(s+N+m+k+e);};break;default:throw new Error(r+typeof (N));};}
,__Ke:function(O){return qx.util.fsm.State._commonTransformAutoActions(c,O,this.getUserData(h));}
,__Kf:function(P){return qx.util.fsm.State._commonTransformAutoActions(g,P,this.getUserData(h));}
,__Kg:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(h));default:throw new Error(z+typeof (Q));};}
,__JN:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
