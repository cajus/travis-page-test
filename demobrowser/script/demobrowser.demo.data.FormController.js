...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.FormController.c223163c726d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,events:{"change":c},members:{__uC:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(g,f,h,name,e,k){if(this.__Ig()){this.__uC.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
...
,addGroupHeader:function(m,l){if(!this.__Ig()){this._groupCounter++ ;}
...
,__Ig:function(){return this.__uC.length===0;}
...
(function(){var a="Widgets supporting selection can only be validated ",b="Added widget not supported.",c="",d="in the form validator",f="mouse",g="changeValid",h="qx.event.type.Event",j="complete",k="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",l="qx.ui.form.validation.Manager",m="This field is required",n="String",o="qx.event.type.Data";qx.Class.define(l,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Ih=[];this.__Ii={};this.setRequiredFieldMessage(qx.locale.Manager.tr(m));}
,events:{"changeValid":o,"complete":h},properties:{validator:{check:k,init:null,nullable:true},invalidMessage:{check:n,init:c},requiredFieldMessage:{check:n,init:c},context:{nullable:true}},members:{__Ih:null,__Ij:null,__Ii:null,__Ik:null,add:function(s,p,r){if(!this.__In(s)){throw new Error(b);}
;if(this.__Io(s)&&!s.getValue){if(p!=null){throw new Error(a+d);}
;}
;var q={item:s,validator:p,valid:null,context:r};this.__Ih.push(q);}
,remove:function(v){var t=this.__Ih;for(var i=0,u=t.length;i<u;i++ ){if(v===t[i].item){t.splice(i,1);return v;}
;}
;return null;}
,getItems:function(){var w=[];for(var i=0;i<this.__Ih.length;i++ ){w.push(this.__Ih[i].item);}
;return w;}
,validate:function(){var B=true;this.__Ik=true;var y=[];for(var i=0;i<this.__Ih.length;i++ ){var z=this.__Ih[i].item;var C=this.__Ih[i].validator;y.push(z);if(C==null){var x=this._validateRequired(z);B=B&&x;this.__Ik=x&&this.__Ik;continue;}
;var x=this._validateItem(this.__Ih[i],z.getValue());B=x&&B;if(x!=null){this.__Ik=x&&this.__Ik;}
;}
;var A=this.__Il(y);if(qx.lang.Type.isBoolean(A)){this.__Ik=A&&this.__Ik;}
;B=A&&B;this._setValid(B);if(qx.lang.Object.isEmpty(this.__Ii)){this.fireEvent(j);}
;return B;}
,_validateRequired:function(E){if(E.getRequired()){if(this.__Io(E)){var D=!!E.getSelection()[0];}
...
;return true;}
,_validateItem:function(K,L){var M=K.item;var O=K.context;var N=K.validator;if(this.__Im(N)){this.__Ii[M.toHashCode()]=null;N.validate(M,M.getValue(),this,O);return null;}
...
,__Il:function(Q){var R=this.getValidator();var S=this.getContext()||this;if(R==null){return true;}
;this.setInvalidMessage(c);if(this.__Im(R)){this.__Ii[this.toHashCode()]=null;R.validateForm(Q,this,S);return null;}
...
;}
;return T;}
,__Im:function(V){var U=false;if(!qx.lang.Type.isFunction(V)){U=qx.Class.isSubClassOf(V.constructor,qx.ui.form.validation.AsyncValidator);}
;return U;}
,__In:function(W){var X=W.constructor;return qx.Class.hasInterface(X,qx.ui.form.IForm);}
,__Io:function(Y){var ba=Y.constructor;return qx.Class.hasInterface(ba,qx.ui.core.ISingleSelection);}
,_setValid:function(bb){this._showToolTip(bb);var bc=this.__Ij;this.__Ij=bb;if(bc!=bb){this.fireDataEvent(g,bb,bc);}
;}
,_showToolTip:function(bg){if(!qx.ui.tooltip||!qx.ui.tooltip.Manager){return;}
;var be=qx.ui.tooltip.Manager.getInstance().getSharedErrorTooltip();if(!bg){var bd;for(var i=0;i<this.__Ih.length;i++ ){var bf=this.__Ih[i].item;if(!bf.isValid()){bd=bf;if(!(bf.getContentLocation)){return;}
...
;}
,getValid:function(){return this.__Ij;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var bh=[];for(var i=0;i<this.__Ih.length;i++ ){var bi=this.__Ih[i].item;if(!bi.getValid()){bh.push(bi.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=c){bh.push(this.getInvalidMessage());}
;return bh;}
,getInvalidFormItems:function(){var bj=[];for(var i=0;i<this.__Ih.length;i++ ){var bk=this.__Ih[i].item;if(!bk.getValid()){bj.push(bk);}
;}
;return bj;}
,reset:function(){for(var i=0;i<this.__Ih.length;i++ ){var bl=this.__Ih[i];bl.item.setValid(true);}
;this.__Ij=null;this._showToolTip(true);}
,setItemValid:function(bn,bm){this.__Ii[bn.toHashCode()]=bm;bn.setValid(bm);this.__Ip();}
,setFormValid:function(bo){this.__Ii[this.toHashCode()]=bo;this.__Ip();}
,__Ip:function(){var bq=this.__Ik;for(var br in this.__Ii){var bp=this.__Ii[br];bq=bp&&bq;if(bp==null){return;}
;}
;this._setValid(bq);this.__Ii={};this.fireEvent(j);}
},destruct:function(){this.__Ih=null;}
});}
)();
(function(){var a="qx.ui.form.validation.AsyncValidator";qx.Class.define(a,{extend:qx.core.Object,construct:function(b){qx.core.Object.call(this);this.__Iq=b;}
,members:{__Iq:null,__Ir:null,__ff:null,__Is:null,validate:function(c,e,d,f){this.__Is=false;this.__Ir=c;this.__ff=d;this.__Iq.call(f||this,this,e);}
,validateForm:function(g,h,i){this.__Is=true;this.__ff=h;this.__Iq.call(i,g,this);}
,setValid:function(k,j){if(this.__Is){if(j!==undefined){this.__ff.setInvalidMessage(j);}
;this.__ff.setFormValid(k);}
else {if(j!==undefined){this.__Ir.setInvalidMessage(j);}
;this.__ff.setItemValid(this.__Ir,k);}
;}
},destruct:function(){this.__ff=this.__Ir=null;}
});}
)();
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Dg=[];}
,members:{__Dg:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__Io(e)){var f=e.getSelection();}
else if(this.__Iv(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__Dg.push({item:e,init:f});}
,remove:function(g){for(var i=0;i<this.__Dg.length;i++ ){var h=this.__Dg[i];if(h.item===g){this.__Dg.splice(i,1);return true;}
;}
;return false;}
,reset:function(){for(var i=0;i<this.__Dg.length;i++ ){var j=this.__Dg[i];this.__It(j.item,j.init);}
;}
,resetItem:function(k){var l;for(var i=0;i<this.__Dg.length;i++ ){var m=this.__Dg[i];if(m.item===k){l=m.init;break;}
;}
;if(l===undefined){throw new Error(a);}
;this.__It(k,l);}
,__It:function(n,o){if(this._supportsValue(n)){n.setValue(o);}
else if(this.__Io(n)||this.__Iv(n)){n.setSelection(o);}
;}
,redefine:function(){for(var i=0;i<this.__Dg.length;i++ ){var p=this.__Dg[i].item;this.__Dg[i].init=this.__Iu(p);}
;}
,redefineItem:function(q){var r;for(var i=0;i<this.__Dg.length;i++ ){if(this.__Dg[i].item===q){r=this.__Dg[i];break;}
;}
;if(r===undefined){throw new Error(a);}
;r.init=this.__Iu(r.item);}
,__Iu:function(s){if(this._supportsValue(s)){return s.getValue();}
else if(this.__Io(s)||this.__Iv(s)){return s.getSelection();}
;}
,__Io:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.ui.core.ISingleSelection);}
,__Iv:function(v){var w=v.constructor;return qx.Class.hasInterface(w,qx.data.controller.ISelection);}
...
,events:{"changeValue":l},properties:{dateFormat:{check:c,apply:s},placeholder:{check:r,nullable:true,apply:E},appearance:{refine:true,init:D},focusable:{refine:true,init:true},width:{refine:true,init:120}},statics:{__Jn:null,__Jo:null,getDefaultDateFormatter:function(){var K=qx.locale.Date.getDateFormat(y).toString();if(K==this.__Jn){return this.__Jo;}
;if(this.__Jo){this.__Jo.dispose();}
;this.__Jo=new qx.util.format.DateFormat(K,qx.locale.Manager.getInstance().getLocale());this.__Jn=K;return this.__Jo;}
},members:{__Jp:null,_forwardStates:{focused:true,invalid:true},_setDefaultDateFormat:function(){this.setDateFormat(qx.ui.form.DateField.getDefaultDateFormatter());}
,_addLocaleChangeListener:function(){if(qx.core.Environment.get(x)){this.__Jp=qx.locale.Manager.getInstance().addListener(o,function(){this._setDefaultDateFormat();}
,this);}
;}
...
},destruct:function(){if(qx.core.Environment.get(x)){if(this.__Jp){qx.locale.Manager.getInstance().removeListenerById(this.__Jp);}
;}
;}
});}
)();
(function(){var a='format',b="short",c="HH:mm",d='_',e="stand-alone",f="HHmmsszz",g="cldr_date_format_",h="HHmm",j="narrow",k="HHmmss",l="qx.debug",m="This case should never happen.",n="cldr_time_format_",o="sat",p="abbreviated",q="wide",r="HH:mm:ss zz",s="full",t="cldr_pm",u="cldr_day_",v="_",w="cldr_month_",x="long",y="wed",z="medium",A="fri",B="cldr_am",C="tue",D="qx.locale.Date",E="mon",F="cldr_date_time_format_",G="sun",H="format",I="thu",J="HH:mm:ss",K='stand-alone';qx.Class.define(D,{statics:{__ig:qx.locale.Manager.getInstance(),getAmMarker:function(L){return this.__ig.localize(B,[],L);}
,getPmMarker:function(M){return this.__ig.localize(t,[],M);}
...
;var Q=[G,E,C,y,I,A,o];var R=[];for(var i=0;i<Q.length;i++ ){var N=u+S+v+length+v+Q[i];R.push(P?this.__ih(S,S===a?K:a,N,O):this.__ig.localize(N,[],O));}
;return R;}
...
;var W=[G,E,C,y,I,A,o];var T=u+U+v+length+v+W[Y];return X?this.__ih(U,U===a?K:a,T,V):this.__ig.localize(T,[],V);}
...
;var bd=[];for(var i=0;i<12;i++ ){var ba=w+be+v+length+v+(i+1);bd.push(bc?this.__ih(be,be===a?K:a,ba,bb):this.__ig.localize(ba,[],bb));}
;return bd;}
...
;var bf=w+bg+v+length+v+(bi+1);return bj?this.__ih(bg,bg===a?K:a,bf,bh):this.__ig.localize(bf,[],bh);}
,getDateFormat:function(bm,bk){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bm,[b,z,x,s]);}
;var bl=g+bm;return this.__ig.localize(bl,[],bk);}
,getDateTimeFormat:function(br,bp,bn){var bq=F+br;var bo=this.__ig.localize(bq,[],bn);if(bo==bq){bo=bp;}
;return bo;}
,getTimeFormat:function(bv,bs){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bv,[b,z,x,s]);}
;var bu=n+bv;var bt=this.__ig.localize(bu,[],bs);if(bt!=bu){return bt;}
...
else {bK=this.__ig.getTerritory()||this.__ig.getLanguage();}
;return bK.toUpperCase();}
,__ih:function(bN,bM,bQ,bP){var bO=this.__ig.localize(bQ,[],bP);if(bO==bQ){var bL=bQ.replace(d+bN+d,d+bM+d);return this.__ig.localize(bL,[],bP);}
else {return bO;}
;}
}});}
)();
...
(function(){var a="-",b="QQQ",c='W',d="qqqq",e="h",f="KK",g='L',h=" is not supported",m="Z",n="Date string '",o="GGGGG",p='abbreviated',q="Y+",r=". Wildcard ",s="eeeee",t="(\\d\\d?)",u="LL",v="abbreviated",w="EEEE",x="^",y=":",z='y',A="quoted_literal",B="weekOfYear",C='3rd quarter',D="SSS",E='a',F="day",G="(-*",H="qqq",I="(\\d\\d)",J="locale",K="K",L='q',M="Provided date is invalid",N="pm",O="Error parsing date '",P='Anno Domini',Q='G',R="a",S='2nd quarter',T="yyyy-MM-dd'T'HH:mm:ss",U="GMT",V="HH:mm:ss",W="dd",X="+",Y="qx.util.format.DateFormat",eo='weekDay',ek="am",ep="eee",el="quarter",em="ee",ej="q",en="QQQQ",eu="HHmmss",ev="ww",ew="H",ex='Y+',eq="wide",er='wide',es="(\\d\\d\\d?)",et="(GMT[\\+\\-]\\d\\d:\\d\\d)",eB="_applyLocale",ff="Q",eC="c",eD="(",ey="weekOfMonth",ez="ccc",gg="long",eA="Malformed date format: ",eE="EEEEE",eF='Y',eG="EE",eL="GGG",eM='-',eN="w",eH="mm",eI='h',eJ="S",eK='s',eS='A',eT="yyyy-MM-dd",eU="G",eV='z',eO="GG",eP=")",eQ="kk",eR="ss",fa="([\\+\\-]\\d\\d\\d\\d)",fb="default",gk='BC',fc='H',eW='S',eX='1st quarter',gi="stand-alone",eY="0",fd="MMMM",fe='c',fq="d",fp='Q',fo='Q1',fu="eeee",ft="QQ",fs="LLLLL",fr="HH",fj='k',fi="m",fh="DDD",fg="D",fn='D',fm="LLLL",fl="(\\d\\d*?)",fk='e',fB="': the value for day or month is too large",fA="literal",fz='K',fy="L",fF="hh",fE="ms",fD="(\\d\\d?\\d?)",fC="cccc",fx="e",fw='y+',fv="|",fQ="W",fP="dayOfYear",fO="GGGG",fU='Q2',fT='B',fS="MM",fR='Q4',fJ="y+",fI='narrow',fH="ccccc",fG="E",fN="wildcard",fM='',fL="' does not match the date format: ",fK="short",gb='d',ga="unkown",fY="narrow",fX="\\d?",gf="'",ge="k",gd='Before Christ',gc='Z',fW="qx.debug",fV='m',dR=" ",dQ="yyyy-MM-dd'T'HH:mm:ss'Z'",gl="String",dO='Q3',dP="z",dN="DD",gj="min",dL="",dM='M',dK="sec",gh='isoUtcDateTime',dI='4th quarter',dJ="(\\d?)",dH="SS",eb="MMM",ec="s",dY="M",ea='w',dW="EEE",dX="$",dV="?",dG='AD',dT="\\d",dU="+?",dS="hour",ei="MMMMM",eg='E',eh='month',ee="LLL",ef="qq",ed="format";qx.Class.define(Y,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(gn,gm){qx.core.Object.call(this);if(!gm){this.__hq=qx.locale.Manager.getInstance().getLocale();this.__hr=qx.locale.Manager.getInstance().bind(J,this,J);}
else {this.__hq=gm;this.setLocale(gm);}
;this.__hs=this.__hq;if(gn!=null){this.__ht=gn.toString();if(this.__ht in qx.util.format.DateFormat.ISO_MASKS){if(this.__ht===gh){this.__hu=true;}
;this.__ht=qx.util.format.DateFormat.ISO_MASKS[this.__ht];}
;}
else {this.__ht=qx.locale.Date.getDateFormat(gg,this.__hq)+dR+qx.locale.Date.getDateTimeFormat(eu,V,this.__hq);}
;}
,properties:{locale:{apply:eB,nullable:true,check:gl}},statics:{getDateInstance:function(){var go=qx.util.format.DateFormat;var gp=qx.locale.Date.getDateFormat(fK)+dL;if(go._dateInstance==null||go._dateInstance.__ht!=gp){go._dateInstance=new go(gp);}
;return go._dateInstance;}
,getDateTimeInstance:function(){var gq=qx.util.format.DateFormat;var gr=qx.locale.Date.getDateFormat(gg)+dR+qx.locale.Date.getDateTimeFormat(eu,V);if(gq._dateTimeInstance==null||gq._dateTimeInstance.__ht!=gr){gq._dateTimeInstance=new gq(gr);}
;return gq._dateTimeInstance;}
,ASSUME_YEAR_2000_THRESHOLD:30,ISO_MASKS:{isoDate:eT,isoTime:V,isoDateTime:T,isoUtcDateTime:dQ},AM_MARKER:ek,PM_MARKER:N},members:{__hr:null,__hq:null,__hs:null,__ht:null,__hv:null,__hw:null,__hx:null,__hu:null,__hy:function(gt,gs){var gu=dL+(gt<0?((-1)*gt):gt);while(gu.length<gs){gu=eY+gu;}
;return gt<0?a+gu:gu;}
,__hz:function(gv){var gw=new Date(gv.getTime());var gx=gw.getDate();while(gw.getMonth()!=0){gw.setDate(-1);gx+=gw.getDate()+1;}
;return gx;}
,__hA:function(gy){return new Date(gy.getTime()+(3-((gy.getDay()+6)%7))*86400000);}
,__hB:function(gz){var gB=this.__hA(gz);var gC=gB.getFullYear();var gA=this.__hA(new Date(gC,0,4));return Math.floor(1.5+(gB.getTime()-gA.getTime())/86400000/7);}
,__hC:function(gD){var gF=this.__hA(gD);var gE=this.__hA(new Date(gD.getFullYear(),gD.getMonth(),4));return Math.floor(1.5+(gF.getTime()-gE.getTime())/86400000/7);}
,__hD:function(gG){var gH=this.__hA(gG);return gH.getFullYear();}
,__hE:function(gJ){var gI=new Date(gJ,2,1);gI.setDate(-1);return gI.getDate()+1===29;}
,__hF:function(gO,gP){var gN=0;var gQ=0;if(!gP){gP=1971;}
...
,__hG:function(gU,gS,gR){var gT;switch(gS){case 11:gT=gU-1;if(gU!=this.__hD(new Date(gT,gS,gR))){gT=gU;}
;break;case 0:gT=gU+1;if(gU!=this.__hD(new Date(gT,gS,gR))){gT=gU;}
;break;default:gT=gU;};return gT;}
,_applyLocale:function(gW,gV){this.__hq=gW===null?this.setLocale(this.__hs):gW;}
,format:function(hn){if(hn==null){return null;}
;if(isNaN(hn.getTime())){if(qx.core.Environment.get(fW)){qx.log.Logger.error(M);}
;return null;}
;if(this.__hu){hn=new Date(hn.getUTCFullYear(),hn.getUTCMonth(),hn.getUTCDate(),hn.getUTCHours(),hn.getUTCMinutes(),hn.getUTCSeconds(),hn.getUTCMilliseconds());}
;var ha=this.__hq;var hf=hn.getFullYear();var hb=hn.getMonth();var he=hn.getDate();var hi=hn.getDay();var hl=hn.getHours();var hs=hn.getMinutes();var hj=hn.getSeconds();var hq=hn.getMilliseconds();var hh=hn.getTimezoneOffset();var hm=hh>0?1:-1;var gX=Math.floor(Math.abs(hh)/60);var hp=Math.abs(hh)%60;this.__hH();var hr=dL;for(var i=0;i<this.__hx.length;i++ ){var hg=this.__hx[i];if(hg.type==fA){hr+=hg.text;}
else {var gY=hg.character;var hk=hg.size;var ht=dV;switch(gY){case z:if(hk==2){ht=this.__hy(hf%100,2);}
else {var hc=Math.abs(hf);ht=hc+dL;if(hk>ht.length){for(var j=ht.length;j<hk;j++ ){ht=eY+ht;}
;}
;if(hf<0){ht=a+ht;}
;}
;break;case eF:ht=this.__hD(hn)+dL;var hc=ht.replace(eM,fM);if(hk>ht.length){for(var j=hc.length;j<hk;j++ ){hc=eY+hc;}
;}
;ht=ht.indexOf(a)!=-1?a+hc:hc;break;case Q:if(hk>=1&&hk<=3){ht=hf>0?dG:gk;}
else if(hk==4){ht=hf>0?P:gd;}
else if(hk==5){ht=hf>0?eS:fT;}
;break;case fp:if(hk==1||hk==2){ht=this.__hy(parseInt(hb/4)+1,hk);}
;if(hk==3){ht=fp+(parseInt(hb/4)+1);}
;break;case L:if(hk==1||hk==2){ht=this.__hy(parseInt(hb/4)+1,hk);}
;if(hk==3){ht=fp+(parseInt(hb/4)+1);}
;break;case fn:ht=this.__hy(this.__hz(hn),hk);break;case gb:ht=this.__hy(he,hk);break;case ea:ht=this.__hy(this.__hB(hn),hk);break;case c:ht=this.__hC(hn);break;case eg:if(hk>=1&&hk<=3){ht=qx.locale.Date.getDayName(v,hi,ha,ed,true);}
else if(hk==4){ht=qx.locale.Date.getDayName(eq,hi,ha,ed,true);}
else if(hk==5){ht=qx.locale.Date.getDayName(fY,hi,ha,ed,true);}
;break;case fk:var hd=qx.locale.Date.getWeekStart(ha);var ho=1+((hi-hd>=0)?(hi-hd):7+(hi-hd));if(hk>=1&&hk<=2){ht=this.__hy(ho,hk);}
...
;break;case dM:if(hk==1||hk==2){ht=this.__hy(hb+1,hk);}
...
;break;case g:if(hk==1||hk==2){ht=this.__hy(hb+1,hk);}
...
;break;case E:ht=(hl<12)?qx.locale.Date.getAmMarker(ha):qx.locale.Date.getPmMarker(ha);break;case fc:ht=this.__hy(hl,hk);break;case fj:ht=this.__hy((hl==0)?24:hl,hk);break;case fz:ht=this.__hy(hl%12,hk);break;case eI:ht=this.__hy(((hl%12)==0)?12:(hl%12),hk);break;case fV:ht=this.__hy(hs,hk);break;case eK:ht=this.__hy(hj,hk);break;case eW:ht=this.__hy(hq,3);if(hk<ht.length){ht=ht.substr(0,hk);}
else {while(hk>ht.length){ht+=eY;}
;}
;break;case eV:if(hk>=1&&hk<=4){ht=U+((hm>0)?a:X)+this.__hy(Math.abs(gX),2)+y+this.__hy(hp,2);}
;break;case gc:if(hk>=1&&hk<=3){ht=((hm>0)?a:X)+this.__hy(Math.abs(gX),2)+this.__hy(hp,2);}
else {ht=U+((hm>0)?a:X)+this.__hy(Math.abs(gX),2)+y+this.__hy(hp,2);}
;break;};hr+=ht;}
;}
;return hr;}
,parse:function(hA){this.__hI();var hD=this.__hv.regex.exec(hA);if(hD==null){throw new Error(n+hA+fL+this.__ht);}
;var hu={era:1,year:1970,quarter:1,month:0,day:1,dayOfYear:1,hour:0,ispm:false,weekDay:4,weekYear:1970,weekOfMonth:1,weekOfYear:1,min:0,sec:0,ms:0};var hy=1;var hv=false;var hw=false;for(var i=0;i<this.__hv.usedRules.length;i++ ){var hx=this.__hv.usedRules[i];var hz=hD[hy];if(hx.field!=null){hu[hx.field]=parseInt(hz,10);}
else {hx.manipulator(hu,hz,hx.pattern);}
;if(hx.pattern==q){var hB=false;for(var k=0;k<this.__hv.usedRules.length;k++ ){if(this.__hv.usedRules[k].pattern==fw){hB=true;break;}
;}
;if(!hB){hv=true;}
;}
;if(hx.pattern.indexOf(fg)!=-1){var hF=false;for(var k=0;k<this.__hv.usedRules.length;k++ ){if(this.__hv.usedRules[k].pattern.indexOf(fq)!=-1){hF=true;break;}
;}
;if(!hF){hw=true;}
;}
;hy+=(hx.groups==null)?1:hx.groups;}
;if(hv){hu.year=this.__hG(hu.weekYear,hu.month,hu.day);}
;if(hw){var hE=this.__hF(hu.dayOfYear,hu.year);hu.month=hE.month;hu.day=hE.day;}
;if(hu.era<0&&(hu.year*hu.era<0)){hu.year=hu.year*hu.era;}
;var hC=new Date(hu.year,hu.month,hu.day,(hu.ispm)?(hu.hour+12):hu.hour,hu.min,hu.sec,hu.ms);if(this.__hu){hC=new Date(hC.getUTCFullYear(),hC.getUTCMonth(),hC.getUTCDate(),hC.getUTCHours(),hC.getUTCMinutes(),hC.getUTCSeconds(),hC.getUTCMilliseconds());}
;if(hu.month!=hC.getMonth()||hu.year!=hC.getFullYear()){throw new Error(O+hA+fB);}
;return hC;}
,__hH:function(){if(this.__hx!=null){return;}
;this.__hx=[];var hK;var hI=0;var hM=dL;var hG=this.__ht;var hJ=fb;var i=0;while(i<hG.length){var hL=hG.charAt(i);switch(hJ){case A:if(hL==gf){if(i+1>=hG.length){i++ ;break;}
;var hH=hG.charAt(i+1);if(hH==gf){hM+=hL;i++ ;}
else {i++ ;hJ=ga;}
;}
else {hM+=hL;i++ ;}
;break;case fN:if(hL==hK){hI++ ;i++ ;}
else {this.__hx.push({type:fN,character:hK,size:hI});hK=null;hI=0;hJ=fb;}
;break;default:if((hL>=E&&hL<=eV)||(hL>=eS&&hL<=gc)){hK=hL;hJ=fN;}
else if(hL==gf){if(i+1>=hG.length){hM+=hL;i++ ;break;}
;var hH=hG.charAt(i+1);if(hH==gf){hM+=hL;i++ ;}
;i++ ;hJ=A;}
else {hJ=fb;}
;if(hJ!=fb){if(hM.length>0){this.__hx.push({type:fA,text:hM});hM=dL;}
;}
else {hM+=hL;i++ ;}
;break;};}
;if(hK!=null){this.__hx.push({type:fN,character:hK,size:hI});}
else if(hM.length>0){this.__hx.push({type:fA,text:hM});}
;}
,__hI:function(){if(this.__hv!=null){return;}
;var hQ=this.__ht;this.__hK();this.__hH();var hW=[];var hR=x;for(var hO=0;hO<this.__hx.length;hO++ ){var hX=this.__hx[hO];if(hX.type==fA){hR+=qx.lang.String.escapeRegexpChars(hX.text);}
else {var hP=hX.character;var hS=hX.size;var hU;for(var hY=0;hY<this.__hw.length;hY++ ){var hT=this.__hw[hY];if(this.__hJ(hT,hP,hS)){hU=hT;break;}
;}
...
;this.__hv={regex:hN,"usedRules":hW,pattern:hR};}
,__hJ:function(ib,ic,id){if(ic===z&&ib.pattern===fw){ib.regex=ib.regexFunc(id);return true;}
else if(ic===eF&&ib.pattern===ex){ib.regex=ib.regexFunc(id);return true;}
else {return ic==ib.pattern.charAt(0)&&id==ib.pattern.length;}
;}
,__hK:function(){var it=qx.util.format.DateFormat;var iw=qx.lang.String;if(this.__hw!=null){return;}
;var iu=this.__hw=[];var ip=qx.locale.Date.getAmMarker(this.__hq).toString()||it.AM_MARKER;var iD=qx.locale.Date.getPmMarker(this.__hq).toString()||it.PM_MARKER;var ig=this.__hq;var il=function(iG,iH){iH=parseInt(iH,10);if(iH>=0){if(iH<it.ASSUME_YEAR_2000_THRESHOLD){iH+=2000;}
...
},destruct:function(){if(this.__hr!=null){qx.locale.Manager.getInstance().removeBinding(this.__hr);}
;this.__hx=this.__hv=this.__hw=null;}
});}
)();
...
;var bI=this.getChildControl(bs);bI.addState(I);bG.add(bI,{column:0,row:0});this.__Jq=[];for(var i=0;i<7;i++ ){bI=this.getChildControl(F+i);bG.add(bI,{column:i+1,row:0});this.__Jq.push(bI);}
;this.__Jr=[];this.__Js=[];for(var y=0;y<6;y++ ){var bI=this.getChildControl(R+(y+1));bG.add(bI,{column:0,row:y+1});this.__Js.push(bI);for(var x=0;x<7;x++ ){var bI=this.getChildControl(w+((y*7)+x));bG.add(bI,{column:x+1,row:y+1});this.__Jr.push(bI);}
;}
...
else {var bL=(bN==null)?-1:bN.getDate();for(var i=0;i<6*7;i++ ){var bO=this.__Jr[i];if(bO.hasState(M)){if(bO.hasState(p)){bO.removeState(p);}
;}
...
;this.__Jq=this.__Jr=this.__Js=null;}
});}
)();
...
(function(){var a="changeModel",b="modelSelection[0]",c="qx.core.Object",d=":\n",e="qx.debug",f="_applyTarget",g="qx.data.controller.Form",h="changeTarget",j="value",k=" of ",l="Could not bind property ",m=".",n="No target is set.",o="qx.ui.form.Form",p="_applyModel";qx.Class.define(g,{extend:qx.core.Object,construct:function(q,s,r){qx.core.Object.call(this);this._selfUpdate=!!r;this.__sG={};if(q!=null){this.setModel(q);}
;if(s!=null){this.setTarget(s);}
;}
,properties:{model:{check:c,apply:p,event:a,nullable:true,dereference:true},target:{check:o,apply:f,event:h,nullable:true,init:null,dereference:true}},members:{__sH:null,__sG:null,addBindingOptions:function(name,v,w){this.__sG[name]=[v,w];if(this.getModel()==null||this.getTarget()==null){return;}
;var t=this.getTarget().getItems()[name];var u=this.__sK(t)?b:j;this.__sH.removeTarget(t,u,name);this.__sH.addTarget(t,u,name,!this._selfUpdate,v,w);}
...
;if(this.__sG[name]&&this.__sG[name][1]){B=this.__sG[name][1].converter(B);}
;E[F[i]]=B;}
else {if(!E[F[i]]){E[F[i]]={};}
;E=E[F[i]];}
;}
...
;var G=this.getTarget().getItems();for(var name in G){var J=G[name];var I=this.__sK(J)?b:j;var H=this.__sG[name];H=H&&this.__sG[name][1];qx.data.SingleValueBinding.updateTarget(J,I,this.getModel(),name,H);}
;}
,_applyTarget:function(L,K){if(K!=null){this.__sJ(K);}
;if(this.getModel()==null){return;}
;if(L!=null){this.__sI();}
;}
,_applyModel:function(O,M){if(this.__sH!=null&&O==null){this.__sH.setModel(null);}
;if(this.__sH!=null&&this.getTarget()!=null){var N=this.getTarget().getItems();for(var name in N){var P=N[name];var Q=this.__sK(P)?b:j;this.__sH.removeTarget(P,Q,name);}
;}
;if(this.__sH!=null){this.__sH.setModel(O);}
;if(this.getTarget()==null){return;}
;if(O!=null){this.__sI();}
;}
,__sI:function(){if(this.__sH==null){this.__sH=new qx.data.controller.Object(this.getModel());}
;var U=this.getTarget().getItems();for(var name in U){var R=U[name];var S=this.__sK(R)?b:j;var T=this.__sG[name];try{if(T==null){this.__sH.addTarget(R,S,name,!this._selfUpdate);}
else {this.__sH.addTarget(R,S,name,!this._selfUpdate,T[0],T[1]);}
;}
catch(V){if(qx.core.Environment.get(e)){this.warn(l+name+k+this.getModel()+d+V.stack);}
;}
;}
;this.getTarget().redefineResetter();}
,__sJ:function(W){if(this.__sH==null){return;}
;var ba=W.getItems();for(var name in ba){var Y=ba[name];var X=this.__sK(Y)?b:j;this.__sH.removeTarget(Y,X,name);}
;}
,__sK:function(bb){return qx.Class.hasInterface(bb.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(bb.constructor,qx.ui.form.IModelSelection);}
},destruct:function(){if(this.__sH){this.__sH.dispose();}
;}
});}
)();
