...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Gallery.ca31117ec003.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
}});qx.Class.define(f,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bv){demobrowser.demo.virtual.AbstractGallery.call(this,bv);this.__Ol=new demobrowser.demo.virtual.GalleryCell();}
,members:{__Ol:null,_createScroller:function(){var bw=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.WidgetCell(this);bw.getPane().addLayer(this.layer);new qx.ui.virtual.behavior.Prefetch(bw,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});return bw;}
...
;if(by){this.__Ol.updateStates(bx,{selected:1});}
else {this.__Ol.updateStates(bx,{});}
;}
;}
...
;var bE=this.__Ol.getCellWidget(bF,bD);bE.setUserData(o,bH);return bE;}
,poolCellWidget:function(bJ){this.__Ol.pool(bJ);}
},destruct:function(){this._disposeObjects(m,r);}
...
;}
,properties:{scroller:{check:i,nullable:true,init:null,apply:g},interval:{check:k,init:200,apply:a}},members:{__wN:null,__wO:null,__du:null,__wP:null,__wQ:null,setPrefetchX:function(o,r,p,q){this.__wN=[o,r,p,q];}
,setPrefetchY:function(v,u,t,s){this.__wO=[v,u,t,s];}
,_onInterval:function(){var w=this.__wN;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__wO;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(z,y){if(y){if(this.__wP){y.getChildControl(j).removeListenerById(this.__wP);}
;if(this.__wQ){y.getChildControl(l).removeListenerById(this.__wQ);}
;}
...
;this.__wP=z.getChildControl(j).addListener(f,this.__du.restart,this.__du);this.__wQ=z.getChildControl(l).addListener(f,this.__du.restart,this.__du);}
else {this.__du.stop();}
;}
,_applyInterval:function(B,A){this.__du.setInterval(B);}
},destruct:function(){this.setScroller(null);this.__wN=this.__wO=null;this._disposeObjects(h);}
});}
)();
...
;this.__Fz=[];this.__FA=[];this._isHorizontal=(w||c)==c;}
,properties:{defaultLineColor:{init:h,check:s,themeable:true},defaultLineSize:{init:1,check:t,themeable:true}},members:{__Fz:null,__FA:null,isHorizontal:function(){return this._isHorizontal;}
...
;this.__Fz[B]=A;if(this.__FB(B)){this.updateLayerData();}
;}
...
;this.__FA[C]=D;if(this.__FB(C)){this.updateLayerData();}
;}
,__FB:function(E){if(this._isHorizontal){var I=this.getFirstColumn();var G=I+this.getColumnSizes().length-1;return E>=I&&E<=G;}
else {var H=this.getFirstRow();var F=H+this.getRowSizes().length-1;return E>=H&&E<=F;}
;}
,getLineSize:function(J){return this.__FA[J]||this.getDefaultLineSize();}
,getLineColor:function(K){return this.__Fz[K]||this.getDefaultLineColor();}
,__FC:function(P,M,O){var top=0;var L,N;for(var y=0;y<O.length-1;y++ ){L=this.getLineColor(M+y);N=this.getLineSize(M+y);top+=O[y];P.push(m,g,f+N+q,j,d,top-((N>1)?Math.floor(N/2):1),q,b,L,n,a);}
;}
,__FD:function(T,Q,S){var V=0;var R,U;for(var x=0;x<S.length-1;x++ ){R=this.getLineColor(Q+x);U=this.getLineSize(Q+x);V+=S[x];T.push(m,g,k+U+q,l,u,p,V-((U>1)?Math.floor(U/2):1),q,b,R,n,a);}
;}
,_fullUpdate:function(ba,bb,X,W){var Y=[];if(this._isHorizontal){this.__FC(Y,ba,X);}
else {this.__FD(Y,bb,W);}
