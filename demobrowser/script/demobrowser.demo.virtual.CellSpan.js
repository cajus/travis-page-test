...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.CellSpan.f7d8482ca05f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,getCellProperties:function(s,q){var r=(s+q)%2==0?c:h;this.__Oi.setBackgroundColor(r);return this.__Oi.getCellProperties(s+a+q);}
...
,__KV:function(G,A,C,H,top,F,D){var B=this._cellProvider.getCellProperties(A,C);var E=B.insets||[0,0];G.push(f,j,o,H,k,b,top,k,this._getCellSizeStyle(F,D,E[0],E[1]),B.style||c,g,m,B.classes||c,g,B.attributes||c,d,B.content||c,a);}
,_fullUpdate:function(P,K,T,N){var S=[];var M=this._spanManager.findCellsInWindow(P,K,T.length,N.length);if(M.length>0){var O=this._spanManager.getCellBounds(M,P,K);var I=this._spanManager.computeCellSpanMap(M,P,K,T.length,N.length);for(var i=0,l=M.length;i<l;i++ ){var R=M[i];var J=O[i];this.__KV(S,R.firstRow,R.firstColumn,J.left,J.top,J.width,J.height);}
;}
else {I=[];for(var i=0;i<T.length;i++ ){I[P+i]=[];}
;}
;var U=0;var top=0;var W=P;var L=K;for(var x=0;x<T.length;x++ ){var U=0;var L=K;var Q=T[x];for(var y=0;y<N.length;y++ ){var V=N[y];if(!I[W][L]){this.__KV(S,W,L,U,top,V,Q);}
;L++ ;U+=V;}
;top+=Q;W++ ;}
...
;return this.__Gx(K,M);}
,__Gx:function(P,N){var O=[];for(var Q in P){if(N[Q]){O.push(N[Q]);}
;}
;return O;}
...
(function(){var a="cell.rowspan",b="x",c="cell.column",d="cell.colspan",e="qx.ui.virtual.layer.WidgetCellSpan",f="_spanManager",g="_cellLayer",h="cell.spanning",j="cell.row",k="cell.empty";qx.Class.define(e,{extend:qx.ui.virtual.layer.Abstract,include:[qx.ui.core.MChildrenHandling],construct:function(n,o,m){qx.ui.virtual.layer.Abstract.call(this);this.setZIndex(12);this._spanManager=new qx.ui.virtual.layer.CellSpanManager(o,m);this._cellProvider=n;this.__sF=[];this._cellLayer=new qx.ui.virtual.layer.WidgetCell(this.__KO());this._cellLayer.setZIndex(0);this._setLayout(new qx.ui.layout.Grow());this._add(this._cellLayer);}
...
;return A;}
,__KO:function(){var self=this;var D=this._cellProvider;var C=this.__sF;var B={getCellWidget:function(G,E){if(!self._spanMap[G][E]){var F=D.getCellWidget(G,E);}
...
,__KP:function(K,L,J,I){this._cells=this._spanManager.findCellsInWindow(K,L,J,I);if(this._cells.length>0){this._bounds=this._spanManager.getCellBounds(this._cells,K,L);this._spanMap=this._spanManager.computeCellSpanMap(this._cells,K,L,J,I);}
else {this._bounds=[];this._spanMap=[];for(var i=0;i<J;i++ ){this._spanMap[K+i]=[];}
;}
;}
,__KQ:function(){var Q=this.getChildren();for(var i=Q.length-1;i>=0;i-- ){var P=Q[i];if(P!==this._cellLayer){this._cellProvider.poolCellWidget(P);this._remove(P);}
;}
...
;}
;}
,_fullUpdate:function(S,U,T,R){this.__KP(S,U,T.length,R.length);this.__KQ();this._cellLayer.fullUpdate(S,U,T,R);}
,_updateLayerWindow:function(W,Y,X,V){this.__KP(W,Y,X.length,V.length);this.__KQ();this._cellLayer.updateLayerWindow(W,Y,X,V);}
