...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow.9a8d9531c906.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var x=new qx.ui.layout.LineSizeIterator(D,this.getSpacingX());var B=z.top;while(x.hasMoreLines()){var C=x.computeNextLine(A);this.__Jc(C,B,A,z);B+=C.height+this.getSpacingY();}
;}
,__Jc:function(L,K,I,E){var H=qx.ui.layout.Util;var F=E.left;if(this.getAlignX()!=l){F=E.left+I-L.width;if(this.getAlignX()==j){F=E.left+Math.round((I-L.width)/2);}
;}
...
,getHeightForWidth:function(P){return this.__Jd(P).height;}
...
;return S;}
,__Jd:function(T){var U=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var V=0;var W=0;var X=0;while(U.hasMoreLines()){var Y=U.computeNextLine(T);X+=1;W=Math.max(W,Y.width);V+=Y.height;}
;return {width:W,height:V+this.getSpacingY()*(X-1)};}
}});}
)();
(function(){var a="No more lines to compute",b="qx.ui.layout.LineSizeIterator";qx.Class.define(b,{extend:Object,construct:function(d,c){this.__hd=d;this.__GI=c;this.__GJ=d.length>0;this.__GK=0;}
,members:{__hd:null,__GI:null,__GJ:null,__GK:null,computeNextLine:function(k){var k=k||Infinity;if(!this.__GJ){throw new Error(a);}
;var p=this.__hd;var n=0;var f=0;var o=[];var j=[];for(var i=this.__GK;i<p.length;i++ ){var m=p[i];var q=m.getSizeHint();var g=this.__GL(i);var h=q.width+g;var e=i==this.__GK;if(!e&&f+h>k){this.__GK=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);j.push(g);f+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__GK=i+1;break;}
;}
;if(i>=p.length){this.__GJ=false;}
;return {height:n,width:f,children:o,gapsBefore:j};}
,__GL:function(r){var s=r==this.__GK;if(s){return this.__hd[r].getMarginLeft();}
else {return Math.max(this.__hd[r-1].getMarginRight(),this.__hd[r].getMarginLeft(),this.__GI);}
;}
,hasMoreLines:function(){return this.__GJ;}
}});}
)();
