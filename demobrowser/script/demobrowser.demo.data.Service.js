...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Service.e60dfda0b419.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="url: ",b="qx.debug.io",c="qx.bom.request.Script",d="Invalid state",e="head",f="error",g="loadend",h="qx.debug",i="script",j="load",k="Unknown response headers",l="browser.documentmode",m="abort",n="",o="Received native readyState: loaded",p="readystatechange",q="Response header cannot be determined for ",r="requests made with script transport.",s="opera",t="unknown",u="Open native request with ",v="Response headers cannot be determined for",w="mshtml",x="engine.name",y="Detected error",z="Send native request",A="on",B="timeout",C="Unknown environment key at this phase",D="Received native load";qx.Bootstrap.define(c,{construct:function(){this.__Ll();this.__Lg=qx.Bootstrap.bind(this._onNativeLoad,this);this.__Lh=qx.Bootstrap.bind(this._onNativeError,this);this.__mj=qx.Bootstrap.bind(this._onTimeout,this);this.__Li=document.head||document.getElementsByTagName(e)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__Ln()?0:15000;}
,events:{"readystatechange":c,"error":c,"loadend":c,"timeout":c,"abort":c,"load":c},members:{readyState:null,status:null,statusText:null,timeout:null,__Lj:null,on:function(name,E,F){this._emitter.on(name,E,F);return this;}
,open:function(H,G){if(this.__mx){return;}
;this.__Ll();this.__mm=null;this.__uh=G;if(this.__Lq(b)){qx.Bootstrap.debug(qx.bom.request.Script,u+a+G);}
;this._readyStateChange(1);}
,setRequestHeader:function(I,J){if(this.__mx){return null;}
;var K={};if(this.readyState!==1){throw new Error(d);}
;K[I]=J;this.__uh=qx.util.Uri.appendParamsToUrl(this.__uh,K);return this;}
,send:function(){if(this.__mx){return null;}
;var M=this.__Lo(),L=this.__Li,N=this;if(this.timeout>0){this.__nt=window.setTimeout(this.__mj,this.timeout);}
;if(this.__Lq(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
;L.insertBefore(M,L.firstChild);window.setTimeout(function(){N._readyStateChange(2);N._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__mx){return null;}
;this.__mm=true;this.__Lp();this._emit(m);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(O){if(this.__mx){return null;}
;if(this.__Lq(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__mx){return null;}
;if(this.__Lq(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__Lj=P;}
,dispose:function(){var Q=this.__Lk;if(!this.__mx){if(Q){Q.onload=Q.onreadystatechange=null;this.__Lp();}
;if(this.__nt){window.clearTimeout(this.__nt);}
;this.__mx=true;}
;}
,isDisposed:function(){return !!this.__mx;}
,_getUrl:function(){return this.__uh;}
,_getScriptElement:function(){return this.__Lk;}
,_onTimeout:function(){this.__Lm();if(!this.__Ln()){this._emit(f);}
;this._emit(B);if(!this.__Ln()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__Lk,R=this.__Lj,T=this;if(this.__mm){return;}
;if(this.__Lq(x)===w&&this.__Lq(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__Lq(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__Lq(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__Lq(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__nt){window.clearTimeout(this.__nt);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__Lm();this._emit(f);this._emit(g);}
,__Lk:null,__Li:null,__uh:n,__Lg:null,__Lh:null,__mj:null,__nt:null,__mm:null,__mx:null,__Ll:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__Lp();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__Lm:function(){this.__Lp();this.readyState=4;this.status=0;this.statusText=null;}
,__Ln:function(){var W=this.__Lq(x)===w&&this.__Lq(l)<9;var V=this.__Lq(x)===s;return !(W||V);}
,__Lo:function(){var X=this.__Lk=document.createElement(i);X.src=this.__uh;X.onerror=this.__Lh;X.onload=this.__Lg;if(this.__Lq(x)===w&&this.__Lq(l)<9){X.onreadystatechange=this.__Lg;}
;return X;}
,__Lp:function(){var Y=this.__Lk;if(Y&&Y.parentNode){this.__Li.removeChild(Y);}
;}
,__Lq:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__mB();}
,members:{responseJson:null,__gJ:null,__ms:null,__mt:null,__mu:null,__mv:null,__mw:null,__mx:null,__my:e,open:function(t,p){if(this.__mx){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__mu=false;q=this.__ms||b;s=this.__mt||this.__my+n+this.__gJ+m;if(!this.__mt){this.constructor[this.__gJ]=this;}
else {if(!window[this.__mt]){this.__mv=true;window[this.__mt]=function(u){o.callback(u);}
;}
else {if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,k+this.__mt+j);}
;}
;}
;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,h+s);}
;r[q]=s;this.__mw=p=qx.util.Uri.appendParamsToUrl(p,r);this.__mA(c,[t,p]);}
,callback:function(v){if(this.__mx){return;}
;this.__mu=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__gJ]=undefined;this.__mz();}
,setCallbackParam:function(w){this.__ms=w;return this;}
,setCallbackName:function(name){this.__mt=name;return this;}
,setPrefix:function(x){this.__my=x;}
,getGeneratedUrl:function(){return this.__mw;}
,dispose:function(){this.__mz();this.__mA(d);}
,_onNativeLoad:function(){this.status=this.__mu?200:500;this.__mA(i);}
,__mz:function(){if(this.__mv&&window[this.__mt]){window[this.__mt]=undefined;this.__mv=false;}
;}
,__mA:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__mB:function(){this.__gJ=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
...
(function(){var a="changeModel",b="qx.core.Object",c="qx.data.controller.Object",d="get",e="[",f="reset",g="_applyModel",h="last";qx.Class.define(c,{extend:qx.core.Object,construct:function(j){qx.core.Object.call(this);this.__jk={};this.__yW=[];if(j!=null){this.setModel(j);}
;}
,properties:{model:{check:b,event:a,apply:g,nullable:true,dereference:true}},members:{__yW:null,__jk:null,_applyModel:function(o,l){for(var i=0;i<this.__yW.length;i++ ){var t=this.__yW[i][0];var q=this.__yW[i][1];var n=this.__yW[i][2];var r=this.__yW[i][3];var p=this.__yW[i][4];var m=this.__yW[i][5];if(l!=undefined&&!l.isDisposed()){this.__Kg(t,q,n,l);}
;if(o!=undefined){this.__Kf(t,q,n,r,p,m);}
...
;}
;}
,addTarget:function(z,w,u,x,y,v){this.__yW.push([z,w,u,x,y,v]);this.__Kf(z,w,u,x,y,v);}
,__Kf:function(F,D,B,G,E,A){if(this.getModel()==null){return;}
;var C=this.getModel().bind(B,F,D,E);var I=null;if(G){I=F.bind(D,this.getModel(),B,A);}
;var H=F.toHashCode();if(this.__jk[H]==undefined){this.__jk[H]=[];}
;this.__jk[H].push([C,I,D,B,E,A]);}
,removeTarget:function(K,J,L){this.__Kg(K,J,L,this.getModel());for(var i=0;i<this.__yW.length;i++ ){if(this.__yW[i][0]==K&&this.__yW[i][1]==J&&this.__yW[i][2]==L){this.__yW.splice(i,1);}
;}
;}
,__Kg:function(R,O,N,M){if(!(R instanceof qx.core.Object)){return;}
;var P=this.__jk[R.toHashCode()];if(P==undefined||P.length==0){return;}
